...
This directory starts empty in a fresh checkout — nothing here is required for `kanxeod` to run; a container recipe only exists once an operator publishes one (`kanxeoctl container recipe add NAME --file=PATH`, or `pkg sync` from a configured git repo). A worked example, matching this project's own `writing-recipes.md` convention of showing a real one rather than describing the shape abstractly:
...
```sh
kanxeoctl container recipe add echo-web --file=container.json
kanxeoctl container apply-recipe echo-web --secret=DEPLOY_TOKEN=a-real-value-never-committed
```
