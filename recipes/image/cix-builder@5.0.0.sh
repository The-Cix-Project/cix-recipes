#
# 5.0.0: libc-dev -> glibc (#187, ADR-0217). The entry below did not
# change job, only provider: libc-dev was Debian headers and CRT
# objects repackaged, glibc is the one this project builds from source
# (ADR-0216) and carries the same headers and the same crt1.o/crti.o/
# crtn.o. Pinned at 2.44-12, the revision every live builder on
# 192.168.15.95 is already running.
#
...
#   glibc     headers and the CRT startup objects (crt1.o/crti.o/
#             crtn.o), and the C library itself -- built by Cix,
#             not repackaged from a build host (ADR-0216)
...
#             not resolve.
#
image_packages="bash:pinned:5.2.37-2 binutils:pinned:2.42-9 coreutils:pinned:9.11-3 flex:pinned:2.6.4-4 gcc:pinned:16.2.0-11 glibc:pinned:2.44-12 m4:pinned:1.4.19-2 make:pinned:4.4.1-4 openssl:pinned:3.0.20-2 tcc:pinned:0.9.27-9 zlib:pinned:1.3.2-6"
