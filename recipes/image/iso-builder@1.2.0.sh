...
#
# Two of the ten are genuinely new here: fdisk 2.42.2-4 and dosfstools
# 4.2-1, the interactive partition editor and the ESP formatter. They
# join for the same reason shim and mokutil did -- mkinstalleriso used
# to read them from absolute paths that exist only on a Debian
# development box, so isotools now harvests them into its artifact and
# needs them installed here first.
#
#
