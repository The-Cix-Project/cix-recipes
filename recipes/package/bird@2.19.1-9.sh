...
pkg_build() {
	...
	#
	# Whitespace-tolerant on purpose. 2.19.1-8 matched "CC = " with
	# spaces and bird's Makefile writes "CC=/usr/bin/gcc" with none, so
	# the gate failed a build that had done exactly what was asked --
	# and printed the line proving itself wrong as it did. An assertion
	# whose pattern is wrong is a bug in the assertion, and the fix is
	# to match loosely and show the evidence, not to match exactly and
	# hope.
	grep -E "^CC[[:space:]]*=[[:space:]]*/usr/bin/gcc" Makefile >/dev/null || {
