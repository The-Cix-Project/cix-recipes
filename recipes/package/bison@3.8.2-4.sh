...
pkg_changelog="3.8.2-4: works around the gnulib/glibc-2.44 posix_spawn_file_actions_addchdir clash, same as m4. 3.8.2-3: declares its build tools so it can be rebuilt through the ordinary install path (#206)"
#
# Build tools derived rather than guessed: the baseline the declaring
# recipes converge on, plus what this recipe's own pkg_build() invokes
# and the libraries it already declares. See
...
pkg_build() {
	...
	CC=tcc CFLAGS="-D_GL_EXTERN_INLINE_STDHEADER_BUG=1" \
	    ...
	    # alias, so autoconf's link probe -- which deliberately declares the

	    # function itself rather than including the header -- concludes it is

	    # absent. gnulib then redeclares it on top of glibc's own declaration
	    ...
	    # Corrected via autoconf's own cache variable, which is truthful: the

	    # function IS available to any translation unit that includes the

	    # header, which is the only legitimate way C code calls it. That lands

	    # on gnulib's strictly safer REPLACE=1 branch. Full trail in

	    # recipes/package/m4/1.4.20-2/build.sh -- this is the same bug, and it
