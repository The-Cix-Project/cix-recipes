...
pkg_changelog="3.8.2-5: works around the gnulib/glibc-2.44 posix_spawn_file_actions_addchdir clash, same as m4. 3.8.2-3: declares its build tools so it can be rebuilt through the ordinary install path (#206)"
#
# Build tools derived rather than guessed: the baseline the declaring
# recipes converge on, plus what this recipe's own pkg_build() invokes
# and the libraries it already declares. See
...
pkg_build() {
	...
	# that genuinely does not exist and concludes the function is absent;
	# gnulib then redeclares it on top of glibc's own declaration:
	#
	...
	# includes the header, the only legitimate way C code calls it -- and
	# lands on gnulib's strictly safer REPLACE=1 branch. Exported rather
	# than prefixed on the command line so no line continuation is
	# involved; a blank line after a trailing backslash silently ends the
	# continuation and configure then sees no CC at all.
	#
	# Same bug as m4; full trail in recipes/package/m4/1.4.20-2/build.sh.
	#
