...
pkg_changelog="7.1-11: rebuilt so ADR-0251's finalize policy actually runs over these bytes (#328). No source or recipe change. The policy strips ELF and has done since 7c4a7332 on 2026-09-06, but it only runs when a package is BUILT -- and the artifact tier correctly installs an already-approved cached artifact rather than rebuilding, so bytes published before that date keep shipping unstripped indefinitely with nothing reporting it. This artifact was published 2026-09-02. Measured on the installer ISO: 5 of 318 ELF objects still carried .debug*, 3.93 MiB in total, mkfs.btrfs alone being 80% debug. The revision exists to make the rebuild real, the same reason ADR-0155 forces elsewhere. 7.1-8: also build and install the btrfs multi-tool, not just mkfs.btrfs -- ADR-0207 makes btrfs the storage substrate and nothing could maintain one (#163)"
