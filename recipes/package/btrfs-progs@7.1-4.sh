...
pkg_build() {
	...
	# flag.
	# EXTRA_CFLAGS: TCC does not predefine __SIZEOF_LONG__, and
	# kernel-lib/bitops.h uses it in real code (BITS_PER_LONG), so -3
	# stopped with
	#   ./kernel-lib/bitops.h:145: error: '__SIZEOF_LONG__' undeclared
	#
	# Worth recording how this was nearly misdiagnosed: probing the
	# macro with the DEVELOPMENT MACHINE's tcc says it IS defined.
	# That tcc is Debian's, which carries patches this project's own
	# tcc 0.9.27-9 does not -- our build is much closer to upstream
	# 0.9.27. A local probe of TCC behaviour is not evidence about the
	# compiler the box actually uses; only the box's own build is.
	#
	# 8 is not a guess: sizeof(long) is 8 on x86_64, the only
	# architecture this project targets, and EXTRA_CFLAGS is upstream's
	# own seam for exactly this -- no source is patched. Same shape as
	# the -Dlinux=1 fix CLAUDE.md records for squashfs-tools, and the
	# same discipline: supply a macro the compiler genuinely lacks,
	# with the value that is genuinely correct here.
	make -j"$(nproc)" DEPCOMMAND=true \
	     EXTRA_CFLAGS="-D__SIZEOF_LONG__=8" mkfs.btrfs
}
