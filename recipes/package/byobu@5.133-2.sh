#
# 5.133-2: declare the text tools byobu's scripts shell out to at
# RUNTIME -- sed, grep, gawk and coreutils. 5.133-1 listed only screen/
# python/procps/bash, so on a minimal image byobu-janitor died with
# 'sed: command not found' (measured in jump, 2026-09-13). These were
# in pkg_build_depends (build sandbox) but not pkg_depends (installed
# runtime) -- the same distinction git 2.55.0-8 and python 3.13.5-7
# needed. python still covers select-session.py; byobu-config's snack
# TUI (python-newt) is a separate optional tool, not packaged here.
#
#
...
pkg_depends="screen python procps bash sed grep gawk coreutils"
