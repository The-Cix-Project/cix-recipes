...
pkg_changelog="v0.1.23-3: build commit 0a5313f6, which is ahead of the v0.1.23 tag and has no tag of its own -- the version string is therefore inaccurate and this should be replaced by a tagged revision. Adds make test as a reported gate; the suite grew by repro, fuzz, seams, typed-package, policy and observe tests that had never run under this platform tcc. v0.1.23-2: define ZSTD_DISABLE_DEPRECATE_WARNINGS. The -1 build failed at zstd.h:72 -- TCC defines neither __GNUC__ nor _MSC_VER, takes the header fallback arm, and that arm emits a #pragma message TCC does not implement, which -Werror makes fatal. CBS keeps -Werror. v0.1.23-1: first build of cix-build-system on a Cix host, closing cix-build-system#124. TCC, against the Cix-built libarchive 3.8.1-4 and zstd 1.5.7-3. Builds and validates a real recipe; not integrated with cixd, and cix-build-system#132 remains open."
...
pkg_build() {
	...
	echo "==================== make test"
	make CC=tcc CPPFLAGS="-Isrc -DZSTD_DISABLE_DEPRECATE_WARNINGS" test; echo "make test rc=$?"
	echo "==================== end make test"
