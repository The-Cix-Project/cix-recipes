...
pkg_version="1.0.5"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.53.47.tar.gz"
pkg_sha256="42fb146b90a652e194213ecae7e34a3424fd8fc6dc9883dcae5327dfa4578afa"
pkg_depends=""
#
# Exactly what building four binaries needs, which is what cix.recipe
# itself declares. 1.0.0 copied cix-tests' list wholesale and was
# refused before it started: that list serves roughly seventy test
# binaries across every subsystem, and one of its entries, util-linux,
# is not installed anywhere on this host at all -- so cix-tests cannot
# currently build either, which is its own finding rather than this
# recipe's problem.
#
...
#
# The harness creates real containers, so its build container needs to
# be able to create a cgroup tree to put them in.
#
# src/container.c grants a nested cgroup2 mount only when the spec has
# BOTH CLONE_NEWCGROUP, which every build container gets, and
# CAP_SYS_ADMIN, which only a recipe can ask for. Without it, cixd
# starts inside the build container and then fails every container
# create with "cgroup_create_parent: mkdir: No such file or directory"
# -- which 1.0.0 and 1.0.1 both did. cix.recipe and cix-tests.recipe
# declare exactly this, for exactly this reason.
#
...
pkg_build() {
	...
	#
	# The harness separates "attacked it and it held" (0) from "attacked
	# it and it did not" (1) from "never got as far as attacking
	# anything" (2), and so does this recipe. 1.0.0 did not, and
	# announced that a build had been wedged by its own harness when in
	# fact the harness could not create a container and had measured
	# nothing at all. A verdict that invents a failure is worse than no
	# verdict, because someone then goes looking for a bug in the
	# daemon.
	#
	echo "=== 1/2: hostile run against this build -- must survive ==="
	rc=0
	build/test_aggressive || rc=$?
	if [ "$rc" -eq 2 ]; then
		echo "cix-aggressive-test: NO RUN -- the harness could not set itself up, so nothing"
		echo "                     was attacked and nothing is claimed about this build."
		exit 1
	fi
	if [ "$rc" -ne 0 ]; then
		echo "cix-aggressive-test: this build was WEDGED by its own harness (rc=$rc)"
		exit 1
	fi
	...
	#
	# Both invocations are guarded with || rather than run bare. A
	# recipe runs under set -e, so a check that returns non-zero kills
	# the script on that line and never reaches the $? below it -- 1.0.4
	# did exactly that and reported "build failed (exit status 1)"
	# instead of the confirmation that the harness had worked. That is
	# the same defect as tcc.recipe's #219 gate, diagnosed hours
	# earlier and then repeated here: an unguarded check is not a gate,
	# it is a silent exit.
	#
	...
	build/test_aggressive || rc=$?
	if [ "$rc" -eq 2 ]; then
		echo "cix-aggressive-test: the reverted-build check could not run, so the pass above"
		echo "                     is unverified and must not be trusted."
		exit 1
	fi
	if [ "$rc" -eq 0 ]; then
