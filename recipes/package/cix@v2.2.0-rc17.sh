...
#
pkg_name="cix"
pkg_version="v2.2.0-rc17"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.2.0-rc17.tar.gz"
pkg_sha256="8909ecffeb81022782632a4a9af09d4eda57e11c533b091e3d60cd4d3766391d"
...
#
# gcc and binutils join for exactly one output: build/cix-boot.efi, the
# UEFI boot manager (ADR-0215). Everything else here is still TCC, per
# the root Makefile's own CC := tcc.
#
# They are not optional and not a preference. UEFI uses the Microsoft
# calling convention, and TCC implements no __builtin_ms_va_list at all
# (ADR-0211 proved that directly), so it cannot compile EFI code. The
# link is `ld -m i386pep`, a PE/COFF emulation that only exists in
# binutils 2.42-9 and later here -- earlier revisions were configured
# for the default target alone.
#
pkg_build_depends="bash coreutils make tcc libc-dev openssl gcc binutils"
#
...
pkg_build() {
	make CIX_VERSION="$pkg_version" \
	    ...
	    build/mkinstalleriso build/cix-boot.efi
}

pkg_install() {
	cp build/cixd build/cixctl build/mkbootroot build/cix-install build/cix-recover \
	   build/mkinstalleriso build/cix-boot.efi "$PKG_DESTDIR/"
