...
pkg_version="v2.23.3"
#
# ONE-OFF SOURCE URL, and the reason is recorded rather than left to be
# rediscovered.
#
# v2.23.1 moved state to the config partition (ADR-0232) and, because
# boot_init mounted /config AFTER creating resolv.conf under it, left
# the host unable to resolve any hostname -- so the box could not fetch
# git.home.arpa, and the fix for that was inside the thing it could not
# fetch. This revision's source was served over plain HTTP by literal IP
# from the LAN for exactly one build, which is this project's documented
# unblock for a host that cannot resolve. pkg_sha256 below is what makes
# that safe: a different server cannot substitute different bytes
# without failing a gate that already exists.
#
# The URL is dead now. Any rebuild of this control plane should use a
# later revision, which fetches from git.home.arpa normally.
#
pkg_source="http://192.168.15.31:8099/cix-v2.23.3.tar.gz"
pkg_sha256="53abcf1ed1377976205fe0df7157ff1fb9d6d4559ac7c3b91cc77c4b7e40e4ac"
