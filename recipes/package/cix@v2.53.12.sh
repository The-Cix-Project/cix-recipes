...
#
pkg_name="cix"
pkg_version="v2.53.12"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.53.12.tar.gz"
pkg_sha256="8d5f561443623429a57981d9232c99f1315bf81e2435da9ecab590f32ae8fc41"
...
pkg_build() {
	...
	#
	# #224: which of the tests that run NOWHERE actually pass here?
	#
	# v2.53.9 settled the question the issue asked -- test_daemon and
	# test_networks both PASS in this container, uid 0 with
	# CAP_SYS_ADMIN, and nothing leaked onto the host. So option 1 is
	# viable and the remaining question is only which tests to promote.
	#
	# This runs every test target that is not already in SELFTESTS and
	# reports the outcome, so one build produces the whole matrix
	# rather than discovering it one release at a time.
	#
	# Each gets a timeout. A test that hangs here would hang every
	# future cix build, and finding that out from a report is much
	# cheaper than finding it out from a wedged release.
	#
	# DELIBERATELY NON-FATAL, for the same reason as v2.53.9: this
	# measures, it does not gate. Promotion into SELFTESTS is a
	# separate, deliberate edit once the matrix is known.
	#
	echo "=== #224 matrix: tests that currently run nowhere ==="
	# "all", explicitly. selftest is the FIRST target in the Makefile,
	# so a bare make runs that and builds only the sixteen binaries the
	# gate already had -- which is why v2.53.10 reported 55 of 73
	# MISSING and drew no conclusion about them. It exited 0 the whole
	# time, because it had done exactly what it was asked.
	make CIX_VERSION="$pkg_version" all >/dev/null 2>&1 || echo "  (build all had failures; probing what exists)"
	grep -oE 'BUILD./test_[a-z_0-9]+' Makefile | sed 's|.*/||' | sort -u > /tmp/224-all.txt
	sed -n '/^SELFTESTS/,/^$/p' Makefile | grep -oE 'test_[a-z_0-9]+' | sort -u > /tmp/224-gate.txt
	comm -23 /tmp/224-all.txt /tmp/224-gate.txt > /tmp/224-todo.txt
	echo "  gate=$(wc -l < /tmp/224-gate.txt) todo=$(wc -l < /tmp/224-todo.txt)"
	pass=0; fail=0; slow=0; missing=0
	while read -r t; do
		[ -n "$t" ] || continue
		if [ ! -x "./build/$t" ]; then
			echo "  MISSING  $t"
		...
		if timeout 120 "./build/$t" >"/tmp/224-$t.log" 2>&1; then
			echo "  PASS     $t"
			pass=$((pass + 1))
		else
			rc=$?
			if [ "$rc" -eq 124 ]; then
				...
				echo "  FAIL     $t rc=$rc: $(tail -3 "/tmp/224-$t.log" | tr '\n' ' ' | cut -c1-110)"
				fail=$((fail + 1))
			fi
		fi
		rm -f "/tmp/224-$t.log"
	done < /tmp/224-todo.txt
	echo "  TOTALS pass=$pass fail=$fail timeout=$slow missing=$missing"
	echo "=== end #224 matrix ==="
}
