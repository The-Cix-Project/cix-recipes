#
# v2.53.46 -- zero the console session struct (#294).
#
# v2.53.45 shipped a console that showed a prompt and then ignored
# every keystroke. struct console_exec_session is malloc()ed and never
# zeroed; every field used to be assigned by hand just below, so the
# allocation's contents never mattered until #294 added a buffer and
# its length to it. pty_out_len came up as garbage, the direct write
# was skipped, and terminal input was copied to a junk offset -- so the
# shell received uninitialised memory instead of keystrokes.
#
# The suite passed throughout, because every console scenario reads
# what the child says on attach and none writes a byte into the
# session. The whole input direction is unasserted; that is #296.
#
#
# v2.53.45 -- a console session becomes the container's own root (#293).
#
# v2.53.44 shipped the namespace join and not the identity inside it. A
# console into the jump container came up as I have no name!@jump, which
# is setns(CLONE_NEWUSER) reinterpreting the uid already held rather
# than carrying one across: the daemon is host uid 0, host 0 is not in
# the container's map, so the session landed on the overflow uid --
# correctly inside the namespace and owning nothing in it. setgid,
# setgroups and setuid to 0 after the join, which the capabilities
# gained by joining are what make legal.
#
# The test asserted the namespace and not the uid inside it, which is
# exactly how this shipped: its assertion was true of the broken
# session. It asserts both now.
#
#
# v2.53.44 -- the two selftest failures that stopped v2.53.42 shipping,
# plus the hostile harness.
#
# test_docindex was right: ADR-0246 shipped and its row in the ADR index
# did not. test_console_exec failed because the console child wrote four
# separate lines, and each is a separate write to the pty, a separate
# relay read and a separate frame -- four chances for a reader to see a
# partial answer, which is the intermittent shape of #291. It reports in
# one write now, so there is one frame or there is none, and none is
# unambiguously a failure rather than a coin toss.
#
# That run also confirmed #293 works: USERNS=mapped arrived, which is a
# console session reporting a real uid mapping from inside the
# container's user namespace.
#
#
# v2.53.42 -- the console pty master is non-blocking (#294), and a
# console session joins the container's user namespace (#293).
#
# #294 is what wedged this box for seventeen minutes and cost a manual
# reset. stallwatch had it five seconds in and said so twelve times:
# state S, wchan n_tty_read, activity empty -- alive, inside a tty
# read, with no request in flight. cixd reads exactly one kind of tty,
# a console session's pty master, and that fd was opened blocking and
# read from the epoll loop. epoll says a readiness notification may be
# spurious, so a blocking read after EPOLLIN is an unbounded hang by
# construction, and on a daemon with one loop that hang is the whole
# control plane. O_NONBLOCK is per file description, so it fixes the
# write side too, which had the identical defect once a slave stops
# reading. The write buffers what the master will not take and drains
# it on EPOLLOUT, because EAGAIN killing a session over transient
# backpressure would be a different bug rather than a fix.
#
# #293: join_namespaces entered mnt, uts, net and pid and not user, so
# a console session ran with host credentials inside the container's
# mount namespace -- it read every on-disk id unmapped and a login
# could not reach the home directory of the account it had just
# authenticated. The same login over SSH always worked, which is what
# located it. The user namespace is joined first, because the
# capabilities gained in it are what make the following mount join
# legitimate. The pty slave is handed to the container's own root
# before that, since the daemon allocates it as host uid 0 which is
# unmapped there, and container root cannot override an inode whose
# owner is unmapped.
#
#
...
#
pkg_name="cix"
pkg_version="v2.53.46"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.53.46.tar.gz"
pkg_sha256="87dc735969f4076fe2fc42127f3011c6affe74c8d98ac2d83df724c95a539760"
...
pkg_build() {
	...
	#
	# #224 tail: the six failures nobody has looked at.
	#
	# Sixteen of the twenty-two that fail in a build container do so for
	# a named environmental reason -- a hand-fetched kernel image, the
	# artifact floor, dnsmasq, a GPU, loop devices, host libraries.
	# These six are not those. They fail for reasons no one has read,
	# and the matrix kept only their last three lines, which is enough
	# to know they failed and not enough to know why.
	#
	# Full output for each, once, so one build settles whether any is a
	# real defect the suite has simply never been able to report.
	# Non-fatal: this is a read, not a gate.
	#
	echo "=== #224 tail: full output from the six unexplained ==="
	make CIX_VERSION="$pkg_version" all >/dev/null 2>&1 || true
	for t in test_slow_client test_ldap; do
		echo "----- $t -----"
		if [ ! -x "./build/$t" ]; then
			echo "  (binary absent)"
			continue
		fi
		if timeout 180 "./build/$t" >"/tmp/six-$t.log" 2>&1; then
			echo "  PASSED THIS TIME -- non-deterministic, which is itself the finding"
		else
			echo "  rc=$?"
		fi
		grep -nE "ldap: |FAIL|POST |PUT " "/tmp/six-$t.log" | tail -45 | sed 's/^/  /'
		rm -f "/tmp/six-$t.log"
	done
	echo "=== end #224 tail ==="
}
