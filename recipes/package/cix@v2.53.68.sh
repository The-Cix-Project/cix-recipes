#
# v2.53.68 -- ADR-0249 up to sixteen handler modules.
#
# pki (the CA tier: root, intermediate, certificates, reset) and
# storage (devices, device mappings, disks, disk roles, formatting,
# unmounting and the partition table -- 1,024 lines, the largest single
# group in the file) bring it to sixteen. main.c 31,284 -> 25,361:
# 5,923 lines gone, 19%.
#
# reissue_host_pki_cert() moved into api_pki.c and is exported, since
# three of its four callers are pki handlers. g_base_dir joined the 66
# paths in daemonpaths.h, being the root every one is derived from.
#
# test_lint caught its third defect of this refactor and the pattern is
# worth recording: a header declaring respond_diskformat_error(int,
# enum diskformat_error) without including diskformat.h. C gives an
# enum first named inside a prototype function-prototype scope, so the
# declaration and the definition are different types, and TCC compiled
# both without complaint three times running. Every api_ header has
# since been audited for this class.
#
# Two handlers stayed under rule 2: handle_disk_format_post() registers
# the format job's pidfd with the event loop, and
# handle_backup_config_put() re-arms a timerfd.
#
pkg_name="cix"
