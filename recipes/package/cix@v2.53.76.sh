#
# v2.53.76 -- identical code to v2.53.75; this revision exists to run a
# control-plane assembly that v2.53.75 could not.
#
# v2.53.75 built cleanly and then failed assembly with
# "/usr/bin/btrfs: No such file or directory": mkbootroot stages that
# binary from cix-hosttools, and that image did not contain
# btrfs-progs. cix-hosttools 2.1.0 fixes the image.
#
# A rebuild is needed rather than a re-run because assembly has no
# trigger of its own -- it happens only as a side effect of a hostbuild
# named cix completing, and a completed hostbuild entry cannot be
# cleared (DELETE /v1/pkg/{name} has no way to name the __hostbuild
# image). Worth an issue: GET /v1/system/assembly reports a lifecycle
# domain that nothing can start.
#
# What it carries is v2.53.75s work: ADR-0251s package finalize policy
# and ADR-0252s decision that an image recipe is authoritative, plus
# v2.53.74s device-resolution fix.
#
pkg_name="cix"
pkg_version="v2.53.76"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.53.76.tar.gz"
