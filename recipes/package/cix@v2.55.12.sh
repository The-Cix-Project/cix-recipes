#
# v2.55.12: {{LDAP:URI}} derives from the registered LDAP servers.
#
# The token read client_uri directly and left itself verbatim when that
# field was empty, while the daemon's own nslcd.conf rendering has
# always used ldap_effective_client_uri(). One container create then
# produced two files disagreeing about the same servers, and the token
# resolved only where an operator had hand-typed a copy of the server
# list the daemon already tracks -- the duplication #66 was filed to
# remove, and which its own spec says this token should be rendered
# from.
#
# nslcd got the good value, so password and NSS lookups worked while
# sshd's AuthorizedKeysCommand searched a nonsense URI and reported no
# keys for the user -- the broken half of the chain being the quiet
# one. #327.
#
# test_container_recipe's LDAP scenario set an explicit client_uri, so
# it only exercised the branch that already worked. The new scenario
# registers a network-attached container as an LDAP server, clears
# client_uri, and asserts the rendered file carries that server's own
# live address and no literal token.
#
pkg_name="cix"
pkg_version="v2.55.12"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.55.12.tar.gz"
pkg_sha256="367afa51ab9be963f79324ed5c85ddd0a689ca9c65f5359d1398ba233fedc142"
pkg_artifact_sha256="9de3fdbd2d24bb68cd59aaac7fa6b556cde2d0772abec07e52565f76a6efd762"
