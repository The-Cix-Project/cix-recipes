#
# v2.55.14: the Storage tree places volumes, and a volume has a glyph.
#
# The tree appended volumes flat onto the disk list, so jump-home
# rendered level with vda and sda and said nothing about which
# partition it consumes. deviceHoldingVolume() already derived the
# answer and diskTreeChildren()/orphanVolumes() were written to use
# it -- all three existed, and the last two were called from nowhere.
# Wired in: jump-home now sits under vda5, which is cix-containers
# mounted at /var/lib/cix, where its host_path actually lives.
#
# Volumes also asked for the icon "rebuildable", which is not a
# TREE_ICONS key, so they drew with no icon at all rather than a
# wrong one. New volume glyph, a stacked drum -- deliberately not the
# storage platter, since a volume sits inside a partition and the two
# must not look alike at 14px.
#
# Replaces v2.55.13, which was withdrawn rather than deployed: the
# commit it was cut from mixed four unrelated changes, and splitting
# that commit rewrote the tag's own archive -- which would have left a
# published, immutable recipe pinning a sha256 git could no longer
# reproduce. Its recipe and artifact were removed with it.
#
# #329.
#
pkg_name="cix"
pkg_version="v2.55.14"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.55.14.tar.gz"
pkg_sha256="acbfef138e01e91395092cd2041526313ae3c8a06fc22b65c1ae5d9efa01e9ea"
pkg_artifact_sha256="6d23baa970114b6fd415e1adfcd7f623c156b3284dbd50f921fe748e82adc84a"
