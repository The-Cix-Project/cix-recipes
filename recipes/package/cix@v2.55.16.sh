#
# v2.55.16: no code change -- a rebuild so the control-plane root
# carries the stripped host tools (#328).
#
# mkbootroot stages cixd's shell-out tools (sfdisk, mkfs.ext4,
# mkfs.btrfs) into the control-plane root from cix-hosttools, and
# mkinstalleriso in turn stages them into the installer media from
# THIS BOX'S OWN running root -- not from the image. So btrfs-progs
# 7.1-11, which removed 2.94 MiB of debug sections from mkfs.btrfs,
# reaches an ISO only after the box boots a root assembled with it.
#
# The first ISO built after the package rebuilds dropped just 0.54 MiB
# of the expected 3.93, which is what surfaced this: the assembled
# squashfs inside the payload was fresh, and the /usr/sbin binaries
# beside it were the ones the running root still had.
#
# The revision exists to make the rebuild real -- POST hostbuild at an
# already-installed version is refused with 409, the same reason
# ADR-0155 forces a bump elsewhere.
#
pkg_name="cix"
pkg_version="v2.55.16"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.55.16.tar.gz"
