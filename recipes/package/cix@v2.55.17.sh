#
# v2.55.17: ADR-0259 -- an attachment's interface may be named.
#
# Interface names inside a container were positional and nothing else
# (container_net.c derived eth<idx> from the attachment index). An
# attachment may now carry ifname; absent it the same eth<index> is
# assigned, so no existing container changes.
#
# Needed on the box before cr-1/cr-2 can gain BIRD: a routing daemon
# should name the interface it means, rather than depend on management
# happening to be the first entry in a recipe's networks array -- a
# dependency that is real, load-bearing, and expressed nowhere.
#
pkg_name="cix"
pkg_version="v2.55.17"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.55.17.tar.gz"
pkg_sha256="5e4fafc3c0a58052ab463ddab27fce5778cd1268cc7b6a5714dfe4bed35688cd"
pkg_artifact_sha256="3efa431c55bdeabbb7c7a12db96a9410b549803f854e78200b1a46d5f2f66556"
