#
# v2.55.18: ADR-0260 -- a container declares the services it runs, and
# cix-init is pid 1 in every one.
#
# cmd is gone from the container body; services[] replaces it. cix-init,
# freestanding (-nostdlib -static, no libc, no header), is built by this
# recipe beside cixd, shipped in the artifact, staged into the
# control-plane root by mkbootroot, and copied by cixd into every
# container's tree at creation. Build containers are containers like
# any other: this very build runs as one oneshot named "build" under
# cix-init once the daemon carrying this is deployed.
#
# The selftest gate this recipe runs now includes test_cix_init and
# test_cixinit_table, both measured in a composed build container by
# probe-cix-init/1..5 before being listed.
#
pkg_name="cix"
pkg_version="v2.55.18"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.55.18.tar.gz"
pkg_sha256="5e4fafc3c0a58052ab463ddab27fce5778cd1268cc7b6a5714dfe4bed35688cd"
