#
# v2.55.19: the ADR-0260 migration preserves the old cmd semantics.
#
# v2.55.18's own selftest refused it, which is the gate working. Every
# container body in the tests and recipes had been translated to a
# oneshot, and a oneshot is ready only once it has exited 0 -- so a
# long-running process declared as one never became ready and autostart
# blocked 35s on each, before the event loop, with the daemon serving
# nothing meanwhile.
#
# The faithful translation is a daemon with on_exit: fail-container:
# ready when it starts, and its exit ends the container with its
# status, which is what one cmd did. Per-service restart stays opt-in
# rather than arriving as a side effect of the migration.
#
# Also: autostart now blocks on readiness only when another definition
# depends on that container.
#
pkg_name="cix"
pkg_version="v2.55.19"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.55.19.tar.gz"
pkg_sha256="a284efcc76905ea4a33757e3f6b2a6ca7b6e5137c9af011d074ab08c9beec299"
