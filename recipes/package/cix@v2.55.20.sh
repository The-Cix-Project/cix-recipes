#
# v2.55.20: services is editable by PATCH after all.
#
# v2.55.19 put `services` on PATCH's refused list in place of
# `readiness`. The two are not alike: readiness fed the definition
# index containerdef caches, services feeds nothing and is parsed from
# the persisted body at each create, exactly as cmd was. Refusing it
# took away issue #11's point. Its own test caught it.
#
# Also: test_cli's last two `-- CMD` argv arrays, and a network-cleanup
# failure that now names which container is still holding the network
# instead of reporting a bare 409.
#
pkg_name="cix"
pkg_version="v2.55.20"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.55.20.tar.gz"
