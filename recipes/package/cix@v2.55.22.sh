#
# v2.55.22: a container is stopped by a message, not a signal.
#
# pid 1 of a PID namespace is SIGNAL_UNKILLABLE and the kernel DISCARDS
# any signal it has not yet installed a handler for. execve() leaves
# every disposition at SIG_DFL until the new program calls sigaction(),
# so a SIGTERM in that window is gone rather than queued, and the
# container waits out the whole SIGKILL grace. One container in
# test_daemon_net hung every run for exactly this reason -- the one the
# test creates and deletes milliseconds apart.
#
# The stop is now CIXINIT_OP_SHUTDOWN on the control socket, which
# waits in the buffer until cix-init reaches its loop; SIGTERM is the
# fallback and SIGKILL still stands behind both.
#
pkg_name="cix"
pkg_version="v2.55.22"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.55.22.tar.gz"
pkg_sha256="e63f8eaaf45b517181a6f254d3b70ee65be20d2773ee45aed880097b0d8726c0"
