#
# v2.57.11: modprobe's reason reaches the operator, and a load that did
# nothing stops reporting 200.
#
# Both found by running #347's acceptance test on a real box the moment
# modules could load at all. DELETE /v1/system/kmod/usb-storage
# answered 404 "modprobe -r could not unload this module" and nothing
# else -- kmod_load()/kmod_unload() captured modprobe's merged output
# into a pipe and read it into a discard buffer, so the one party that
# knew why could not say so.
#
# Separately, modprobe on an already-loaded module exits 0 and does
# nothing, so POST with options answered 200 and echoed back parameters
# the kernel had never seen. That case is 409 now.
#
pkg_name="cix"
pkg_version="v2.57.11"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.11.tar.gz"
pkg_sha256="ce51a923d351ac99c9609a6c4c779d205d6c9140607ec23e6e2b6559a5af1d0a"
