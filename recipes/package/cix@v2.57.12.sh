#
# v2.57.12: v2.57.11 plus the blocking-waits budget it should have
# carried.
#
# Replacing load_boot_modules()'s hand-rolled fork+execve with
# kmod_load() removed a waitpid(), and test_blocking_waits refuses a
# build that goes UNDER budget without saying so -- a budget nobody
# tightens is a ceiling that only ever rises. 27 -> 26, 63 -> 62.
#
# Everything else here is v2.57.11: modprobe's own reason now reaches
# the operator on both load and unload, and POST with options on an
# already-loaded module is 409 rather than a 200 that applied nothing.
#
pkg_name="cix"
pkg_version="v2.57.12"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.12.tar.gz"
pkg_sha256="b9973976671ca278980e8680a69bf60381b26aaa6eb14d149843ab9d8dc2ea8c"
pkg_artifact_sha256="97ded03e5704dcad6a0265e4e3801381e582bf489c12f21db38506a0f6a867c2"
