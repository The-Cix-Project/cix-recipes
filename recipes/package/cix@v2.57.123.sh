#
# v2.57.123: a TCP readiness probe could never report ready (#413)
#
# cix-init's probe_connect() opened SOCK_STREAM|SOCK_NONBLOCK, called
# connect(), and read only rc == 0 as ready. A non-blocking TCP connect
# returns EINPROGRESS even when the peer is listening locally and the
# handshake has already finished, so that could never return 1 for
# AF_INET on any kernel -- every tcp_port probe in the platform has
# always ended in its timeout, which cix-init treats as ready, so the
# failure was recorded and discarded on every start.
#
# Measured by probe-ready-tcp/3 in a build container, kernel 7.2.3: a
# listening loopback port gives -1 EINPROGRESS then poll(POLLOUT, 0)
# revents 0x4 with SO_ERROR 0; a port with nothing listening also gives
# EINPROGRESS, then revents 0x1c with SO_ERROR 111. AF_UNIX through the
# same function returns 0, which is why nslcd's socket probe worked and
# hid this.
#
# Now probe_connect_unix() plus probe_tcp(), which carries the in-flight
# connect in probe_fd the way probe_pid already carries a command probe.
# test_container_restart, already gated, asserted only pid ordering and
# probe echo -- both satisfied by a probe that times out -- and now
# asserts depR's listener has no failure and neverready's is
# probe-timeout.
#
#
pkg_name="cix"
pkg_version="v2.57.123"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.123.tar.gz"
pkg_sha256="6cb7e7d6f70d9d32cbbd015002055261e1e4ddbc41438cc0dbbe02814a8810d6"
