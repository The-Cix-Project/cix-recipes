#
# v2.57.13: setting a module parameter can be confirmed (#354).
#
# GET /v1/system/kmod/{name} reported `params` -- modinfo, what the
# module ACCEPTS, which reads identically whether the module is loaded
# or not. Nothing reported what the kernel was RUNNING WITH. Not a
# formality: modprobe on an already-loaded module exits 0 without
# applying anything, a parameter can be rejected, and a read-only one
# keeps its built-in default, so "it worked" was an inference from an
# exit status.
#
# current_params is /sys/module/<name>/parameters, reported beside
# params. Found because #347's acceptance test asserted the option was
# readable back by looking in modinfo, which lists delay_use whether or
# not the kernel ever saw it -- an assertion that could not fail for
# the reason it was written to catch.
#
pkg_name="cix"
pkg_version="v2.57.13"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.13.tar.gz"
pkg_sha256="42b4403d9933baa28da17597f1c6b748cb534260103a0a64152807615be13f38"
pkg_artifact_sha256="07bd95036b91a1f4deea38ff60bb750db3da2d0d868d9c09fe984204d980fd4f"
