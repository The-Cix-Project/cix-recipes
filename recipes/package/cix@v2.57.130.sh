#
# v2.57.130: four cheap fixes -- a contract error, a publish-time gate,
# a tab collision and the scroll jump (#420, #408, #425, #432)
#
# #420: openapi.yaml declares /storage-roles; its prose said
# /diskroles 22 times, README's endpoint table included, and that name
# reached the public website. The GET response schema also said
# required: [diskroles] against a declared property of storage_roles --
# a required list naming a property that does not exist, so the
# document was invalid and wrong about the live API, which returns
# {"storage_roles":[]}.
#
# #408: test_recipe_hygiene refuses command substitution in a pkg_*
# value. The build container sources the recipe, so zstd@1.5.7-2's
# backticked changelog ran `sed:` as a program and died with the very
# message it was quoting; ADR-0107 makes that revision permanently
# unbuildable, and it has happened at least three times. All eight
# existing violations sit in superseded revisions and this test already
# scans latest revisions only, so the gate starts clean with no
# grandfathered list. The predicate was checked against all 1404 recipe
# files first: exactly those eight, zero latest.
#
# #425: a container detail's tab bar is exempt from address navigation
# wholesale rather than by name. "packages" collided with the
# top-level #packages route, so clicking it left the container.
#
# #432: unchangedAndRendered() skips a re-render when the payload is
# unchanged AND the panel has rows -- both halves load-bearing.
# Applied to the build-log and images tables, which share the
# pkg-build-config tick, so opening a log no longer costs the reader
# their scroll position. The other 44 blank-and-rebuild sites stay open
# on the issue.
#
#
pkg_name="cix"
pkg_version="v2.57.130"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.130.tar.gz"
pkg_sha256="ebf40eaf7a5928c43a4f4a5a476fa8934e87951ac9f77a5c8f7153e9b7f0b711"
pkg_artifact_sha256="d7db96350797f7747a25e9bcab72aca7c1f0232c6cfc4676b7edf67df7181d8e"
