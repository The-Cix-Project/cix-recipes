#
# v2.57.133: the platform's own containers are out of the container
# list by default (#426)
#
# GET /v1/containers returned every registry entry unfiltered, so while
# builds were running it listed __pkgbuild-<chain index> alongside the
# operator's own -- one per chain slot, up to max_concurrent_jobs,
# which is ten on a real host. A busy box showed ten machine-owned rows
# among a dozen real ones, told apart only by reading the names.
#
# Now excluded unless ?include_internal=1 asks for them, with
# hidden_internal reporting the count left out. That count matters: an
# operator watching a build who cannot find __pkgbuild-0 should be told
# it is hidden, not left to conclude the container is gone.
#
# The filter lives on the endpoint rather than in each client, which
# was the real decision in the issue. A dashboard-side filter would
# have given the web a view `cixctl container ls` could not have, and
# put "what counts as internal" in two languages free to drift. So
# registry_name_is_internal() is the one definition, and both surfaces
# reach it through the same parameter: container ls --all, and a
# remembered dashboard toggle that is off by default as asked.
#
# Only the live registry list needed changing -- a build container is
# registered under a synthetic image and has no def at all, so
# containerdef_write_json_inactive_list() can never emit one. And
# nothing depended on the old behaviour: test_pkg's __pkgbuild
# references read the pkg entry's build_container field, not the list,
# and reaching a build container by name is a lookup rather than a
# listing.
#
# show running-config excludes them unconditionally: a transient job
# the daemon created for itself is not configuration.
#
#
pkg_name="cix"
pkg_version="v2.57.133"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.133.tar.gz"
pkg_sha256="7fda202daa596c6dcc87cebf45879a0687b2cf3366eaafc42cec5ed0047b405a"
pkg_artifact_sha256="794f1231f69d1d1c989d1c867cd7f0ca5999b696edb47f9764e71b4531e89bf5"
