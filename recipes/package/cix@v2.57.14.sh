#
# v2.57.14: GET /v1/devices says who is holding a device (#356).
#
# Nothing on that endpoint meant "in use". `assignable` looks like the
# answer and is not -- it means the host considers this grantable,
# computed per bus with no reference to any grant. Nothing refuses a
# second grant either, so the same USB/PCI/GPU device could be handed to
# any number of containers, each getting its own BPF_CGROUP_DEVICE rule
# for the same major/minor, with no way to notice.
#
# Two of four buses already solved this -- a moved interface stops being
# enumerated, a bridge-enslaved one reports assignable:false, a
# role-assigned disk is excluded -- and device_enumerate() already took
# os_containers_dir and passed it only to enumerate_disk(). The
# machinery existed and was never extended.
#
# held_by is supplied as a callback so device.c still knows nothing
# about containers, and is matched against the RESOLVED grant id rather
# than the spec text, because a named mapping can expand to several
# devices.
#
pkg_name="cix"
pkg_version="v2.57.14"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.14.tar.gz"
pkg_sha256="b76fead7cfba54aa1c3d1ba4b5248e04bb264986f3f72f9b78e32bcb382724b1"
pkg_artifact_sha256="71b1b9297d7bed3155037a62f77cd17ef387ed29c7acb10d7c16b149f27a8be1"
