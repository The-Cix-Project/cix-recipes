#
# v2.57.140: the CLI tree guard runs both ways (#437)
#
# Reported as "it does not do second or third parameter completion". The
# completion engine was never the problem -- it walks three levels and
# always has. The table was incomplete and test_clitree only looked one
# way: it verified every flag in cmdtree.h exists in the parser, never
# the reverse, and its own comment argued the reverse could not be done.
#
# Measured: 239 flag literals in cli/src/main.c against 207 in the
# table. 33 flags the parser accepts and completion never offered, plus
# pki export and pki import missing as whole subcommands.
#
# All 33 placed by attributing each literal to its enclosing function.
# Two new checks close the direction, asking "anywhere in the table"
# rather than "on the right node" -- 85 of 274 paths have no matching
# cmd_* function, so name-based attribution would invent failures.
#
# Comments are stripped before scanning and flag shape is validated;
# without either, the guard produces false negatives and false positives
# respectively, both measured.
#
pkg_name="cix"
pkg_version="v2.57.140"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.140.tar.gz"
pkg_sha256="44710ee18b26f66c3e24a039180246034487bb52c3254cb2532d631426cb4623"
pkg_artifact_sha256="b675cfa9ff3e72b8292c4d016af5632cdebd9006db49845e19d5bb5a92b0b95d"
