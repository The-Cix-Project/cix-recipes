#
# v2.57.141: cixd always binds loopback (ADR-0284)
#
# Verifying ADR-0283 on a live host destroyed that host's reachability.
# The address a box answers on was recorded in THREE places, not one:
# net.conf, the network registry, and the loader entry's --bind=<ip>.
# The endpoint updated the first two; at the next boot cixd bound the
# third, because bootstrap_management_network() assigns the global
# g_bind_addr while main() passed its own argv-derived local to the
# listener. EADDRNOTAVAIL, non-zero return, and -- cixd being PID 1 --
# #131's park banner, which also took away the console shell cixd forks
# onto /dev/tty0 and /dev/ttyS0. Recovery needed an edit to the ESP.
#
# The owner's fix, and it is a subtraction: cixd ALWAYS binds 127.0.0.1
# and the chosen address is a second listener, never an alternative. The
# console shell and the stall watchdog talk to loopback, so neither can
# be broken by a management address change. The listener binds
# g_bind_addr. A chosen address that cannot be bound is logged, not
# fatal, and net.conf is left alone so the next boot retries it.
#
# Also: a same-subnet move now sets promote_secondaries on the bridge
# first and refuses if it cannot -- measured, deleting the primary with
# it at 0 FLUSHES the new secondary and leaves the bridge with no host
# address at all.
#
pkg_name="cix"
pkg_version="v2.57.141"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.141.tar.gz"
pkg_sha256="69a2bea4d3c289890e87a014045694f4234a3a83c5851d1984574a559e2e5bbf"
pkg_artifact_sha256="6dd908bc39e6f46b3553beea5d0ac1f451fabbad6b40729393e9e4472d072697"
