#
# v2.57.144: a container sees its own /proc/partitions (#452, ADR-0286
# tier 1), plus the v2.57.143 payload it supersedes -- the dashboard
# survives blocked localStorage (#453), a listener rebinds on a live
# management-address move (#454), a log tail stops once it has enough
# (#450), git declares zlib at run time (#455), and jump's recipe gains
# dns_servers.
#
# ADR-0286 tier 1: cix-procfuse serves a tenth file, /proc/partitions,
# rendered from the reader's own mountinfo so a container sees only the
# device backing its own mounts (vdb5) instead of every disk on the
# host (sda, sr0, vda and the pool's sibling partitions). Tier 2 (btrfs
# statfs reporting the qgroup limit, so df shows the quota) is a
# separate kernel change and is NOT in this build.
#
pkg_name="cix"
pkg_version="v2.57.144"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.144.tar.gz"
pkg_sha256="6aa3abdbd0befea706d22f5cdff556b9ce0058c8f2beaa2862abaf08a3b7cbd3"
