#
# v2.57.15: a second grant of a device is refused unless both sides set
# "shared" (#356).
#
# v2.57.14 made the problem visible; this is the policy. Measured before
# it: creating a second container with a device the first already held
# returned 201, and held_by then listed both -- two BPF_CGROUP_DEVICE
# rules for one major/minor with nothing saying so.
#
# BOTH grants must set shared, the newcomer and every existing holder.
# If only the newcomer had to opt in, a container that asked for a
# device without saying shared would silently lose exclusive use because
# someone else asked nicely. Default false: silence reads as "I need
# this to myself".
#
# Checked per RESOLVED device, so a vendor_model mapping or a gpu:N
# group with one contended member refuses that device rather than
# letting the rest through.
#
pkg_name="cix"
pkg_version="v2.57.15"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.15.tar.gz"
pkg_sha256="59ee348c7007f28a38d27ded87701d387cf3ba527e7ab36b5aa218417405f23d"
