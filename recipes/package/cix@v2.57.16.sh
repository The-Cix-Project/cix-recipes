#
# v2.57.16: GET /v1/dhcp/leases always returned an empty list (#357),
# plus v2.57.15's device exclusivity (#356), which never deployed.
#
# Found by the owner connecting a phone to the platform's access point
# and seeing no leases. The endpoint answered 200 {"leases":[]} while
# GET /v1/containers/dns-1/files?path=/run/dnsmasq.leases returned a
# real lease for a real client -- worse than not existing, since an
# operator reads that as "no clients".
#
# dnsmasq writes the lease file under /run, which src/mountns.c mounts
# as a fresh tmpfs at every container start, so the writable layer
# underneath is masked and the file has no on-disk existence. The read
# used container_file_host_path() -- correct for the daemon-written
# config beside it, wrong for a file the container writes. It now goes
# through /proc/<pid>/root like handle_container_file_read() does.
#
# create_container_from_body() already refuses a files[] entry under
# /run for exactly this reason. The rule was known in one file and lost
# in another.
#
pkg_name="cix"
pkg_version="v2.57.16"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.16.tar.gz"
pkg_sha256="5e37e01fc017a638f43c3d690a336990d1503964012a9479f3fda127c98e3c39"
pkg_artifact_sha256="db7e2551ade5c1a528fe756cea43e719d90e84fb494b174af894c04002474151"
