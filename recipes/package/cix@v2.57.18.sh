#
# v2.57.18: the host-side FUSE server for a container's own limits
# (#336, ADR-0262 part 1).
#
# One host-side process, forked and supervised the way stallwatch is,
# answering /proc/meminfo, cpuinfo, stat, uptime, loadavg and swaps from
# the REQUESTER's cgroup -- resolved from fuse_in_header.pid through
# /proc/<pid>/cgroup, which is what makes one server correct for every
# container rather than one server per container.
#
# The measurement ADR-0262 demanded first: a build job's cpu.max is
# "100000 100000" (1.0 CPU) while nproc inside reports 2, so a
# quota-derived cpuinfo would turn every make -j$(nproc) into -j1.
# cpuinfo follows cpuset.cpus.effective instead.
#
# Part 1 is the server and its mount; nothing about any container
# changes yet. This build exists to prove 869 lines of new C compile
# under -Wall -Werror with the platform's own tcc, and that the mount
# comes up on a real host.
#
pkg_name="cix"
pkg_version="v2.57.18"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.18.tar.gz"
pkg_sha256="8c6802e1dc9bb69b4d0fd134cb53fcd9e5e2c35ec6d43eb6ced1c7e876aef0cc"
