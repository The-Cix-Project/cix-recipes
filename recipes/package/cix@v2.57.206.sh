...
# kept here for exactly one version because of that. The daemon that
# will build this recipe is v2.57.205, which still reads the field and
# roots the build container on the named image; the daemon this recipe
# produces ignores it entirely and composes the container from
# pkg_build_depends above. So this is the one-version skew a clean
# cut-over on a field the builder reads always costs -- the same shape
# as the CLI/daemon skew already documented in CLAUDE.md -- and not a
# field with a future. v2.57.207 onward carries no pkg_build_image, and
# the parse line that read it is gone from pkg.c as of this build.
#
# Its previous note, kept because the reasoning is still the record of
# why "cix-builder" and not something else: ADR-0208 gives cix-builder
# the one job of building Cix. This said "toolchain" -- an image no
# recipe in this repo describes, which existed only on one host and
# died with it. cix-builder is what the guide documents and what the
# taxonomy names (#305).
