#
# v2.57.21: the procfuse diagnostics of v2.57.20, plus the #309 test
# fixes that let its build actually report on them.
#
# v2.57.20 failed its gate twice on
# "DELETE neta (now unused) expected 204, got 409" -- identical both
# times, which read as a regression. It was not. test_networks'
# wait_container_gone() gave up SILENTLY after 5s, so a teardown slower
# than the budget surfaced one call later as a failure naming the
# network delete. That wait now reports what it saw, and
# test_cix_init's three whole-tv_sec bounds -- thresholds that float
# across a full second on clock alignment -- are milliseconds.
#
pkg_name="cix"
pkg_version="v2.57.21"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.21.tar.gz"
pkg_sha256="cd856ee397b84dba1f90a23d1c898c3f351dc21ff2ea5f428f7ad76f4f03d6da"
pkg_artifact_sha256="38e4ed2909473b2c08105740c14749508370aa6a7412e461d389a61a64243556"
