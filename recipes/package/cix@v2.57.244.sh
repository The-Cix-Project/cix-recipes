#
# v2.57.244: the control-plane root carries CPDL engine v0.1.34.
#
# NO CIX CODE CHANGE. This release exists to re-stage /usr/bin/cbs.
# The daemon validates every published recipe with the HOST's engine
# -- PKG_CBS_BIN "/usr/bin/cbs", exec'd by a cixd that is PID 1 --
# and mkbootroot stages that binary out of cix-hosttools. So a newer
# engine installed into an image is invisible to `pkg recipe add`
# until a root is assembled with it, which is exactly what happened:
# cbs@v0.1.34-1 was installed into cix-builder AND cix-hosttools,
# and publishing a recipe using a v0.1.34 operation was still
# refused with
#
#   error[CPDL-E2001]: parse: expected `phase operation`, found `case`
#
# Seven operations arrive with it, every one filed from the recipe
# corpus in a single day of conversions and every one found by a
# recipe failing rather than by reading the specification:
#
#   cbs#205  non-archive main sources      -- unblocks
#            ca-certificates and rtw88-firmware AT ALL; neither has
#            a tarball to point at
#   cbs#206  stderr capture and assertions -- restores bird's
#            --version gate
#   cbs#207  globs as run arguments        -- hostapd's link line
#   cbs#208  literal ${} in a block string -- hostapd's toolwrap
#   cbs#209  glob bindings across phases   -- hostapd's binding
#   cbs#210  anchored source replacements  -- libcap's shebang
#   cbs#211  named post-install check cases
#
# cbs#211 is the one that changes how recipes are written. Across
# the 98 latest .cbs recipes: 367 verification operations, and
# THREE recipes using the check phase built for them. The cause was
# structural -- check ran before install, so it could not see the
# staged artifact, and 239 of 315 require operations assert on
# ${dest}.
#
#
# v2.57.243: a recipe's mirror urls are tried, not parsed and thrown
# away (#507).
#
# CPDL lets a source declare several urls -- ordered mirrors for one
# source identity, sharing that source's one checksum. cbs parses
# them all, stores them all, emits them all in explain, and its own
# fetcher walks the list. The daemon read urls[0] and dropped the
# rest, and struct pkg_recipe had nowhere to put a second url anyway.
# So a mirror list validated, published, and did nothing.
#
# freetype@2.13.3-7 declared three urls after savannah began refusing
# this host's TLS connections and failed at fetch naming only the
# first. Not egress: libnl fetched from github and net-tools from
# sourceforge in the same window, with 13.4 GiB free and health ok --
# checked first, because a full disk is what a flaky mirror looked
# like in #503.
#
# The fetch now tries a source's own url then each of its mirrors in
# document order, and names every url it failed on rather than only
# the last -- a fallback list whose failures collapse into one
# message is how a misspelled mirror reads exactly like one that is
# down. Ceiling is 12 fallback urls across all sources; a thirteenth
# is refused at publish rather than dropped.
#
# EXPECT freetype@2.13.3-7 TO INSTALL ON THIS BUILD and to have
# failed on the one before it, with savannah unchanged. That is the
# test: there is no unit test for a fetch loop.
#
#
# v2.57.235: the dashboard's login prompt stops reappearing on a
# timer (#490).
#
# Gating the identity reads in v2.57.231 made a comment in the web
# UI false without anyone re-reading it: promptReauth() said "several
# concurrent GETs are never gated". refreshLdapUsers is in
# ALL_REFRESHERS, which the sweep runs for every visitor on every
# page, so with nobody signed in it 401s every cycle and each 401
# opened the login modal -- worse straight after a successful login,
# because a second gated read from the same cycle reopens it.
#
# A background refresh no longer opens the modal, and neither gated
# refresher calls its endpoint without a token. The second half
# matters on the host too: every pointless 401 wrote an audit
# warning, so an open tab was filling the audit log overnight.
#
# v2.57.234: the explain sweep keys on the engine's bytes, not on the
# version string it prints (#496, ADR-0307 clause 7).
#
# cix-build-system closed #183 with commit f22e6aa -- a real change to
# manifest.c -- and pushed it to main with VERSION still reading
# 0.1.29, the same as the tag. That is by design: upstream ships
# fixes between tags and its own suite asserts `cbs --version` equals
# VERSION, so a newer engine reports an older number. Keyed on that
# string, the sweep decides nothing changed and leaves every derived
# explain.json stale, silently -- the exact failure clause 7 exists
# to end.
#
# Keyed now on "<version> <sha256 of /usr/bin/cbs>".
#
# EXPECT THE SWEEP TO FIRE ON THIS BOOT. The recorded key is the old
# version-only form, so it cannot match the new one; every PBS
# recipe's identity is re-derived once and the log line reads
# "cbs 0.1.29 -> cbs 0.1.29 <digest>", which is the engine moving
# without renaming itself, stated out loud.
#
# v2.57.233: a metadata value that is too long says so (#493).
#
# A PBS recipe whose changelog exceeds its 511-byte field was
# answered "recipe content failed to parse, or its pkg_name= doesn't
# match name". Both are false -- cbs explain had already succeeded on
# it and the code reporting the error was reading that output, and
# the name matched -- so it sent the author hunting for a CPDL syntax
# error that does not exist. The real reason was in the log store and
# only there.
#
# It now reaches the caller with its limits, through the same
# pkg_recipe_add_last_error() channel v2.57.232 added for the
# format "tar.gz" refusal. Nearly needed for real the same day:
# binutils 2.42-13's changelog came out at 537 bytes and was caught
# by counting before publishing, not by being told.
#
...
#
pkg_name="cix"
pkg_version="v2.57.244"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.244.tar.gz"
pkg_sha256="284890375f22f51ab843e1f8d8467530c0d8964964d8c25db12cafa659a925e0"
pkg_artifact_sha256="4d1525f5128f58f9c3c1c1aca90d9066ca2ec7ef5905b405d7df5baff397fa58"
