#
# v2.57.28: backlog squash -- three fixes.
#
# #360: a path parameter was never percent-decoded (only query strings
# were), so /v1/pkg/cix%40__hostbuild 404s while
# /v1/pkg/cix@__hostbuild returns the same package. Clients encode @ by
# default. %2F and %00 are refused rather than decoded: routing has
# already happened, and a decoded separator would change the path's
# meaning after the decision about it was taken.
#
# #343: every container gets a /tmp tmpfs, mode 1777, fresh per start.
# Three investigations have ended at a missing /tmp.
#
# #333: PUT files folded four syscalls into one 500 message; each step
# now names itself with its errno, which is what the next deploy reads
# to find out why that endpoint fails on every running container.
#
pkg_name="cix"
pkg_version="v2.57.28"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.28.tar.gz"
pkg_sha256="b2fe4e05ec3499022513ea4958512fd2758197e440111ca34d3cfd305535d4fe"
