#
# v2.57.32: the CPU count in /sys, and a DELETE for container files.
#
# sysconf(_SC_NPROCESSORS_ONLN) reads /sys/devices/system/cpu/online,
# not /proc, so it answered with the host's count. Measured in a
# container pinned to one CPU on a two-CPU host: CONF=2 ONLN=2 NPROC=1
# -- nproc right, every ONLN-based thread-pool sizing wrong by two.
# online/present/possible now report the container's cpuset verbatim.
#
# Not fixed, deliberately: htop still shows the second CPU as offline.
# _SC_NPROCESSORS_CONF counts cpuN DIRECTORIES and no file bind removes
# a directory (#363). A private sysfs mount is not the answer -- every
# container already gets one, and CPU topology is scoped by no Linux
# namespace.
#
# DELETE /v1/containers/{name}/files: the endpoint had GET and PUT but
# no DELETE, so anything staged once was staged forever -- which is why
# containers built before cix-init moved to /sbin still carry an inert
# copy at the old path.
#
pkg_name="cix"
pkg_version="v2.57.32"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.32.tar.gz"
pkg_sha256="59f21333506f626e54da4c039e84513819f6d4281e341418c5e34fe23565052b"
