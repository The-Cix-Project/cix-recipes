#
# v2.57.33: v2.57.32's contents, plus the operation-count gate it broke.
#
# v2.57.32 never deployed: DELETE /v1/containers/{name}/files is the
# 292nd operation and test_apigen asserts the count deliberately, so a
# new route has to move a number in a diff. That is the gate working.
#
# The same build also failed test_console_exec once and passed it on a
# re-run of identical source -- #331, not a regression. It reported
# TIMED OUT rather than PEER CLOSED, which is the discriminator that
# issue's machinery exists to provide, with the event loop measured
# healthy (worst_pass_ms=20) at the time.
#
# Contents, unchanged from v2.57.32: /sys/devices/system/cpu/{online,
# present,possible} follow the container's cpuset, because
# sysconf(_SC_NPROCESSORS_ONLN) reads /sys and answered with the host
# (measured CONF=2 ONLN=2 NPROC=1 in a one-CPU container). htop's
# offline-CPU meter is NOT fixed by this -- _SC_NPROCESSORS_CONF counts
# cpuN directories and no file bind removes a directory (#363). Plus
# DELETE for container files, so a staged path that changes can be
# cleaned up without rebuilding the container.
#
pkg_name="cix"
pkg_version="v2.57.33"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.33.tar.gz"
pkg_sha256="549103115704352ca189a3a15989cbbfa5ee9cb4d3d33d02e811ace9c94158d0"
