#
# v2.57.34: a volume on btrfs can have a size limit (#364).
#
# PUT /v1/volumes/{name}/quota refused every volume on this platform's
# default filesystem, so no volume on the host could be bounded at all
# -- the unbounded-way-to-fill-a-disk volume.h gives as the reason #93
# mattered.
#
# volume_create() now uses cix_btrfs_subvol_create_or_dir(), so a volume
# is a subvolume on btrfs and a plain directory elsewhere. Volumes that
# predate it are converted in place by volume_convert_to_subvolume(),
# the same copy-and-swap volume_migrate() already does between disks --
# persisting nothing, therefore idempotent, and leaving the volume
# complete at its own path on any failure.
#
# Setting a limit performs the conversion, refused while a container
# mounting the volume is RUNNING (a bind mount pins the inode,
# ADR-0183). Clearing a limit is never blocked.
#
# Does NOT change what df reports -- btrfs does not reflect qgroups in
# statfs(2), measured against a rootfs that already has one. Volume
# usage reporting is #365.
#
pkg_name="cix"
pkg_version="v2.57.34"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.34.tar.gz"
pkg_sha256="90004d7b9e9276653739ed6bd0058570de45a8376f753493b4c9ad8ac804c58f"
pkg_artifact_sha256="ee5776e67de7f9c0dcc392118a82d4502f393c646c7feadb1abb6027803050cb"
