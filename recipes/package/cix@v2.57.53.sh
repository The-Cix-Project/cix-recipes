#
# v2.57.53: an action is attributable to a person (#371, ADR-0271)
#
...
# host boots a root built from all of it -- each can be stuck, each
# fails its own way, none had a vocabulary. Four kinds now walk SUBSETS
# of one stage enum, never private lists.
#
# Four stages added: acquire (where a pipeline waits for what it
# depends on, and where forking will attach), assemble and stage
# (host-only, failing independently), verify (deployment-only, the
# ready probe). deploy moves into the host pipeline.
#
# test_pipeline asserts the stage count and FAILED when this landed --
# ADR-0256's "deliberate act with an ADR, not a quiet enum append".
#
# Edges are STRUCTURAL: an edge exists because a deployment names an
# image and a manifest names packages, drawn when everything is
# healthy. Drawing them only where something is blocked gives a working
# host a picture of disconnected boxes.
#
# And "container recipe" is now "deployment" -- it describes what to
# run and where, not how to build anything. Clean cut-over.
#
# ADR-0269. Nothing forks yet; acquire reports blocked.
#
pkg_name="cix"
pkg_version="v2.57.53"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.53.tar.gz"
pkg_sha256="fb172834814ca4635e783cafd048f90c84086dc85579c1d028df833e7ae19a55"
pkg_artifact_sha256="3509e2d27b6441db16faf0353711c79accd970b21527dc05762bc93676461db0"
