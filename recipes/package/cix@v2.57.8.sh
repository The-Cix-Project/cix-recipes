#
# v2.57.8: no kernel module could ever load, on any host this platform
# has assembled (#347, ADR-0266).
#
# GET /v1/system/kmod/e1000e answered "no such module (not
# built/available)", and so did tg3, usb-storage and ehci-hcd -- every
# module load_boot_modules() tries at boot. Not "failed to load". Not
# present. The root carried no module tree and no modprobe.
#
# Every part of the mechanism was already built: kernel.recipe runs
# modules_install then depmod -b, mkbootroot stages both a modules_dir
# and a kmod_bin_dir correctly with a test proving it, kmod is
# packaged, and daemon/src/kmod.c knows how to call it. The only
# missing thing was the call -- spawn_cix_bootroot_assembly() passed a
# literal "" for both.
#
# That is ADR-0263's bug on the next argument along, and it survived
# because 192.168.15.95 is a VM on virtio_net, built in, so nothing
# ever asked for a module. On bare metal it is fatal.
#
# The tree now comes from ARTIFACTS_DIR/kernel/lib/modules; the tools
# from a new one-package cix-kmod image. test_bootroot_args, in
# SELFTESTS, fails the build if any of mkbootroot's four optional
# arguments is a string literal at that call site.
#
pkg_name="cix"
pkg_version="v2.57.8"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.8.tar.gz"
pkg_sha256="60df629eec7be0979e30f0279337164f0e55b03a9cffbb33f0046ff6646e2438"
pkg_artifact_sha256="378be33c5b7d42d28972d5aa88237699dca3bfc702a3d3922e5ee0e77368b39b"
