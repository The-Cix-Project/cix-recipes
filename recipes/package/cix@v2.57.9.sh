#
# v2.57.9: a control-plane assembly can be asked for, not only watched
# (#308).
#
# GET /v1/system/assembly has reported on assembly since ADR-0230 with
# no way to request one -- it happened only as a side effect of a
# hostbuild named "cix" completing, and POST /v1/pkg/hostbuild answers
# an already-installed version with 409. So re-assembling a root meant
# inventing a cix version whose code had not changed.
#
# Which is exactly what v2.57.8 needed: #347's module staging runs in
# the daemon that PERFORMS the assembly, so the round that builds the
# fix assembles a root using the old logic, and the fix only takes
# effect on the assembly after that. This round is that second
# assembly, and it carries a real endpoint rather than a bumped
# version number.
#
pkg_name="cix"
pkg_version="v2.57.9"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.9.tar.gz"
pkg_sha256="ef2096e941a3286dba18c8b6625a6572385d13267908ad2c1b201f3b3febcef4"
