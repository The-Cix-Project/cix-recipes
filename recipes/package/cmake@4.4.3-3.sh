#
# cmake 4.4.3-3 -- first packaging that is usable by another recipe.
#
...
#
# RUNTIME dependency, and revision 2 got this wrong in the mirror image
# of revision 1's mistake. Revision 1 did not declare openssl at all and
# could not build. Revision 2 declared it as a BUILD dependency only --
# so cmake built, linked libssl.so.3, installed cleanly, and then died
# the first time another recipe tried to use it:
#
#   cmake: error while loading shared libraries: libssl.so.3:
#   cannot open shared object file: No such file or directory
#
# A build environment is composed from exactly the tools a recipe
# declares (ADR-0199/0209), so fastfetch's environment had cmake and no
# openssl. The library a produced binary LINKS is a property of the
# package, not of whoever builds with it -- declaring it here means
# every consumer gets it by installing cmake, instead of each consumer
# having to know what cmake happens to link.
pkg_depends="openssl"
#
