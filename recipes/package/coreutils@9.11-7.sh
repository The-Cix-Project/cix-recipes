...
pkg_install() {
	...
	# Found rather than hardcoded (#184): these are libraries in the
	# build sandbox, and this platform is collapsing its library
	# directories, so openssl and glibc move under this recipe's feet.
	# A fixed directory here would break on the day they do.
	mkdir -p "$PKG_DESTDIR/usr/lib"
	for lib in libcap.so.2 libcap.so.2.66 libcrypto.so.3; do
		found=""
		for dir in /usr/lib /lib/x86_64-linux-gnu /usr/lib/x86_64-linux-gnu; do
			if [ -e "$dir/$lib" ]; then found="$dir/$lib"; break; fi
		done
		[ -n "$found" ] || {
			echo "coreutils: $lib is not in the build sandbox" >&2
			exit 1
		}
