...
pkg_changelog="10.5.2-6: declares its build tools so it can be rebuilt through the ordinary install path (#206). dhcpcd's pkg_build() patches a source file with an inline python3 script before configuring, so python is a genuine build tool for this recipe even though nothing in the built output uses it."
...
#
pkg_build_depends="tcc make linux-headers bash coreutils sed grep gawk binutils findutils diffutils python"
