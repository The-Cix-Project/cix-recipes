...
#
pkg_name="dhcpcd"
pkg_version="10.5.2-7"
pkg_changelog="10.5.2-7: openssl: dhcpcd links libcrypto and failed with "library 'crypto' not found". It was already in pkg_depends as a runtime dependency; a composed build environment holds only what pkg_build_depends names (ADR-0199), so it has to be declared for the build too. 10.5.2-6: declares its build tools so it can be rebuilt through the ordinary install path (#206). dhcpcd's pkg_build() patches a source file with an inline python3 script before configuring, so python is a genuine build tool for this recipe even though nothing in the built output uses it."
