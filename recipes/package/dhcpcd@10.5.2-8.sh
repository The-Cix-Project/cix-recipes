...
#
pkg_name="dhcpcd"
pkg_version="10.5.2-8"
pkg_changelog="10.5.2-8: openssl: dhcpcd links libcrypto and the build failed with 'library crypto not found'. It was already in pkg_depends as a runtime dependency; a composed build environment holds only what pkg_build_depends names (ADR-0199), so it has to be declared for the build too. 10.5.2-7 got this right and still failed to run: its changelog text contained an unescaped double quote, which closed the assignment early and left the shell trying to execute the rest as a command (/build/recipe.sh: line 40: crypto: command not found). Recipe metadata is shell, so a quote in prose is a syntax element -- the same trap as backticks in a changelog, already documented."
...
#
pkg_build_depends="tcc make linux-headers bash coreutils sed grep gawk binutils findutils diffutils python openssl"
