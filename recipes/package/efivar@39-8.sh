...
pkg_build() {
	make CC=/usr/bin/gcc ENABLE_DOCS=0 LIBDIR=/usr/lib \
	     ERRORS_GCC=-Wno-error=discarded-qualifiers -j"$(nproc)"
}

# LIBDIR defaults to $(PREFIX)/lib64; this project stages runtime
# libraries in one directory, /usr/lib (#184), so it is set explicitly
# rather than moved afterwards -- the lesson openssl 3.0.20-2
...
pkg_install() {
	...
	test -e "$PKG_DESTDIR/usr/lib/libefivar.so.1"
	...
	# No relocation: with LIBDIR at /usr/lib the install writes the .pc
	# straight into /usr/lib/pkgconfig, which is where pkgconf looks
	# (#184). The move existed only because those were two directories.
	...
	# sat in the library directory -- a file that verifies, resolves,
	...
	*"libdir=/usr/lib"*) ;;
	*)
		echo "efivar.pc does not name /usr/lib" >&2
		exit 1
		;;
	esac
}
