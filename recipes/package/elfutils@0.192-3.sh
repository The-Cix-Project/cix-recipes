...
#
# With CC=gcc, configure then got past __thread and failed one step
# later, on its own terms: "checking for __cxa_demangle in
# -lstdc++... no" -> "configure: error: __cxa_demangle not found in
# libstdc++, use --disable-demangler to disable demangler support."
# This project has no libstdc++ package staged in the dev image at
# all (gcc.recipe ships a C compiler only), and the demangler is a
# real, optional eu-readelf/eu-nm feature (pretty-printing mangled
# C++ symbol names) -- entirely orthogonal to this recipe's actual
# purpose (libelf, for the kernel's own tools/objtool). configure's
# own suggested flag disables exactly that one optional feature.
pkg_build() {
