...
pkg_depends=""
# Verified from the ELF rather than reasoned about: the built binary's
# only DT_NEEDED entries are libm.so.6 and libc.so.6, both in the glibc
# baseline, and the package ships no libraries of its own at all. That
# is what --without-readline bought -- readline, libtinfo, and the
# foreign copy of it are simply gone.
#
# pkg_artifact_sha256 approves bytes 192.168.15.95 actually produced
# and published, from this recipe's own build steps (-6 and -7 differ
# only by this line). It is NOT carried forward from an earlier
# version, which the Build Provenance Mandate forbids.
#
# It exists because a fresh composed build environment cannot build
# gawk at all: composing one puts the OLD gawk in it, and that copy
# ships Debian's libreadline against a libtinfo the baseline no longer
# provides, so awk dies in the dynamic linker before configure gets
# anywhere. An artifact install needs no build environment (issue
# #144), which is what lets every downstream ./configure run awk again.
...
pkg_build_depends="bash coreutils make tcc linux-headers sed grep gawk ncurses binutils"
pkg_changelog="5.3.0-9: rebuilt against tcc 0.9.28rc (ADR-0223). The 2017 0.9.27 release could give two simultaneously-live locals the same stack slot (#216), a fault that corrupts values silently wherever the aliased pair is only read and written, so every binary it produced is suspect rather than merely the ones that failed. No source change: the revision exists to make the rebuild real, because an image version is a hash of the package manifest (ADR-0155) and a same-version reinstall is deduped and discarded. 5.3.0-8: libc-dev retired; linux-headers declared for the kernel uapi headers glibc's own limits.h needs (#187)"
