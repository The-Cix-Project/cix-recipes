...
MINIEXTRACT
	...
	fixup_exec_tools
	fixup_install_headers_dir
	( while true; do fixup_exec_tools; fixup_install_headers_dir; sleep 2; done ) &
	fixup_pid=$!
	# -j2 (2026-08-18, user-directed): the original -j1 downgrade had two
	# separate real reasons (see the comment above) -- an unbounded
	# -j$(nproc) hanging the whole host (now independently mitigated by
	# pkg-build-config's own real memory_max cgroup ceiling), and a
	# *suspected*, never fully proven race between two parallel jobs both
	# regenerating gcc's own as/collect-ld/nm at once. That second
	# concern isn't conclusively resolved by anything added since -- this
	# is a real, acknowledged step back into not-fully-characterized
	# territory, tried because the box has real spare cores and the user
	# directed it after being told the honest risk, not because the risk
	# was resolved first.
	make -j2 bootstrap
	make_rc=$?
