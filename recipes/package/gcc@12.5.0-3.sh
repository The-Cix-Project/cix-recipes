...
MINIEXTRACT
	...
	fixup_pid=$!
	# -j4 (2026-08-18, user-directed, after -j2 already surfaced real
	# evidence of a genuine parallel-build timing issue in this exact
	# bootstrap -- see the -3 comment above). The original -j1 downgrade
	# had two separate real reasons: an unbounded -j$(nproc) hanging the
	# whole host (now independently mitigated by pkg-build-config's own
	# real memory_max cgroup ceiling), and a *suspected*, never fully
	# proven race between two parallel jobs both regenerating gcc's own
	# as/collect-ld/nm at once. Neither concern is conclusively resolved
	# by anything added since -- this is a real, acknowledged, and now
	# twice-escalated step into not-fully-characterized territory, tried
	# because the user directed it after being told the honest risk
	# both times, not because the risk was resolved first.
	make -j4 bootstrap
	make_rc=$?
