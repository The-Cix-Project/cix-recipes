...
MINIEXTRACT
	...
	# target libgcc. The real mechanism (see the longer root-cause
	# comment just below, found by actually reading gcc's own
	# configure.ac/exec-tool.in rather than assumed): `as` isn't a
	# symlink/copy regenerated by an on-demand Makefile rule at all --
	# it's a `#!/bin/sh` script instantiated ONCE, at gcc's own
	# sub-configure time, from `exec-tool.in`. A corrupted first copy is
	# never revisited by `make` because nothing about `make` owns it;
	# what actually fixes it is re-running gcc's own `config.status` to
	# regenerate it fresh from the same cached substitution values.
	#
	# The identical shape recurred one step further into that same
	# retry, past gcc/as: `collect2: fatal error: posix_spawnp: Exec
	# format error` linking libgcc_s.so.1. First guess was that collect2
	# was failing to spawn a file literally named `gcc/ld` (matching this
	# project's own Environment notes bullet on collect2's private-search
	# convention) -- deleting `gcc/ld`/`gcc/ld1` and retrying had zero
	# effect across three separate attempts, unlike gcc/as's own
	# consistent one-retry fix. That asymmetry was the real signal
	# something was wrong with the theory itself, not just the fix.
	#
	# Root cause found by reading gcc's own real source
	# (gcc/configure.ac + gcc/exec-tool.in, not guessed): `as`, `nm`, and
	# the linker wrapper are ALL generated the identical way -- a
	# `#!/bin/sh` script instantiated from `exec-tool.in` at configure
	# time (`AC_CONFIG_FILES(as:exec-tool.in, ...)`), which dispatches on
	# its OWN invoked name (`basename "$0"`) to decide what real tool to
	# exec. Its `case "$invoked" in` only ever has three real names:
	# `as`, `collect-ld`, and `nm` -- there is no `ld` case at all.
	# collect2 -- gcc's own real linker-invocation driver -- has always
	# invoked a helper literally named `collect-ld` (a well-known,
	# real GCC internal detail, not this project's own convention);
	# `gcc/ld` was never the right file to be deleting. This also
	# explains why the earlier `gcc/nm` deletion broke nm entirely
	# instead of triggering a clean regenerate: `nm` -- like `as` and
	# `collect-ld` -- is a generated exec-tool.in script too, so deleting
	# it *should* have been safe by the same logic, but nothing in this
	# specific build ever re-triggers `AC_CONFIG_FILES` after configure
	# has already run once -- these files are written ONCE, at configure
	# time, not regenerated on demand by any `make` rule at all. gcc/as's
	# own apparent "fixed by one retry" behavior most likely wasn't a
	# real regeneration at all (there's no rule to trigger one) -- more
	# plausibly the STAGE-BUBBLE copy mechanism
	# (`--enable-bootstrap`'s stage1->stage2->stage3 promotion, which
	# copies the entire gcc/ build directory forward) overwrote a
	# corrupted first copy with a working one from whichever stage
	# didn't hit the corruption -- not independently confirmed, but it
	# doesn't need to be: regardless of why the old delete-and-hope
	# approach seemed to work once, the actually-correct,
	# general fix isn't "delete and hope a Makefile rule notices" at
	# all -- it's forcing gcc's own configure-time generation to run
	# again, which really can regenerate all three from a clean state:
	# gcc's own sub-directory `config.status` (written once by gcc's own
	# sub-configure, a real, standard autoconf file, not this project's
	# own invention) can regenerate any of its own `AC_CONFIG_FILES`
	...
	if [ "$i" -eq 3 ]; then
		# gcc/ld doesn't exist (collect2 invokes collect-ld, never a
		# file literally named ld -- see the real root-cause comment
