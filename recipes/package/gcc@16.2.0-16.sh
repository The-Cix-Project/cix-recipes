...
# that compiler.
#
# Fetched from mirrors.kernel.org, not ftp.gnu.org.
#
# ftp.gnu.org accepts this site's TCP connections on 443 and 80 and then
# answers nothing -- measured, 12 s to an empty reply, while
# mirrors.kernel.org and www.kernel.org answer in under a second from
# the same host. Not DNS, not IPv6, not the internet: that one server is
# not talking to this egress, and it takes 28 other recipes with it.
#
# The swap is a no-op to what gets built, and this recipe already said
# so before the change: the tarball was verified byte-identical across
# both mirrors, and pkg_sha256 (unchanged here, e6738e29...) is what
# makes that a fact rather than a hope. glibc and m4 have fetched from
# this same mirror for some time; this follows them.
#
# Source is GNU's own canonical release, checksum verified
...
#
pkg_name="gcc"
pkg_version="16.2.0-16"
pkg_source="https://mirrors.kernel.org/gnu/gcc/gcc-16.2.0/gcc-16.2.0.tar.xz https://gcc.gnu.org/pub/gcc/infrastructure/gmp-6.3.0.tar.bz2 https://gcc.gnu.org/pub/gcc/infrastructure/mpfr-4.2.2.tar.bz2 https://gcc.gnu.org/pub/gcc/infrastructure/mpc-1.3.1.tar.gz"
...
#   gawk       config.status substitutes with awk. Missing in -12, and
#              the build died on `./config.status: line 1221: awk:
#              command not found` followed by `make: *** No rule to
#              make target 'bootstrap'` -- a message that names the
#              bootstrap rather than the missing tool, so the cause was
#              two lines above the failure.
#   diffutils  cmp, which contrib/compare-debug uses to compare stage 2
#              against stage 3. Without it the comparison step -- the
#              whole point of --enable-bootstrap, and the only thing
#              that proves the compiler reproduces itself -- cannot
#              run. coreutils does not provide cmp; diffutils does.
#
...
pkg_toolchain="gcc"
pkg_toolchain_reason="structural: this package IS the toolchain: its --enable-bootstrap stage 1 is seeded by the previous Cix-built gcc"
pkg_changelog="16.2.0-16: stage the runtime libraries into /usr/lib rather than the multiarch directory (#184 stage 3). These three files -- libgcc_s.so.1 and the two libstdc++ names -- are the last thing besides glibc still installing into /lib/x86_64-linux-gnu, and stage 4 cannot flip glibc slibdir until that directory holds nothing anyone needs. They are copied out of usr/lib64, which is where gcc own libdir puts them, precisely so a consumer can find them on the loader search path; /usr/lib has been on that path since glibc 2.44-7, so the destination changes and nothing else does. usr/lib64 itself is deliberately left alone: it is gcc own convention, reached through gcc own compiled-in prefix rather than by any search this platform controls, and it is not what blocks stage 4. The artifact approval is dropped because those bytes came from the previous revision. 16.2.0-15: fetch from mirrors.kernel.org. ftp.gnu.org accepts this site's connections and then answers nothing (12 s to an empty reply on both 443 and 80, while kernel.org answers in under a second), which blocked every gcc rebuild and 28 other recipes with it. Byte-identical: same pkg_sha256, and this recipe already documented having verified the tarball against both mirrors. glibc and m4 already fetch from here. 16.2.0-14: declare pkg_toolchain=gcc and its reason (#222, ADR-0226)"
pkg_build_depends="gcc tcc binutils make bash coreutils sed grep gawk diffutils m4 findutils patch tar bzip2 gzip zlib linux-headers"
# 16.2.0-13 (#199): gawk and diffutils added. -12 was the first gcc
# that could be built here at all, and it got as far as configure
# before dying on a tool it had not declared. That is ADR-0199 working
# exactly as designed -- a build environment contains what the recipe
# declared and nothing else, so an undeclared tool is a real failure
# rather than a silent success against whatever the sandbox happened to
# have. It is also why the list above is derived from what pkg_build()
# invokes rather than assumed.
#
# 16.2.0-12 (#187): libc-dev -> linux-headers, and this is the last
# thread of the contamination rather than a tidy-up.
#
# pkg_depends is a RUNTIME dependency, copied into every composed build
# environment that declares gcc. libc-dev stages the build host's whole
# /usr/include, which on Debian includes
# /usr/include/x86_64-linux-gnu/sys/cdefs.h -- a glibc 2.36 header. GCC
# searches the multiarch include directory BEFORE /usr/include, so:
#
#   features.h (2.44, /usr/include) includes <sys/cdefs.h>
#   -> resolves to the 2.36 copy in the multiarch dir
#   -> its include guard fires, so 2.44's cdefs.h is never read
#   -> __COLD is never defined
#   -> stdio.h (2.44) uses it, and the parser desyncs:
#
#     /usr/include/stdio.h:872: expected declaration specifiers
#         before '__COLD'
#
# which surfaced as binutils' configure reporting "cannot run C compiled
# programs" -- a message about the symptom three steps downstream. gcc
# itself compiles and runs fine in that same environment; only a build
# whose configure includes stdio.h early enough trips over it.
#
# So every environment with gcc in it has been compiling against a 2.36
# header shadowing our own 2.44 one. Dropping this is what actually
# retires libc-dev, since gcc was pulling it in for everybody.
...
#pragma GCC optimize ("O0")' libgcc/unwind-dw2-btree.h
	...
	if [ "$make_rc" -ne 0 ]; then
		...
		# -9: postmortem battery. Both -6 and -8 died at stage 2 with
		# corrupt binaries and the wreckage was locked inside an
		# exited container nothing could exec into -- every follow-up
		# question needed another multi-hour run. So the recipe now
		# asks those questions itself, in place, while the tree is
		# hot, and prints the answers into the build log. Pure
		# diagnostics: runs only on failure, changes nothing, still
		# exits 1 with the original status.
		echo "=== POSTMORTEM: toolchain identity ==="
		as --version 2>&1 | head -1
		ld --version 2>&1 | head -1
		/usr/bin/gcc --version 2>&1 | head -1
		echo "=== POSTMORTEM: stage-1 compiler self-check (its own binary) ==="
		if [ -x ./prev-gcc/xgcc ]; then S1=./prev-gcc; else S1=./gcc; fi
		echo "stage dir: $S1"
		PMDIR=/build/pm; mkdir -p "$PMDIR"
		echo 'int main(void){return 42;}' > "$PMDIR/pm.c"
		for opt in -O0 -O2; do
			if timeout 30 "$S1/xgcc" "-B$S1/" $opt "$PMDIR/pm.c" -o "$PMDIR/pm" 2>&1; then
				timeout 30 "$PMDIR/pm"; rc=$?
				echo "POSTMORTEM: stage1 xgcc $opt compile+run rc=$rc (want 42)"
			else
				echo "POSTMORTEM: stage1 xgcc $opt COMPILE FAILED"
			fi
		done
		echo "=== POSTMORTEM: stage-1 OUTPUT check -- C++ hello against stage-1 target libstdc++ ==="
		printf '#include <cstdio>\nstruct S{S(){puts("ctor");}};static S s;int main(){puts("main");return 7;}\n' > "$PMDIR/pm.cc"
		TL=$(ls -d prev-x86_64-*/libstdc++-v3 x86_64-*/libstdc++-v3 2>/dev/null | head -1)
		if [ -n "$TL" ] && [ -x "$S1/xg++" ]; then
			if timeout 60 "$S1/xg++" "-B$S1/" -O2 -static-libstdc++ -static-libgcc \
				-nostdinc++ -I"$TL/include" -I"$TL/include/x86_64-pc-linux-gnu" \
				-I../libstdc++-v3/libsupc++ \
				-L"$TL/src/.libs" -L"$TL/libsupc++/.libs" \
				"$PMDIR/pm.cc" -o "$PMDIR/pmxx" 2>&1; then
				timeout 30 "$PMDIR/pmxx"; rc=$?
				echo "POSTMORTEM: stage1-output C++ static run rc=$rc (want 7; watch for abort/segv=>131+)"
			else
				echo "POSTMORTEM: stage1-output C++ static COMPILE/LINK FAILED"
			fi
		else
			echo "POSTMORTEM: no stage-1 target libstdc++ tree found (TL='$TL')"
		fi
		echo "=== POSTMORTEM: broken stage-2 cc1, if present ==="
		if [ -x ./gcc/cc1 ]; then
			echo 'int x;' > "$PMDIR/min.c"
			timeout 30 ./gcc/cc1 -quiet "$PMDIR/min.c" -o /dev/null 2>&1; rc=$?
			echo "POSTMORTEM: stage2 cc1 on 'int x;' rc=$rc (139=segv, 134=abort)"
			readelf -l ./gcc/cc1 2>/dev/null | grep -E "GNU_EH_FRAME|LOAD" | head -6
			readelf -S ./gcc/cc1 2>/dev/null | grep -E "eh_frame|Name" | head -4
		else
			echo "POSTMORTEM: ./gcc/cc1 not present"
		fi
		echo "=== POSTMORTEM: end ==="
		exit 1
	fi
...
pkg_install() {
	...
	# The real runtime .so's version suffix belongs to the release, not
	# the recipe: -9's bootstrap went fully green (stages 2/3 byte-
	# identical, all self-tests passed) and then died right here on a
	# hardcoded libstdc++.so.6.0.35 carried forward from an older
	# lineage -- 16.2 ships 6.0.36. Glob the versioned file and assert
	# exactly one match, so a future GCC bump can never repeat this
	# while a surprising layout still fails loudly instead of staging
	# garbage.
	mkdir -p "$PKG_DESTDIR/usr/lib"
	# The gdb pretty-printer libstdc++.so.6.0.NN-gdb.py sits right next
	# to the real shared object and matched the -10 glob too (its own
	# assert caught it). Drop it first -- nothing here ships gdb
	# integration -- then match only names ending in a digit, so the
	# real .so is the sole match and a surprising layout still trips
	# the assert loudly.
	rm -f "$PKG_DESTDIR"/usr/lib64/libstdc++.so.6.0.*-gdb.py
	set -- "$PKG_DESTDIR"/usr/lib64/libstdc++.so.6.0.*[0-9]
	[ "$#" -eq 1 ] && [ -e "$1" ] || {
		echo "gcc: expected exactly one usr/lib64/libstdc++.so.6.0.*[0-9], found: $*" >&2
		exit 1
	}
	cp -a "$PKG_DESTDIR/usr/lib64/libgcc_s.so.1" \
	   "$PKG_DESTDIR/usr/lib64/libstdc++.so.6" "$1" \
	   "$PKG_DESTDIR/usr/lib/"
	# The point of the copy is that a consumer finds these on the
	# loader search path, so assert they are actually there (#184).
	# Getting this wrong is invisible at build time and shows up as a
	# missing libgcc_s at the first thread exit in something unrelated.
	for f in libgcc_s.so.1 libstdc++.so.6; do
		[ -e "$PKG_DESTDIR/usr/lib/$f" ] || {
			echo "gcc: $f did not land in usr/lib" >&2
			exit 1
		}
	done
	[ -z "$(find "$PKG_DESTDIR/lib/x86_64-linux-gnu" -type f 2>/dev/null)" ] || {
		echo "gcc: something still installs into the multiarch directory (#184)" >&2
		find "$PKG_DESTDIR/lib/x86_64-linux-gnu" >&2
		exit 1
	}
