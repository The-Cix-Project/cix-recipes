...
MINIEXTRACT
	...
	#
	# Real, minimal, scoped fix: pin btree_destroy specifically to -O0
	# via a real GCC function attribute, so this one tiny cleanup-path
	# function (irrelevant to real-world runtime performance) skips
	# whatever -O2 pass introduces the corrupted encoding, while every
	# other function in this file (and the rest of GCC) keeps its normal
	# optimization level. A real, standard, well-precedented technique
	# for routing around a confirmed compiler bug pending an upstream
	# fix -- not a change to this recipe's own logic, a patch to the
	# fetched third-party source itself, applied fresh on every build
	# (never a change to any file this project's own tree tracks
	# directly).
	sed -i '/^btree_destroy (struct btree \*t)$/i __attribute__((optimize("O0")))' \
		libgcc/unwind-dw2-btree.h
	grep -q '__attribute__((optimize("O0")))' libgcc/unwind-dw2-btree.h || {
		echo "FATAL: btree_destroy patch did not apply -- source layout changed" >&2
		exit 1
	}

	mkdir -p build
	cd build
	# CC/CXX=/usr/bin/gcc|g++ here means "whatever this pkgbuild
	# sandbox's own /usr/bin/gcc currently resolves to" -- confirmed,
	# user-directed as of this version, that this is this project's own
	# previously self-hosted gcc/12.5.0 build (not Debian's ambient
	# 12.2.0 the 16.1.0 recipe comment above described at the time it
	# was written -- this sandbox's own ambient toolchain has since been
	# contaminated by gcc.recipe's own earlier output, the same class of
	# effect CLAUDE.md's own "bare cc can no longer be trusted to mean
	# TCC" note already documents for other tools). Continuing the
	# self-hosted lineage (12.5.0 seeds 16.2.0's stage 1) is the
	# intended, deliberate choice here, not an accident to route around
	# -- accepted with the known, real risk that 12.5.0 itself has a
	# confirmed, deterministic, unrelated ICE bug (segfaults compiling
	# an ordinary rol64() shift/or expression, found diagnosing kernel
	# 6.18.40-13's own build), which could in principle also affect
	# stage1's own output here in ways not yet characterized.
