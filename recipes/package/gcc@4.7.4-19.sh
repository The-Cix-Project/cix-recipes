	# EXTRA_GCC_FLAGS's DEFAULT is not empty -- it is the very channel
	# that forwards GCC_FOR_TARGET and LIMITS_H_TEST into each stage's
	# gcc sub-make (generated toplevel Makefile, lines 820-823; the
	# toplevel also sets MAKEOVERRIDES=, which is why a bare command
	# line variable dies at the subdir boundary while these survive).
	# So the override REPLACES the default with a value carrying the
	# default's own entries verbatim, plus the one addition:
	#
	#   - "GCC_FOR_TARGET=$(GCC_FOR_TARGET)" is passed as the same
	#     make-reference the default uses -- single-quoted through the
	#     shell so make, not bash, expands it inside the stage recipe,
	#     and double-quoted inside the value because its expansion
	#     contains spaces
	#   - LIMITS_H_TEST likewise, resolving to the true this command
	#     line sets
	#   - STMP_FIXPROTO is deliberately dropped: fixproto was removed
	#     from GCC in 4.5, the variable is empty here, and the
	#     default's sed idiom exists only to turn that empty into a
	#     harmless XFOO= -- omitting it is the same no-op without the
	#     contortion
	#   - INSTALL_HEADERS_DIR=install-headers-cp is the point: route
	#     the stage header copy through cp -p -r instead of this
	#     project's broken tar (#122)
	make bootstrap LIMITS_H_TEST=true \
		EXTRA_GCC_FLAGS='"GCC_FOR_TARGET=$(GCC_FOR_TARGET)" "LIMITS_H_TEST=$(LIMITS_H_TEST)" INSTALL_HEADERS_DIR=install-headers-cp' \
		-j"$(nproc)" || make_rc=$?#
...
MINIEXTRACT
	...
	#
	#
	# Compose limits.h the way GCC's own stmp-int-hdrs does, from the
	# source tree: limitx.h + glimits.h + limity.h.
	#
	# -13 seeded install-tools/include/limits.h instead, which on this
	# box is glimits.h alone -- it defines INT_MAX and friends itself
	# but never does #include_next, so glibc's limits.h (SSIZE_MAX and
	# the rest of the POSIX limits) becomes unreachable behind it. The
	# bootstrap failed hours in, at the first stage2 file needing
	# SSIZE_MAX. The wrapper halves (limitx/limity) are what chain to
	# the system header, and composing is only correct because
	# /usr/include/limits.h exists on these images (libc-dev ships it)
	# -- the same condition stmp-int-hdrs' own LIMITS_H_TEST checks.
	#
	# The -11 verification probe passed over exactly this breakage,
	# because it tested INT_MAX -- which glimits alone satisfies. The
	# probe below now demands SSIZE_MAX, which only the full chain
	# provides. A probe that can pass while the thing it guards is
	# broken is worse than none.
	#
	compose_limits_h() {
		cat /build/src/gcc/limitx.h /build/src/gcc/glimits.h /build/src/gcc/limity.h > "$1"
	}
	...
	*)  seed_fixed="/usr/lib/gcc/$(/usr/bin/gcc -dumpmachine)/$(/usr/bin/gcc -dumpversion)/include-fixed" ;;
	...
	mkdir -p "$seed_fixed"
	if [ ! -f "$seed_fixed/limits.h" ] || ! grep -q include_next "$seed_fixed/limits.h"; then
		echo "gcc 4.7.4-14: composing $seed_fixed/limits.h (limitx+glimits+limity)"
		compose_limits_h "$seed_fixed/limits.h"
	fi
	...
	printf '#include <limits.h>\nint probe[INT_MAX > 0 ? 1 : -1];\nlong chain[SSIZE_MAX > 0 ? 1 : -1];\n' > /build/limits_probe.c
	if ! /usr/bin/gcc -fsyntax-only /build/limits_probe.c; then
		echo "gcc: seed repair failed -- the C preprocessor still cannot resolve limits.h" >&2
		exit 1
	fi
	if [ -x /usr/bin/g++ ] && ! /usr/bin/g++ -x c++ -fsyntax-only /build/limits_probe.c; then
		echo "gcc: seed repair failed -- the C++ preprocessor still cannot resolve limits.h" >&2
		exit 1
	fi
	...
	#
	# There is deliberately NO stage-directory header seeding here, and
	# -15's failure is the reason -- it was self-inflicted (#116, #55).
	#
	# GCC's stmp-int-hdrs rule does, by design:
	#
	#     mv include-fixed/limits.h include-fixed/syslimits.h
	#
	# because on a healthy build, fixincludes has already left the FIXED
	# SYSTEM limits.h at that path, and the rename is how it becomes the
	# syslimits.h that the composed limits.h chains through. -13 through
	# -15 ran a loop that kept planting the composed limits.h into stage
	# include-fixed dirs -- so the Makefile moved OUR composed file into
	# syslimits.h, making the chain self-referential: limitx defines
	# _GCC_LIMITS_H_, quoted-includes syslimits.h, which -- being
	# another copy of limitx -- sees the guard already set and expands
	# to nothing. include_next never fires, glibc's limits.h is never
	# reached, SSIZE_MAX does not exist, and nothing errors on the way.
	# The -15 tree is the evidence: prev-gcc/include-fixed/syslimits.h
	# was a 172-line copy of the composed file, while stage2's own gcc/
	# dir -- which the loop had less time to touch -- held the correct
	# 8-line gsyslimits wrapper beside a correctly composed limits.h,
	# which is exactly what the build's own machinery produces once
	# LIMITS_H_TEST is overridden. The machinery was already fixed; the
	# loop was poisoning it.
	#
	# With no pre-existing limits.h in a stage dir, stmp-int-hdrs takes
	# its other branch: syslimits.h = gsyslimits.h (the 8-line
	# include_next wrapper) and limits.h = the composed xlimits.h that
	# LIMITS_H_TEST=true guarantees. Correct by the build's own rules,
	# with no external writer racing them.
	#
	fixup_exec_tools
	( while true; do fixup_exec_tools; sleep 2; done ) &
	fixup_pid=$!
	#
	# LIMITS_H_TEST=true on the make command line -- the actual root of
	# issue #55, finally located.
	#
	# gcc/Makefile builds its limits.h from a conditional:
	#
	#   xlimits.h: if $(LIMITS_H_TEST) then compose limitx+glimits+limity
	#              else glimits alone
	#
	# where LIMITS_H_TEST asks whether $(SYSTEM_HEADER_DIR)/limits.h
	# exists. On this box that test answers FALSE -- while
	# /usr/include/limits.h demonstrably exists (its own line 124 is
	# quoted in this recipe's history; libc-dev ships it; the
	# -fsyntax-only probe above depends on it and passes). A false
	# answer produces the glimits-only limits.h: INT_MAX works, no
	# #include_next, glibc's POSIX limits unreachable -- the exact
	# incomplete header every layer of #55 kept finding installed, and
	# the exact SSIZE_MAX failure that killed the -13 and -14
	# bootstraps at stage2, where the -14 run proved the build
	# machinery OVERWRITES a correct pre-seeded file with the broken
	# composition.
	#
	# Why the test misfires here (SYSTEM_HEADER_DIR indirection, shell
	# quirk, sysroot default) is not run to ground; what is verified is
	# the condition the test exists to check. Overriding a make
	# variable on the command line beats the Makefile's definition and
	# propagates through MAKEFLAGS into every recursive stage make, so
	# every stage composes correctly by the build's OWN rule -- no
	# external loop racing the Makefile, and identical headers in every
	# stage, which the stage2/stage3 comparison depends on.
	#
	# `|| make_rc=$?` because recipes run under set -e: a bare failing
	# make exits the recipe on the spot, which is why -12 through -15
	# died without ever printing their failure diagnostics.
	make_rc=0
	#
	# INSTALL_HEADERS_DIR=install-headers-cp: same verified-fact
	# override pattern as LIMITS_H_TEST above, for the next layer down.
	#
	# The bootstrap copies each stage's include-fixed to the next stage
	# via `(tar -cf - .) | (tar xpf -)` -- a pipeline whose left-side
	# exit status upstream deliberately ignores. The build sandbox's
	# tar (this project's own TCC-built 1.35) archives ONLY its first
	# argument and never recurses into directories, while exiting 0 --
	# proved with three probes recorded on the CRITICAL tar issue: an
	# archive of a two-file directory contains one entry, "./". So the
	# stage copy silently moved nothing, stage2 lost syslimits.h, and
	# its libgcc configure died at the CPP sanity check -- after tar
	# was confirmed PRESENT, because presence is not correctness.
	#
	# install-headers-cp is GCC's own alternative for exactly this
	# situation: `cp -p -r`, coreutils, verified working here. The
	# override propagates through MAKEFLAGS into the stage sub-makes
	# the same way LIMITS_H_TEST does.
	#
	# INSTALL_HEADERS_DIR rides in EXTRA_GCC_FLAGS, not bare on the
	# command line, and the -17 run is why. The toplevel Makefile sets
	# MAKEOVERRIDES= (line 20385 of the generated file), deliberately
	# stopping command-line variables from propagating into sub-makes;
	# a curated FLAGS_TO_PASS list forwards specific ones explicitly,
	# and LIMITS_H_TEST is on that list (which is why THAT override
	# worked) while INSTALL_HEADERS_DIR is not (which is why the -17
	# run still echoed real-install-headers-tar). EXTRA_GCC_FLAGS is
	# expanded by the toplevel directly inside every stage's gcc
	# sub-make invocation, so anything in it lands on the sub-make's
	# own command line -- where it beats the subdir Makefile's
	# assignment. Both facts read from the generated Makefile in the
	# preserved -17 tree, not inferred.
	# EXTRA_GCC_FLAGS's DEFAULT is not empty -- it is the very channel
	# that forwards GCC_FOR_TARGET, STMP_FIXPROTO and LIMITS_H_TEST to
	# each stage's gcc sub-make (read from the generated toplevel
	# Makefile, lines 820-823). Overriding it wholesale, as -18 briefly
	# did, would have clobbered the LIMITS_H_TEST forwarding and
	# reintroduced the glimits-only limits.h while appearing to fix the
	# header copy. The replacement value therefore carries everything
	# the default carried -- with the two variables resolved to the
	# verified facts this recipe already asserts -- plus the one
	# addition. GCC_FOR_TARGET's default expansion is the in-tree xgcc
	# invocation, which $(MAKE) re-derives identically in the sub-make
	# when passed explicitly here.
	make bootstrap LIMITS_H_TEST=true \
		EXTRA_GCC_FLAGS='LIMITS_H_TEST=true STMP_FIXPROTO=stmp-fixproto INSTALL_HEADERS_DIR=install-headers-cp' \
		-j"$(nproc)" || make_rc=$?
	#
	...
	if [ "$make_rc" -ne 0 ]; then
		echo "=== per-stage limits.h state (composed = has include_next) ==="
		for d in gcc prev-gcc stage1-gcc stage2-gcc stage3-gcc; do
			f="/build/src/build/$d/include-fixed/limits.h"
			if [ -f "$f" ]; then
				echo "$d: $(wc -l < "$f") lines, include_next=$(grep -c include_next "$f")"
			else
				echo "$d: no limits.h"
			fi
		done
