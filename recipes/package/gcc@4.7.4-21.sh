...
MINIEXTRACT
	...
	*)  seed_fixed="/usr/lib/gcc/$(/usr/bin/gcc -dumpmachine)/$(/usr/bin/gcc -dumpversion)/include-fixed" ;;
	...
	#
	#
	# Header installs go through cp, fixed at the SOURCE (#122):
	# gcc/config.build's shell default `build_install_headers_dir=
	# install-headers-tar` is what every stage's configure sources, and
	# this project's tar archives only its first argument while exiting
	# 0 -- so GCC's stage header copy silently moved nothing and stage2
	# lost syslimits.h. One line changed pre-configure makes every
	# generated Makefile carry INSTALL_HEADERS_DIR = install-headers-cp
	# natively (cp -p -r, coreutils, verified working). Chosen over
	# make-level overrides after two of those failed to reach the
	# expansion (the toplevel sets MAKEOVERRIDES= and forwards only a
