...
EOF
	...
	*)  seed_fixed="/usr/lib/gcc/$(/usr/bin/gcc -dumpmachine)/$(/usr/bin/gcc -dumpversion)/include-fixed" ;;
	...
	if [ ! -f "$seed_fixed/limits.h" ] && [ -f "$seed_tools/include/limits.h" ]; then
		echo "gcc 6.4.0-8: seeding $seed_fixed/limits.h from the compiler's own template"
		cp "$seed_tools/include/limits.h" "$seed_fixed/limits.h"
	fi
	if [ ! -f "$seed_fixed/syslimits.h" ] && [ -f "$seed_tools/gsyslimits.h" ]; then
		echo "gcc 6.4.0-8: seeding $seed_fixed/syslimits.h from the compiler's own template"
	...
	# a single bad pass.
	#
	CC=/usr/bin/gcc CXX=/usr/bin/g++ CFLAGS="-g -O0" CXXFLAGS="-g -O0" \
	...
	make -j2 all > /build/make.log 2>&1 &
	make_pid=$!
	(
		prev=-1
		stalled=0
		while true; do
			...
			if [ "$lines" != "$prev" ]; then
				last=$(tail -n 1 /build/make.log 2>/dev/null | cut -c 1-120)
				echo "[make progress] ${lines} log lines; last: ${last}"
				prev=$lines
				stalled=0
				continue
			fi
			...
			echo "=== [make STALLED] no new output for $((stalled * 30))s ==="
			echo "last log line: $(tail -n 1 /build/make.log 2>/dev/null | cut -c 1-200)"
			# What every build process is actually waiting on. wchan is
			# the whole point: do_wait means "waiting for a child", so
			# the interesting process is the one blocked on anything
			# else -- that is where the build really is.
			...
			#
			# -9 is a diagnostic revision: it reports the stall and
			# then LEAVES IT ALONE, where -8 killed it.
			#
			# This hang has now happened four times at the identical
			# instruction -- always cc1 in __futex_wait compiling one
			# libgcc conftest -- and killing it each time destroys the
			# only copy of the state worth examining. The stage1
			# compiler that hangs exists nowhere but inside this
			# container, so it cannot be run against the failing input
			# after the fact.
			#
			# Keeping it alive makes the container exec-able while it
			# is still stuck, which is what turns "gcc hangs" into a
			# reproducer. The build is already lost either way; the
			# only question is whether anything is learned from it.
			#
			# Do not leave this in a revision meant to actually build
			# gcc: a stalled build that is never killed holds a build
			# slot indefinitely.
			#
			# -9 left the stall alive so the compiler could be examined
			# while stuck; that job is done (the reproducer is in #116),
			# and a stalled build that is never killed holds its slot
			# and wedges the container in "deleting" (#119). Back to
			# killing it.
			echo "=== killing the stalled build ==="
			for proc in /proc/[0-9]*; do
				comm=$(tr -d "\0" < "$proc/comm" 2>/dev/null)
				case "$comm" in
	...
	progress_pid=$!
	# set +e around the wait/cleanup: recipes run under set -e, and both
	# of these legitimately return nonzero. `wait` on a failed make is
	# how make_rc gets its value at all, and `wait` on a helper we just
	# SIGTERM'd returns 143. gcc/4.7.4-10 built an entire compiler and
	# then died on exactly that second one.
	set +e
	wait "$make_pid"
	make_rc=$?
