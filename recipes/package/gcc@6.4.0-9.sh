	...
	*)  seed_fixed="/usr/lib/gcc/$(/usr/bin/gcc -dumpmachine)/$(/usr/bin/gcc -dumpversion)/include-fixed" ;;
	...
	(
		...
		while true; do
			...
			# slot indefinitely.
			#
			echo "=== leaving the stalled build alive for diagnosis (see 6.4.0-9) ==="
			echo "=== exec into this container and examine pid list above ==="
			# Keep reporting every 10 minutes so the log shows it is
			# still stuck rather than going silent.
			stalled=20
			continue
		done
	) &
