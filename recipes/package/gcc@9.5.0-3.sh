...
EOF
	...
	*)  seed_fixed="/usr/lib/gcc/$(/usr/bin/gcc -dumpmachine)/$(/usr/bin/gcc -dumpversion)/include-fixed" ;;
	...
	# nothing bounding it.
	#
	echo "=== gcc 9.5.0-3: make complete, entering self-test"
...
pkg_install() {
	...
	install_include_dir="$PKG_DESTDIR/usr/lib/gcc/x86_64-unknown-linux-gnu/9.5.0/include"
	...
	ln -sf /usr/bin/ld "$PKG_DESTDIR/usr/libexec/gcc/x86_64-unknown-linux-gnu/9.5.0/ld"
}
