...
pkg_build() {
	...
	#
	# The fix is upstream's own other line: append $(LDADD), exactly as
	# cldr_plurals_LDADD already does. Applied to Makefile.in rather
	# than Makefile.am because configure generates the Makefile from
	# the shipped .in and patching the .am would need automake to be
	# re-run. Idempotent by construction: lines that already carry
	# $(LDADD) are skipped, so cldr_plurals is not doubled.
	#
	...
	#
	# The same absorption failure again, one library along (#220).
	#
	# gettext-tools bundles libxml2 under gnulib-lib/libxml, and its own
	# configure chooses it: "checking whether to use the included
	# libxml... yes". It is built as the convenience archive
	# libxml_rpl.la and named in libgettextlib's LIBADD:
	#
	#   @INCLUDED_LIBXML_TRUE@am__append_57 = libxml_rpl.la
	#   libgettextlib_la_LIBADD = $(gl_libgettextlib_LTLIBOBJS) $(am__append_57) ...
	#
	# libtool is supposed to absorb a convenience archive into the
	# library that names it, and under --disable-shared it does not --
	# which is precisely the libcroco failure above, one library along.
	# msgfmt then fails to link on xmlNewNode, xmlAddChild,
	# xmlDocFormatDump and a dozen more, because src/its.c uses libxml2
	# directly and the objects were compiled and thrown away.
	#
	# Merged with ar for the same reason and in the same shape as the
	# libtextstyle merge above, and asserted rather than assumed.
	#
	if ! make -j"$(nproc)" -C gnulib-lib; then
		echo "gettext: gnulib-lib itself failed to build" >&2
		exit 1
	fi
	G=gnulib-lib/.libs
	if [ -f "$G/libxml_rpl.a" ]; then
		tmp=$(mktemp -d)
		abs="$(pwd)/$G/libxml_rpl.a"
		( cd "$tmp" && ar x "$abs" ) || { echo "gettext: could not unpack libxml_rpl.a" >&2; exit 1; }
		if ls "$tmp"/*.o >/dev/null 2>&1; then
			ar r "$G/libgettextlib.a" "$tmp"/*.o || {
				echo "gettext: could not merge libxml_rpl.a into libgettextlib.a" >&2
				exit 1
			}
			ranlib "$G/libgettextlib.a" || exit 1
		fi
		rm -rf "$tmp"
	fi
	xmldefs=$(nm "$G/libgettextlib.a" 2>/dev/null | grep -c ' T xmlNewNode')
	if [ "$xmldefs" -eq 0 ]; then
		echo "gettext: libgettextlib.a still does not define xmlNewNode after the merge (#220)" >&2
		ls -la "$G" >&2 || true
		nm "$G/libxml_rpl.a" 2>/dev/null | grep -c ' T xml' >&2 || true
		exit 1
	fi
	echo "  libgettextlib.a now defines the bundled libxml2 (xmlNewNode present)"
