...
CRTGATE
	if ! tcc -B"$PKG_DESTDIR/usr/lib/x86_64-linux-gnu" \
	         -Wl,-rpath="$PKG_DESTDIR/lib/x86_64-linux-gnu" \
	         /build/crtgate.c -o /build/crtgate 2>/build/crtgate.err; then
		echo "TCC cannot link against the CRT objects this glibc stages:" >&2
	...
	# Gate: every symlink must resolve, inside the staged tree, to the
	# exact bytes of the real file -- a dangling or wrong link here is
	# a build environment that cannot link C++, discovered at gcc's
	# bootstrap instead of now.
	for l in "$PKG_DESTDIR/usr/lib"/*.so*; do
		test -L "$l" || continue
		if ! cmp -s "$l" "$PKG_DESTDIR/lib/x86_64-linux-gnu/$(basename "$l")"; then
			echo "symlink $l does not resolve to its slibdir original" >&2
			exit 1
		fi
	done
