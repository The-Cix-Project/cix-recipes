...
pkg_changelog="2.44-16: rebuilt so the member-less libpthread.a, librt.a and libdl.a survive into the artifact (#324, #328). glibc 2.34 folded pthread, rt and dl into libc.so.6 but still installs those three archives with no members, because that empty archive is the link-time contract for -lpthread, -lrt and -ldl. ADR-0251 clause 2 dropped them as archives whose shared counterpart ships beside them, and the fix exempting an empty one landed after 2.44-14 was published, so the cached artifact still lacks them. Measured on the running box: the installed glibc ships exactly libm, libmcheck, libg and libc_nonshared and nothing else, and gcc fails with cannot find -lpthread and cannot find -ldl. That one gap blocks every gcc-linked package passing those flags; btrfs-progs and openssl both hit it while rebuilding for #328. 2.44-15 was an earlier attempt at this same rebuild, withdrawn after it failed; its recipe checksum was independently verified correct against the upstream tarball, so that failure was not a bad checksum. No source change. 2.44-13: declare pkg_toolchain=gcc and its reason (#222, ADR-0226)"
