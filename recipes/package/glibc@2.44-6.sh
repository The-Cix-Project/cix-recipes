...
pkg_install() {
	...
	# --libdir, which is why that is set to the multiarch path above.
	...
	for f in "$PKG_DESTDIR/usr/lib/x86_64-linux-gnu"/*crt*.o; do
		test -e "$f" || continue
		cp -a "$f" "$PKG_DESTDIR/lib/x86_64-linux-gnu/"
	done
	for f in libc.so libm.so libc.a libc_nonshared.a libpthread.a librt.a libdl.a; do
		test -e "$PKG_DESTDIR/usr/lib/x86_64-linux-gnu/$f" || continue
		cp -a "$PKG_DESTDIR/usr/lib/x86_64-linux-gnu/$f" \
