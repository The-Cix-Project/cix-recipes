...
pkg_build() {
	...
	#
	# Three math.h constructs TCC does not provide, supplied on the
	# command line. configure tests for all three together and reports
	# them together:
	#
	#   configure: error: cannot find required macros: NAN, isgreater()
	#                     and isgreaterequal()
	#
	# NAN was already handled. The other two are glibc macros that
	# expand to __builtin_isgreater / __builtin_isgreaterequal, which
	# TCC does not implement (the #208 family), so the configure probe
	# fails to compile and autoconf reports the macro as absent.
	#
	# Defining the BUILTINS rather than the macros is deliberate, and
	# is the same shape as libnl's shim. Defining isgreater() itself
	# would be redefined by <math.h> the moment it is included;
	# defining what that macro expands TO means glibc's own definition
	# is used and simply resolves to something TCC can compile.
	#
	# The plain comparison is correct for every use here. The only
	# difference from the real builtins is that they compare without
	# raising an FP exception on a NaN operand; htop compares CPU and
	# memory percentages, which are never NaN, and nothing in it
	# inspects the FP exception flags.
	#
	CC=tcc ac_cv_prog_CC=tcc \
		CFLAGS='-DNAN=(__builtin_nanf("")) -D__builtin_isgreater(x,y)=((x)>(y)) -D__builtin_isgreaterequal(x,y)=((x)>=(y))' \
