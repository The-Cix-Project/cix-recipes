...
pkg_install() {
	...
	#
	if nm -a "$ip_bin" 2>/dev/null | grep -q 'do_ipvrf'; then
		echo "do_ipvrf is still in the binary -- the removal did not take" >&2
		exit 1
	fi
	nm -a "$ip_bin" 2>/dev/null | grep -q 'vrf_link_util' || {
