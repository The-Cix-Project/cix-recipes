...
pkg_install() {
	...
	#
	# Verify the declared capability loss in BOTH directions.
	#
	# Checked against the BUILD INPUTS, not the binary's symbol table.
	# -12 used `nm` and reported do_ipvrf absent (pass) and
	# vrf_link_util absent (fail) -- which is exactly what a binary with
	# no symbol table looks like, meaning the passing half proved
	# nothing. A check that cannot tell "removed" from "cannot see"
	# is not a check.
	#
	grep -q 'iplink_vrf\.o' ip/Makefile || {
		echo "iplink_vrf.o is not in IPOBJ -- ip link type vrf was lost, which was not intended" >&2
		exit 1
	}
	if grep -q 'ipvrf\.o' ip/Makefile; then
		echo "ipvrf.o is still in IPOBJ -- the removal did not take" >&2
		exit 1
	fi
	if grep -q 'do_ipvrf' ip/ip.c; then
		echo "do_ipvrf is still referenced in ip.c" >&2
		exit 1
	fi
	echo "capability check: ip vrf removed (#211), ip link type vrf retained (iplink_vrf.o linked)"

	# Report what the symbol table can actually see, rather than
	# asserting on it. Informational: a stripped binary is a fine
	# outcome, it just cannot answer the question above.
	syms=$(nm -a "$ip_bin" 2>/dev/null | wc -l)
	echo "symbol table entries in ip: $syms"
