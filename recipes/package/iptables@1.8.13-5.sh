...
pkg_artifact_sha256="e825efe054e3403aa66466cfd2e8bb166e5bd904b95ad13bbaeb81adff8a85b8"
...
pkg_changelog="1.8.13-5: rebuilt against tcc 0.9.28rc (ADR-0223). The 2017 0.9.27 release could give two simultaneously-live locals the same stack slot (#216), a fault that corrupts values silently wherever the aliased pair is only read and written, so every binary it produced is suspect rather than merely the ones that failed. No source change: the revision exists to make the rebuild real, because an image version is a hash of the package manifest (ADR-0155) and a same-version reinstall is deduped and discarded. 1.8.13-4: corrects the -3 assertion, which looked for libxtables.pc -- the pkg-config module is named xtables, not after the library file. 1.8.13-3: overrides AM_DEPFLAGS, which passes -Wp,-MMD,... that TCC cannot parse. 1.8.13-2: enables the nftables backend, now that libnftnl>=1.2.6 exists as a Cix package (1.2.9); ships libxtables/libip4tc/libip6tc headers and pkg-config files so other packages can build against them; declares libmnl/libnftnl (#206, #207)"
