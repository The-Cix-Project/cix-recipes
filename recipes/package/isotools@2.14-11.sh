...
pkg_name="isotools"
pkg_version="2.14-11"
pkg_changelog="2.14-11: mokutil: isotools checks for it with test -x and exits telling the operator to install it onto the build image first. Declaring it is that same requirement expressed where the composer can satisfy it. 2.14-10: declares its build tools so it can be rebuilt through the ordinary install path (#206). isotools assembles a bootable ISO by invoking grub-mkrescue, sbsign, xorriso and mformat. It already checks for each with test -x and exits with an instruction to install it onto the build image first -- declaring them is the same requirement, expressed where the composer can act on it (ADR-0199) instead of at run time."
...
#
pkg_build_depends="tcc make linux-headers bash coreutils sed grep gawk binutils findutils diffutils grub sbsigntools xorriso mtools mokutil"
