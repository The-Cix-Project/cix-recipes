...
#
# Re-pinned from v1.4.0 (Phase 40/ADR-0057, task #852) -- that pin had
# gone 45 releases stale and predated kanxeo-recover.c (ADR-0146)
# entirely, so this same hostbuild round now also builds+stages it,
# alongside kanxeod/kanxeoctl/web/mkbootroot/kanxeo-install/
# mkinstalleriso, closing the gap iso_build_start()'s own required-
# artifacts check (daemon/src/main.c) was already surfacing.
#
...
pkg_name="kanxeo"
pkg_version="v1.49.0"
pkg_source="https://osakka:REPLACE_WITH_REAL_TOKEN@git.home.arpa/api/v1/repos/itdlabs/kanxeo/archive/v1.49.0.tar.gz"
...
#
# build/kanxeo-recover joins as of this re-pin (ADR-0146): mkinstalleriso
# now takes it as a required argument (image/src/mkinstalleriso.c), so
# a self-built ISO needs it staged here to produce a recovery-capable
# image, exactly like kanxeo-install already is.
pkg_build() {
	make build/kanxeod build/kanxeoctl build/mkbootroot build/kanxeo-install build/kanxeo-recover build/mkinstalleriso
}

pkg_install() {
	cp build/kanxeod build/kanxeoctl build/mkbootroot build/kanxeo-install build/kanxeo-recover build/mkinstalleriso \
	   "$PKG_DESTDIR/"
