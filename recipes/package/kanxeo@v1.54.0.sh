...
#
# Re-pinned from v1.53.0 (task #852/#865): the very first real
# end-to-end self-hosted deploy against 192.168.15.95 (building a real
# kanxeo-hosttools image for the first time ever) found and fixed a
# genuine mksquashfs LD_LIBRARY_PATH bug (ADR-0154) blocking the
# server-side bootroot assembly step this exact hostbuild round
# depends on -- v1.54.0 is the first tag carrying that fix.
#
...
pkg_name="kanxeo"
pkg_version="v1.54.0"
pkg_source="https://osakka:REPLACE_WITH_REAL_TOKEN@git.home.arpa/api/v1/repos/itdlabs/kanxeo/archive/v1.54.0.tar.gz"
...
#
# build/kanxeo-recover joins the same round (ADR-0146): mkinstalleriso
