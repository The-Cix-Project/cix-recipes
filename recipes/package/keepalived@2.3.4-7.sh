...
#
# ipset support is restored here, which closes the set: 2.3.4-2 lost
# iptables, nftables AND ipset when it stopped picking up the old shared
# build host's ambient Debian libraries, and -3/-4 (nftables), -5
# (iptables) and -6 (ipset) have each brought one back from a package
# Cix built itself.
#
# The blocker was not that libipset did not exist -- ipset had been
# BUILDING it all along and discarding it, on the reasoning that its own
# CLI links it statically so nothing else could want it. ipset@7.24-8
# ships it.
#
