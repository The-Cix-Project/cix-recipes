...
# by every subsequent `pkg sync` (confirmed directly: `pkg recipe show
# kernel --version=6.18.40-10` still showed the literal placeholder
...
# --build-image, not declared) was already correct; -10 was purely a
# re-pin to dodge -9's OWN instance of this same caching gap, and -11
# is purely a second re-pin for the identical reason, this time
# getting the live-token substitution done BEFORE this version's very
# first sync rather than after.
#
