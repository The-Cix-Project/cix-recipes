...
pkg_depends=""
pkg_artifact_sha256="4a61559f989bea17a2164167bb0316adae1061214428b649f0759a0ae9909519"

# Re-pinned to -16: -15 plus pkg_artifact_sha256= -- 192.168.15.95 has
# no gcc-bearing "dev" build image (its kernels have always come from
# the dev-machine build), so the btrfs kernel was built on the dev
# machine from -15's own exact documented procedure (allnoconfig +
# fragment merge + olddefconfig, make bzImage + modules +
# modules_install + depmod) and published to the artifact cache; this
# line lets `pkg hostbuild kernel` on the box take the artifact tier
# and skip the on-box build it cannot run. Same QEMU-verified binary
# test_installer booted four times.
#
