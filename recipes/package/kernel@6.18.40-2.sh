...
# Re-pinned to -2 (ROADMAP Part 165, the ADR-0165 production
# regression investigation): image/kernel/qemu-part1.config gained
# CONFIG_CFS_BANDWIDTH=y, the real, previously-undiscovered Kconfig
# gap that made every cgroup v2 cpu.max write on 192.168.15.95 fail
# with ENOENT (see that commit's own message for the full root-cause
# chain, and container_create_last_error_step()'s own doc comment for
# how it was actually diagnosed on a box with no shell at all).
#
