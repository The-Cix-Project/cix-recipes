#
# 7.2.3-11: btrfs statfs() reports a subvolume's own qgroup limit, so a
...
# report 16 GiB, because btrfs_statfs() reads the superblock's totals
# and never consults the per-subvolume qgroup -- even though the limit
# is set, correct, and the daemon reads it (GET /v1/volumes/.../usage,
# source "qgroup"). Tier 1 (cix-procfuse, cix v2.57.145) already gave
# the container its own /proc/partitions; this is the df half.
#
# A 44-line patch to fs/btrfs, applied to the pinned 7.2.3 source
# before configure: btrfs_qgroup_statfs_limit() looks up the level-0
# qgroup for the subvolume statfs was called on and, if it carries a
# limit, btrfs_statfs() overrides f_blocks/f_bfree/f_bavail with it.
# MAX_EXCL is preferred over MAX_RFER because exclusive bytes are what a
# snapshot-seeded subvolume actually owns, matching what Cix sets
# (cix_btrfs_qgroup_limit_excl). A complete no-op wherever no qgroup
# limit is set, which is every filesystem that is not one of ours -- so
# host disk reporting is unchanged (the daemon's own statfs readers,
# main.c on g_base_dir and disk.c on a mount_path, are never given a
# limit; measured, ADR-0286).
#
# The patch is applied with `patch -p1 --forward -F0`, which fails the
# build rather than fuzzing, and the build then verifies the injected
# symbol is present in the tree -- so a future 7.2.3 source bump that
# moves an anchor fails loudly here instead of silently shipping an
# unpatched kernel. No config symbol changes; the olddefconfig gate is
# untouched. This is a standing patch obligation: it must be rebased on
# every kernel version bump (ADR-0286 records the trade).
#
#
...
EOF
	...
	# subvolume's own qgroup limit. Applied to the pinned 7.2.3 source
	# here, before configure. --forward -F0 means a hunk that does not
	# apply exactly fails the build; the grep after it means a patch
	# that somehow applied to nothing also fails, rather than shipping a
	# kernel whose df still lies.
	#
	cat > /build/extra/btrfs-statfs-qgroup.patch <<'CIXPATCH'
--- a/fs/btrfs/qgroup.h
+++ b/fs/btrfs/qgroup.h
@@ -330,6 +330,9 @@
...
+/* Cix ADR-0286: the level-0 qgroup limit for a subvolume, for statfs. */
...
CIXPATCH
	patch -p1 --forward -F0 < /build/extra/btrfs-statfs-qgroup.patch || {
		echo "kernel: ADR-0286 btrfs statfs patch did not apply cleanly" >&2
		exit 1
	}
	for sym in \
		"fs/btrfs/qgroup.h:btrfs_qgroup_statfs_limit" \
		"fs/btrfs/qgroup.c:btrfs_qgroup_statfs_limit" \
		"fs/btrfs/super.c:btrfs_qgroup_statfs_limit"; do
		f=${sym%%:*}; needle=${sym#*:}
		if ! grep -q "$needle" "$f"; then
			echo "kernel: ADR-0286 patch verify failed -- $needle absent from $f" >&2
			exit 1
		fi
	done
