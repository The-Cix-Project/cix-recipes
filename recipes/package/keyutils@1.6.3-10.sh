...
#               nothing.
pkg_toolchain="gcc"
pkg_toolchain_reason="missing language feature: linking needs a symbol TCC does not provide, and the alternative was fabricating a placeholder archive for a binary this package discards"
pkg_build_depends="bash coreutils make tcc linux-headers binutils sed grep gcc"
pkg_changelog="1.6.3-10: rebuilt so ADR-0251's finalize policy actually runs over these bytes (#328). No source or recipe change. The policy strips ELF and has done since 7c4a7332 on 2026-09-06, but it only runs when a package is BUILT -- and the artifact tier correctly installs an already-approved cached artifact rather than rebuilding, so bytes published before that date keep shipping unstripped indefinitely with nothing reporting it. This artifact was published 2026-09-04. Measured on the installer ISO: 5 of 318 ELF objects still carried .debug*, 3.93 MiB in total, mkfs.btrfs alone being 80% debug. The revision exists to make the rebuild real, the same reason ADR-0155 forces elsewhere. 1.6.3-9: install to /usr/lib rather than the multiarch directory (#184 stage 2). The multiarch triplet is a Debian convention for letting several architectures share one filesystem and a Cix image has one architecture. LIBDIR and USRLIBDIR now name the same directory, which is what upstream's own split assumed a distribution would decide; the development symlink becomes an ordinary same-directory one rather than an absolute path into another partition-era location. The artifact approval is dropped because those bytes came from the previous revision. 1.6.3-8: declare pkg_toolchain=gcc and its reason (#222, ADR-0226)"
...
#
# LIBDIR and USRLIBDIR both name /usr/lib (#184). Upstream splits them
# so a distribution can put the runtime object on the root filesystem
# and the development symlink under /usr; this platform has one library
# directory, so the split collapses and `-L/usr/lib -lkeyutils` resolves
# in the directory it is already looking at.
...
# checked.
pkg_build() {
	make CC=/usr/bin/gcc CXX=/usr/bin/g++ LIBDIR=/usr/lib USRLIBDIR=/usr/lib \
	...
	make CC=/usr/bin/gcc LIBDIR=/usr/lib USRLIBDIR=/usr/lib \
...
pkg_install() {
	...
	cp -a libkeyutils.so.1.10 "$PKG_DESTDIR/usr/lib/"
	ln -s libkeyutils.so.1.10 "$PKG_DESTDIR/usr/lib/libkeyutils.so.1"
	ln -s libkeyutils.so.1 "$PKG_DESTDIR/usr/lib/libkeyutils.so"
	...
	# -L, not -e, for the development symlink. The original reason has
	# gone: it used to point at an ABSOLUTE /lib/x86_64-linux-gnu path,
	# correct on an installed image but unresolvable inside PKG_DESTDIR
	# where nothing is mounted at /, so `test -e` -- which FOLLOWS a
	# symlink -- rejected a correctly-created link and failed -5
	# silently, because `test` says nothing when it fails. With one
	# library directory the link is relative and same-directory, so -e
	# would now pass. -L is kept anyway because it asserts the thing
	# actually meant: that this is a symlink, not a second copy of the
	# object.
	test -e "$PKG_DESTDIR/usr/lib/libkeyutils.so.1.10"
	test -L "$PKG_DESTDIR/usr/lib/libkeyutils.so.1"
