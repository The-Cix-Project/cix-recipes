...
#
# pkgconf because configure locates lzma and zstd through pkg-config.
# Undeclared, PKG_CONFIG expands to nothing and the shell runs the bare
# flag, so the -3 build reported three times over:
#
#   ./configure: line 20671: --modversion: command not found
#
# and #302's missing-command gate refused the build naming --modversion
# as the missing program. Reading that as a strange tool rather than as
# an unset variable is the trap; the tool is pkg-config.
pkg_build_depends="tcc make linux-headers bash coreutils sed grep gawk binutils findutils diffutils pkgconf zlib xz zstd"
pkg_depends="zlib xz zstd"
pkg_changelog="3.8.1-4: declare pkgconf. configure finds lzma and zstd through pkg-config; without it PKG_CONFIG is empty and the shell runs --modversion as a command, three times. The -3 blake2 packing fix and its _Static_assert gate both passed, and configure ran to completion. 3.8.1-3: bracket the two BLAKE2 param structs with real #pragma pack lines instead of rewriting the macro. TCC rejects a _Pragma between a struct body and its semicolon, which is where BLAKE2_PACKED puts one, so the -2 spelling fix failed on the same line -- caught by the _Static_assert gate before any object compiled. 3.8.1-2: give archive_blake2.h the pragma spelling TCC implements, gated by a _Static_assert on both param struct sizes. The -1 build died at archive_blake2.h:106 because TCC defines neither _MSC_VER nor __GNUC__ and so took the header pack 1 arm, which is not TCC syntax. 3.8.1-1: libarchive, the second package cix-build-system needs to build on a Cix host (cix-build-system#124). Library only, zlib/lzma/zstd filters, every other optional dependency explicitly disabled."
