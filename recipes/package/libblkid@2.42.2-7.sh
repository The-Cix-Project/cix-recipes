...
pkg_build_depends="bash coreutils make gcc linux-headers sed grep gawk binutils pkgconf findutils diffutils libuuid"
pkg_changelog="2.42.2-7: declare diffutils. The -6 build tripped the missing-command gate (#302) on 'cmp', which util-linux's own configure and make use and which comes from diffutils -- so -6 could not build at all, and neither could -5 or anything before it; the gate is simply the first thing that said so out loud rather than letting a build quietly proceed without a tool it used. libuuid, the sibling recipe this was fixed alongside, already declared it. 2.42.2-6: install the runtime library to /usr/lib, the one library directory (#184), the same fix as libuuid 2.42.2-4 and for the same reason. util-linux splits its own output when --prefix=/usr is given and --libdir is not: the real libblkid.so.1 goes to /lib, historically so early boot could reach it, while the development symlink and static archive go to /usr/lib. The loader does not search a bare /lib here, so anything that actually dlopen()s or links this at runtime fails with \"error while loading shared libraries\" -- a failure that surfaces wherever the consumer happens to be rather than here. Fixed alongside libuuid because it is the identical split from the identical configure invocation, found by the same sweep. No source change. 2.42.2-5: declare pkg_toolchain=gcc and its reason (#222, ADR-0226)"

pkg_build() {
	CC=/usr/bin/gcc ./configure --prefix=/usr --libdir=/usr/lib --disable-all-programs --enable-libblkid
	make -j"$(nproc)"
}
...
pkg_artifact_sha256="94d64ccd9f143c7f4c94c2612ebced4d3346ac2bf792fc096de29c7a75f8ec2e"
