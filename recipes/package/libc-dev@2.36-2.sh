...
pkg_install() {
	...
	# libc.so ever staged. Same real content, same rewrite: glibc's own
	# `/usr/lib/x86_64-linux-gnu/libm.so` here is a real GNU ld script
	# (confirmed via `file`+`cat`, not a plain symlink), not a binary --
	# `GROUP ( /lib/.../libm.so.6 AS_NEEDED ( /lib/.../libmvec.so.1 ) )` --
	# the exact same "rewrite /usr/lib paths to /lib" treatment libc.so
	# already gets above, since this image's own runtime libs live under
	# /lib, not /usr/lib. libmvec.so.1 itself is only AS_NEEDED
	# (optional, SIMD-vectorized math variants) -- not staged separately,
	# same as this recipe never needed to stage every AS_NEEDED sibling
	# libc.so's own script references either.
	sed 's|/usr/lib/x86_64-linux-gnu/|/lib/x86_64-linux-gnu/|' /usr/lib/x86_64-linux-gnu/libm.so \
