...
# resolves to the same file -- so this is a fix, not a trade.
	...
	# -lrt: the identical story one library over again, and found the
	# same way -- systemd's meson.build calls cc.find_library('rt')
	# unconditionally, and configuring sd-boot (issue #177) died on
	# "C shared or static library 'rt' not found" while a complete
	# glibc sat installed. glibc >= 2.34 folded librt's functions into
	# libc.so.6 exactly as it folded libpthread and libdl, and ships no
	# librt.so link at all -- only librt.a and a stub librt.so.1
	# (confirmed by listing them directly). So the static compat
	# archive is what satisfies -lrt, which is why copying it is the
	# whole fix.
	cp -a /lib/x86_64-linux-gnu/librt.a "$PKG_DESTDIR/lib/x86_64-linux-gnu/"
