...
#
# This list was MEASURED, not reasoned about. 2.78 also declared sed,
# grep and gawk on the assumption that a build of this shape must use
# them; a clean build invokes none of the three. The only place any of
# them appears in libcap is Make.Rules' own `lib=$(shell ldd /usr/bin/ld
# | grep -E ...)`, and passing lib=lib overrides that assignment so the
# shell command is never expanded at all. Declaring a tool that is not
# used is as wrong as omitting one that is: it is the "no extras" half
# of the same property, and unlike the "no less" half nothing enforces
# it -- so it has to be checked rather than assumed.
#
# Sufficiency is enforced by the build itself. Minimality is review,
# not enforcement (ADR-0199).
pkg_build_depends="tcc make libc-dev bash coreutils binutils diffutils"
