...
pkg_changelog="1.2.9-4: install to /usr/lib rather than the multiarch directory (#184 stage 2). It also links against libmnl, which moved first, so this build is the check that a consumer still resolves a moved library through both pkg-config and the linker. The multiarch triplet is a Debian convention for sharing one filesystem between architectures and a Cix image has one architecture; /usr/lib is already on the loader search path, being glibc libdir since 2.44-7. The pkgconfig relocation goes with it -- it existed only because libdir was not where pkgconf looks. The artifact approval is dropped because those bytes came from -3. 1.2.9-3: rebuilt against tcc 0.9.28rc (ADR-0223). The 2017 0.9.27 release could give two simultaneously-live locals the same stack slot (#216), a fault that corrupts values silently wherever the aliased pair is only read and written, so every binary it produced is suspect rather than merely the ones that failed. No source change: the revision exists to make the rebuild real, because an image version is a hash of the package manifest (ADR-0155) and a same-version reinstall is deduped and discarded. 1.2.9-2: the --version-script strip used [^ ]*, which excludes only a literal space -- it ate the TAB and line-continuation backslash after libnftnl.map and left an unparseable Makefile. 1.2.9: first packaging -- unblocks iptables, and restores the nftables/firewall features keepalived lost when it stopped building against an ambient Debian libnftnl (#206, #207)"
...
pkg_build() {
	...
	# One library directory (#184) -- see libmnl 1.0.5-5, which this
	# links against and which moved first.
...
pkg_install() {
	...
	# The pkgconfig relocation is gone: with libdir at /usr/lib the
	# install writes .pc files where pkgconf already looks (#184).
	...
	#
	test -e "$PKG_DESTDIR/usr/lib/libnftnl.so.11"
	...
	readelf -d "$PKG_DESTDIR/usr/lib/libnftnl.so.11" | grep NEEDED
	readelf -d "$PKG_DESTDIR/usr/lib/libnftnl.so.11" \
