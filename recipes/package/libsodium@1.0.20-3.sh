...
pkg_build() {
	...
	# -include stdatomic.h, and this is upstream's latent bug rather
	# than a TCC defect. private/common.h reads:
	#
	...
	#
	# and includes <stdatomic.h> nowhere. configure's own C11 probe
	# DOES include it, so the probe passes and the branch it enables
	# then cannot compile. GCC never notices: it defines both macros
	# and the #ifdef arm wins, so the #elif arm is dead code on every
	# compiler upstream builds with. Measured here, on the box:
	#
	#   checking if C11 memory fences are supported... yes
	#   checking if gcc memory fences are supported... no
	#   hash_sha256_cp.c:159: error: 'memory_order_acquire' undeclared
	#
	# tcc is simply the first compiler to be exactly one and not the
	# other. Supplying the header the branch needs is truthful -- the
	# fence is real and this is where C11 declares it -- and it is the
	# alternative to the tempting fix, which is to let the probe fail
	# so ACQUIRE_FENCE becomes (void) 0. That builds, and silently
	# removes a memory fence from a crypto library.
	#
	# Safe as a force-include specifically because <stdatomic.h> is a
	# compiler-provided header that pulls in no system headers: the
	# hazard libnl's shim documents -- a -include processed before the
	# TU can define _GNU_SOURCE, latching glibc's feature-test macros
	# early -- needs a header that reaches glibc, and this one does not.
	CPPFLAGS="-include stdatomic.h" \
	    CC=tcc ./configure \
	    --prefix=/usr \
	    --libdir=/usr/lib \
	    --disable-asm \
