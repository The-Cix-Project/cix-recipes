...
pkg_changelog="4.4.36-10: the -9 changelog carried an unescaped double quote and broke its own recipe. A recipe is a shell script and pkg_changelog is a double-quoted string, so a bare quote inside it closes the string early -- the remainder was then parsed as shell, and the build died with line 60: flag: No such file or directory, naming a word from the prose. bash -n does not catch it because the result is still valid syntax, just a redirect from a file that does not exist. 4.4.36-9: declare pkgconf. The -8 build tripped the missing-command gate (#302) on '--atleast-pkgconfig-version', which is not a command at all but a pkg-config FLAG: configure runs pkg-config as a variable followed by its flags, and an empty variable leaves the flag in argv[0]. The gate was right and its message was misleading. The build exited 0 regardless, which is exactly the silent-loss case the gate exists to refuse."
