#
# -7: builds and installs libpamc (task: login(1), #288-adjacent).
# Without it this package ships a HEADER THAT DOES NOT COMPILE:
# security/pam_misc.h's very first include is <security/pam_client.h>,
# which lives in libpamc, and nothing here ever built that directory.
#
# The failure is silent in the worst way, because nothing links
# pam_misc directly today -- it only surfaces when something runs a
# compile test on the header. util-linux's configure does exactly
# that, and reported:
#
#   checking for security/pam_appl.h... yes
#   checking for security/pam_modules.h... yes
#   checking for security/pam_misc.h... no
#   configure: error: login selected, but required PAM conversation
#                     functions not available
#
# which reads as a missing PAM entirely. Measured rather than inferred:
# in the failed build's own preserved container (keep_on_failure,
# ADR-0175), GET .../files reported pam_misc.h 200, _pam_macros.h 200,
# libpam_misc.so 200 -- and pam_client.h 404, libpamc.so 404. The
# header was there and simply could not be included.
#
# libpamc is in the tarball and needs no configure flag; it was only
# ever missing from this recipe's explicit per-directory make list,
# which exists so nothing builds that this platform has not asked for.
#
#
...
pkg_name="linux-pam"
pkg_version="1.6.1-9"
pkg_changelog="1.6.1-9: declare diffutils; configure reaches for cmp and nothing declared it (#302 gate, found rebuilding from source). 1.6.1-8: rebuilt so every PAM module carries PT_GNU_STACK (#228). A shared object without that header reads to the loader as requiring an executable stack, and glibc 2.44 refuses to grant it at dlopen time, so a PAM stack loaded by a correctly marked program cannot open its own modules. It went unnoticed because glibc takes its bookkeeping from the main executable and skips the check entirely when that executable is itself unmarked, which sshd and login both are. tcc has never emitted the header, the fix landed in tcc 0.9.28rc-10, and the build image was never upgraded past rc-9 until now. No source change: the revision exists because an image version is a hash of the package manifest (ADR-0155), so a same-version rebuild is deduped and thrown away. 1.6.1-7: build and install libpamc. Without it this package shipped a header that cannot compile: security/pam_misc.h includes security/pam_client.h, which lives in libpamc, and nothing here built that directory. It stayed invisible because nothing links pam_misc directly yet; util-linux configure runs a compile test on the header and concluded PAM was unusable, refusing to build login(1). Confirmed in the failed build container: pam_misc.h present, pam_client.h absent. 1.6.1-6: install to /usr/lib rather than the multiarch directory (#184 stage 2). PAM computes SECUREDIR as libdir/security and bakes that value into libpam.so's own DEFAULT_MODULE_PATH, so overriding libdir moves the libraries and the modules and the compiled-in lookup path together, exactly as it did for the multiarch directory before. nss-pam-ldapd 0.9.13-5 moves with it in the same change, because its own pam-seclib-dir must name the identical directory or libpam looks somewhere its module is not. The artifact approval is dropped because those bytes came from the previous revision. 1.6.1-5: rebuilt against tcc 0.9.28rc (ADR-0223). The 2017 0.9.27 release could give two simultaneously-live locals the same stack slot (#216), a fault that corrupts values silently wherever the aliased pair is only read and written, so every binary it produced is suspect rather than merely the ones that failed. No source change: the revision exists to make the rebuild real, because an image version is a hash of the package manifest (ADR-0155) and a same-version reinstall is deduped and discarded. 1.6.1-4: declare build tools incl. libxcrypt for crypt() and findutils (#206); keep the pkg-config file (#174)"
...
# autotools tree.
#
# 1.6.1-9: diffutils. Its configure reaches for cmp and it was never
# declared -- named by the #302 missing-command gate the first time
# this package was actually rebuilt from source, restoring the fleet
# after the btrfs migration (there was no artifact to install from).
#
#   ./configure: cmp: command not found
#
# diffutils is what ships it ("GNU diff/cmp/diff3/sdiff"). The gate
# refuses on any occurrence of the phrase, so this is the whole set --
# it reported exactly one.
#
pkg_build_depends="tcc make linux-headers bash coreutils sed grep gawk binutils findutils libxcrypt diffutils"
...
# platform's one library directory, /usr/lib (#184) -- both the
# libraries (--libdir= above) AND the modules: confirmed
...
# read "/lib/x86_64-linux-gnu/security/" before this revision moved
# libdir, and reads "/usr/lib/security/" after it (checked via a real
...
pkg_install() {
	...
	make -C libpamc DESTDIR="$PKG_DESTDIR" install
	...
	rm -f "$PKG_DESTDIR/usr/lib"/*.la "$PKG_DESTDIR/lib64/security"/*.la
