...
pkg_changelog="1.4.20-1: upstream bump so the vendored gnulib knows glibc 2.44 (#198); source moved to a reachable mirror (#167)"
...
#
# 1.4.20-1 (#198): an upstream bump, not another workaround.
#
# 1.4.19's vendored gnulib predates glibc 2.44 and declares a
# posix_spawn_file_actions_addchdir prototype that clashes with the one
# glibc now provides:
#
#   ./spawn.h:1445: error: incompatible types for redefinition of
#       'posix_spawn_file_actions_addchdir'
#
# That only became visible once libc-dev stopped pinning the whole
# catalogue to glibc 2.36 headers (#187) -- the runtime had been 2.44
# for a long time, so every package was compiling against headers eight
# releases behind the library it linked. m4 is simply the first source
# whose own bundled copy of gnulib is too old to know the difference.
#
# This recipe already carries ONE gnulib workaround
# (_GL_EXTERN_INLINE_STDHEADER_BUG, kept below -- it answers a real TCC
# conformance gap and is unrelated to glibc's version). A second
# workaround, this time for a glibc that gnulib simply predates, would
# be treating "our vendored copy is old" as a permanent condition. The
# upstream release already contains the fix.
#
# pkg_source also moves from ftp.gnu.org to mirrors.kernel.org, which
# this network can actually reach over TLS (#167) -- the same host every
# other GNU source in this set is moving to.
#
pkg_build() {
