...
pkg_changelog="1.4.20-5: rebuilt against tcc 0.9.28rc (ADR-0223). The 2017 0.9.27 release could give two simultaneously-live locals the same stack slot (#216), a fault that corrupts values silently wherever the aliased pair is only read and written, so every binary it produced is suspect rather than merely the ones that failed. No source change: the revision exists to make the rebuild real, because an image version is a hash of the package manifest (ADR-0155) and a same-version reinstall is deduped and discarded. 1.4.20-4: correct an upstream gnulib typo in obstack.h's non-GCC obstack_free -- OBSTACK_CPTR for _OBSTACK_CPTR, unreachable under GCC and fatal under TCC (#198)"
...
pkg_build() {
	#
	# One-character correction to an upstream gnulib typo, applied to
	# the source template before configure copies it into lib/obstack.h.
	#
	# gnulib's obstack.h defines obstack_free twice: a __GNUC__ version
	# using a statement expression, and a plain-C fallback under
	# `#else /* not __GNUC__-like */`. The fallback misspells the
	# pointer-comparison cast on its very first line:
	#
	#   (((_OBSTACK_CPTR) (h)->chunk < (OBSTACK_CPTR) (h)->temp.tempptr
	#                                   ^ no leading underscore
	#
	# The other three casts in the same expression -- and every one of
	# the ~six uses elsewhere in the header -- spell it _OBSTACK_CPTR,
	# which is the name the header actually typedefs (obstack.in.h:144
	# and :146). OBSTACK_CPTR is defined nowhere at all.
	#
	# GCC and Clang take the __GNUC__ branch, so the fallback is never
	# compiled and upstream has never seen this. TCC defines no
	# __GNUC__, takes the fallback, and dies the first time any caller
	# expands the macro:
	#
	#   builtin.c:1472: error: 'OBSTACK_CPTR' undeclared
	#
	# Gated in both directions: the typo must be present before (or the
	# upstream release has fixed it and this patch is now stale noise
	# that should be removed), and absent after.
	#
	if ! grep -q '(OBSTACK_CPTR)' lib/obstack.in.h; then
		echo "m4: lib/obstack.in.h no longer carries the OBSTACK_CPTR" \
		     "typo -- upstream has fixed it, so drop this patch" >&2
		exit 1
	fi
	sed -i 's/(OBSTACK_CPTR)/(_OBSTACK_CPTR)/g' lib/obstack.in.h || exit 1
	if grep -q '[^_]OBSTACK_CPTR)' lib/obstack.in.h; then
		echo "m4: OBSTACK_CPTR typo survived the patch" >&2
		exit 1
	fi
	...
	#
	# Read lib/Makefile, not lib/spawn.h: spawn.h is generated during
	# `make` (lib/Makefile.in:10170 substitutes this same variable into
	# it), so at this point it does not exist yet. 1.4.20-2 gated on
	# spawn.h and failed here with `grep: lib/spawn.h: No such file or
	# directory` -- a gate that could only ever report absence.
	if ! grep -q '^REPLACE_POSIX_SPAWN_FILE_ACTIONS_ADDCHDIR = 1$' lib/Makefile; then
