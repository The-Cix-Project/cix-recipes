...
# binutils for strip: ADR-0251 finalizes every ELF this produces,
# and the -2 revision compiled, linked and passed its own
# ./minisign -v gate before failing there. Worth reading as the
# finalize step working -- it names the missing tool and the ADR
# rather than shipping an unstripped binary.
pkg_build_depends="tcc bash coreutils linux-headers binutils libsodium"
pkg_depends="libsodium"
pkg_changelog="0.12-3: declare binutils -- ADR-0251 finalize needs strip, and -2 reached it after compiling, linking and passing ./minisign -v. 0.12-2: declare linux-headers -- glibc's <limits.h> pulls <linux/limits.h> via bits/local_lim.h, and the -1 revision failed on minisign.c line 4. 0.12-1: minisign, the independent oracle test_releasekey has been skipping for want of (#406)."
