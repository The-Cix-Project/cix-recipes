...
pkg_changelog="0.96-5: declares autoconf/automake/libtool -- mtr ships no configure and its bootstrap.sh runs autoheader/automake/autoconf. 0.96-4: declares its build tools so it can be rebuilt through the ordinary install path (#206)"
#
# Build tools derived rather than guessed: the baseline the declaring
# recipes converge on, plus what this recipe's own pkg_build() invokes
# and the libraries it already declares. See
