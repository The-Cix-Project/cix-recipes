...
#
pkg_build_depends="tcc make linux-headers bash coreutils sed grep gawk binutils findutils diffutils pkgconf ncurses autoconf automake libtool m4 glibc"
pkg_changelog="0.96-8: install to /usr/lib rather than the multiarch directory (#184 stage 2). The multiarch triplet is a Debian convention for letting several architectures share one filesystem and a Cix image has one architecture. /usr/lib is already on the loader search path, being glibc libdir since 2.44-7. The artifact approval is dropped because those bytes came from the previous revision. 0.96-7: declares glibc, which provides the libresolv.so.2 this recipe stages. The copy itself is unchanged and still correct -- mtr genuinely needs libresolv at runtime and the image baseline does not carry it -- but it was taken from the build sandbox without naming its source (#110). The recipe's own comments already noted that the fuller fix is adding libresolv to pkg_seed_image_baseline(); that is a daemon change and is left as one rather than smuggled in here. 0.96-6: rebuilt against tcc 0.9.28rc (ADR-0223). The 2017 0.9.27 release could give two simultaneously-live locals the same stack slot (#216), a fault that corrupts values silently wherever the aliased pair is only read and written, so every binary it produced is suspect rather than merely the ones that failed. No source change: the revision exists to make the rebuild real, because an image version is a hash of the package manifest (ADR-0155) and a same-version reinstall is deduped and discarded. 0.96-5: declares autoconf/automake/libtool -- mtr ships no configure and its bootstrap.sh runs autoheader/automake/autoconf. 0.96-4: declares its build tools so it can be rebuilt through the ordinary install path (#206)"
...
pkg_install() {
	...
	# Found rather than hardcoded, because the SOURCE is a library in
	# the build sandbox and this platform is collapsing its library
	# directories (#184) -- glibc and gcc move in later stages, and a
	# recipe naming one fixed directory would break on the day they do.
	resolv=""
