...
pkg_version="2.6.14-2"
pkg_changelog="2.6.14-2: declare build tools so this can be rebuilt (#206); keep the pkg-config file, this package ships the headers and .so it describes (#174)"
...
pkg_build_depends="tcc make linux-headers bash coreutils sed grep gawk binutils openssl libuuid"
