...
pkg_version="2.6.14-4"
pkg_changelog="2.6.14-4: rebuilt against tcc 0.9.28rc (ADR-0223). The 2017 0.9.27 release could give two simultaneously-live locals the same stack slot (#216), a fault that corrupts values silently wherever the aliased pair is only read and written, so every binary it produced is suspect rather than merely the ones that failed. No source change: the revision exists to make the rebuild real, because an image version is a hash of the package manifest (ADR-0155) and a same-version reinstall is deduped and discarded. 2.6.14-3: declare build tools incl. findutils, which openldap's shtool needs (#206); keep the pkg-config file (#174)"
...
# recipe's own pkg_build() demonstrably needs: ./configure with --with-tls=openssl (so openssl's headers and library are a
# BUILD-time need, not merely a runtime one), `make depend` and per-directory
# make, and tcc as the pinned compiler. libuuid is linked the same way.
...
# Iterated once against a real build: the first attempt died in
# liblunicode with `/bin/sh: line 3: find: command not found`, which
# openldap's own shtool mkln depends on. findutils is a real build
# tool here, not a guess -- the error named it.
pkg_build_depends="tcc make linux-headers bash coreutils sed grep gawk binutils findutils openssl libuuid"
