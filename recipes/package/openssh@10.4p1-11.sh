...
pkg_artifact_sha256="00ed99fd65732958399ff49c53f2a9431412e801122d938580997e814312c243"
pkg_depends="openssl zlib linux-pam libxcrypt"
#
...
#
pkg_build_depends="tcc make linux-headers bash coreutils sed grep gawk binutils findutils diffutils pkgconf openssl zlib linux-pam libxcrypt"
pkg_changelog="10.4p1-11: rebuilt against tcc 0.9.28rc (ADR-0223). The 2017 0.9.27 release could give two simultaneously-live locals the same stack slot (#216), a fault that corrupts values silently wherever the aliased pair is only read and written, so every binary it produced is suspect rather than merely the ones that failed. No source change: the revision exists to make the rebuild real, because an image version is a hash of the package manifest (ADR-0155) and a same-version reinstall is deduped and discarded. 10.4p1-10: declares libxcrypt instead of copying the build host's libcrypt.so.1 into the package -- a foreign binary, and the wrong soname besides, since Cix's libxcrypt provides libcrypt.so.2. 10.4p1-9: declares its build tools so it can be rebuilt through the ordinary install path (#206)"
...
pkg_install() {
	...
	#
	# libcrypt is NOT copied in any more; it is declared. The old copy
	# took /lib/x86_64-linux-gnu/libcrypt.so.1 straight off the build
	# host -- a foreign binary inside a Cix package, which the Build
	# Provenance Mandate forbids. It was also the wrong library by
	# then: Cix's own libxcrypt provides libcrypt.so.2, and .so.1 only
	# existed because the build host happened to carry Debian's.
	#
	# So the copy could not have kept working regardless, which is the
	# useful part -- it failed loudly (cp: cannot stat) the moment this
	# recipe was built in a composed environment that contains only
	# what it declares.
	#
}
