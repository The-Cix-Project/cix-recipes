...
#
# Revision 4 then found the most consequential thing so far:
#
#   make: cc: No such file or directory
#   error[CPDL-E4001]: process exited with status 2; expected 0
#
# The recipe declares `requires build { compiler "tcc" }` and CBS did
# not put it in the environment, so make fell through to its built-in
# default `cc`, which this composed build environment does not have.
# ADR-0001's own TCC-always clause -- "CBS must not discover or fall
# back to an ambient GCC, Clang, or another host compiler. CPDL must
# not provide a mechanism for recipes to waive this rule" -- is not
# implemented: the declaration is metadata, reported in explain --json
# and otherwise inert. Had a gcc named `cc` been present, this build
# would have SUCCEEDED with the wrong compiler and said nothing, which
# is #109 on the Cix side exactly.
#
# The other bundled CBS recipes already know this. cix.cbs, gcc.cbs
# and tcc.cbs each set `env "CC" = "tcc"` by hand -- the same manual
# pin 83 of Cix's 134 shell recipes do. Only zstd.cbs trusted the
# declaration.
#
# So this revision adds the explicit env binding, which is what a CPDL
# recipe actually has to do, and asks the remaining question: with the
# compiler pinned the way real recipes pin it, does the rest of the
# pipeline work -- build, check, install, manifest, package, verify,
# extract, and the same build twice compared byte for byte?
#
