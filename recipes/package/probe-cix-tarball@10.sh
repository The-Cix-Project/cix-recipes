...
pkg_changelog="10: the cix-recipes archive at the commit cix-tests pins, so its fixture can verify floor artifacts against their recipes. 1: learns a gitea archive's sha256 from the box's own fetch log rather than downloading it in the dev sandbox, which the sandbox rule forbids."
