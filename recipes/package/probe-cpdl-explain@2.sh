...
pkg_changelog="2: fixes the glob (match recipes/ at the top or one level down) and the argument order (cbs explain FILE --json). 1: runs cbs explain --json over every published .cbs to name the recipe the engine-change sweep failed on."
...
pkg_build() {
	...
	bad=0
	ls
	for f in recipes/package/*.cbs */recipes/package/*.cbs; do
		[ -f "$f" ] || continue
		total=$((total + 1))
		if ! cbs explain "$f" --json > /dev/null 2> /run/explain.err; then
			bad=$((bad + 1))
			echo "REJECTED: $f"
			cat /run/explain.err
		fi
