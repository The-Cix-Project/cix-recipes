...
pkg_depends=""
#
# What this probe runs (#206). It is a diagnostic that deliberately exits
# non-zero, but it still has to START, and ADR-0199 refuses a build
# environment for a recipe that declares no tools -- so until now the one
# recipe whose entire job is to inspect a build environment could not get
# one.
#
pkg_build_depends="gcc coreutils"
pkg_toolchain="gcc"
...
pkg_changelog="11-3: declares gcc and coreutils, which it was already using -- a probe whose job is to inspect a build environment could not be given one (#206). 11-2: declare pkg_toolchain=gcc and its reason (#222, ADR-0226)"
