...
pkg_changelog="2: ends in a deliberate exit 1, because version 1 exited 0 and its output was never retained -- only a failing build's stdout is captured. 1: re-measures whether gcc's include-fixed/syslimits.h is still a copy of limits.h, which elfutils@0.192-12 works around by rewriting that file -- a write CPDL cannot make, since it is outside the confined build roots. Not re-measured since gcc moved to 16.2.0."
