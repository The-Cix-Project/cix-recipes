...
#
# Revision 3 answers the two questions revision 2 raised, both of which
# decide whether the fix is safe rather than merely correct:
#
#   [3] a port with NOTHING listening also returned -1 EINPROGRESS, not
#       ECONNREFUSED. So connect() alone cannot tell "refused" from
#       "connecting", and an inline poll(0) must not mistake one for
#       ready. What does poll/SO_ERROR say for that fd, immediately and
#       after a moment?
#   [6] production never probes loopback. cixinit_table.c sets
#       ready_addr_be to the CONTAINER'S OWN IP, so jump's probe dials
#       192.168.150.109:22. A local address is delivered locally, so it
#       should behave like loopback -- but that is an inference, and the
#       fix's shape depends on it.
#
...
pkg_changelog="3: measures the two things revision 2 raised -- what poll/SO_ERROR say for a port with nothing listening (the false-positive risk, since that also returned EINPROGRESS rather than ECONNREFUSED), and the production case, a connect to the container own non-loopback address. 2: same as 1, whose own C file defined _GNU_SOURCE while the compile line already did (tcc: \"_GNU_SOURCE redefined\" under -Werror). 1: measures whether a non-blocking TCP connect to a listening local peer returns 0 or EINPROGRESS, which decides whether cix-init's tcp_port readiness probe can ever report ready (#413), and whether an inline poll(0) or a carried fd is the fix."
...
int main(void)
{
	...
	if (rc != 0 && fd >= 0) {
		...
		rc = poll(&p, 1, 0);
		getsockopt(fd, SOL_SOCKET, SO_ERROR, &soerr, &slen);
		printf("    immediately: poll rc=%d revents=0x%x SO_ERROR=%d (%s)\n", rc,
		        (unsigned)p.revents, soerr, soerr == 0 ? "no error yet" : strerror(soerr));
		printf("    an inline poll(0) would report %s\n",
		        (rc > 0 && (p.revents & POLLOUT) && soerr == 0) ? "READY -- A FALSE POSITIVE"
		                                                        : "not ready (correct)");
		usleep(50000);
		p.fd = fd;
		p.events = POLLOUT;
		...
		slen = sizeof(soerr);
		getsockopt(fd, SOL_SOCKET, SO_ERROR, &soerr, &slen);
		printf("    after 50ms:  poll rc=%d revents=0x%x SO_ERROR=%d (%s)\n", rc,
		        (unsigned)p.revents, soerr, soerr == 0 ? "no error" : strerror(soerr));
	}
	if (fd >= 0)
		close(fd);
	...
	printf("\n[6] the production case: the container's OWN non-loopback address\n");
	{
		...
		if (getifaddrs(&ifa) == 0) {
			for (it = ifa; it != NULL; it = it->ifa_next) {
				...
				if (sa->sin_addr.s_addr == htonl(INADDR_LOOPBACK))
					continue;
				own = sa->sin_addr;
				printf("    interface %s carries %s\n", it->ifa_name, inet_ntoa(own));
				found = 1;
				break;
			}
			freeifaddrs(ifa);
		...
		if (!found) {
			printf("    this build container has no non-loopback IPv4 address --\n");
			printf("    the production address case is NOT measured here\n");
		} else {
			struct sockaddr_in a;
			socklen_t alen = sizeof(a);
			int ofd = socket(AF_INET, SOCK_STREAM | SOCK_CLOEXEC, 0);
			int oport, one = 1;
			...
			a.sin_addr = own;
			a.sin_port = 0;
			if (bind(ofd, (struct sockaddr *)&a, sizeof(a)) != 0 || listen(ofd, 8) != 0 ||
			    getsockname(ofd, (struct sockaddr *)&a, &alen) != 0) {
				printf("    could not listen on %s: %s\n", inet_ntoa(own), strerror(errno));
			} else {
				struct pollfd p;
				int soerr = -1;
				socklen_t slen = sizeof(soerr);

				oport = ntohs(a.sin_port);
				printf("    listening on %s:%d\n", inet_ntoa(own), oport);
				fd = socket(AF_INET, SOCK_STREAM | SOCK_NONBLOCK | SOCK_CLOEXEC, 0);
				memset(&a, 0, sizeof(a));
				a.sin_family = AF_INET;
				a.sin_addr = own;
				a.sin_port = htons((unsigned short)oport);
				rc = connect(fd, (struct sockaddr *)&a, sizeof(a));
				printf("    connect rc=%d errno=%d (%s)\n", rc, rc == 0 ? 0 : errno,
				        rc == 0 ? "none" : strerror(errno));
				p.fd = fd;
				...
				getsockopt(fd, SOL_SOCKET, SO_ERROR, &soerr, &slen);
				printf("    immediately: poll rc=%d revents=0x%x SO_ERROR=%d (%s)\n", rc,
				        (unsigned)p.revents, soerr, soerr == 0 ? "connected" : strerror(soerr));
				printf("    => %s\n",
				        (rc > 0 && (p.revents & POLLOUT) && soerr == 0)
				                ? "an inline poll(0) is enough for the production address too"
				                : "NOT finished at timeout 0: the fd must be carried across turns");
				close(fd);
			}
			close(ofd);
