...
pkg_build() {
	...
	which tcc || echo "  tcc not in PATH"
	tcc -v 2>&1 | head -2
	...
	for kind in "static inline" "static" "inline" "extern inline"; do
		...
		tcc -c p.c -o p.o 2>/dev/null
		tcc -c q.c -o q.o 2>/dev/null
		sym=$(nm p.o 2>/dev/null | grep " helper$" | awk '{print $2}')
		out=$(tcc -o prog p.o q.o 2>&1)
		if [ -n "$out" ]; then
			echo "  '$kind': symbol=${sym:-?}  LINK FAILED -- $(printf '%s' "$out" | head -1)"
		else
			./prog && r=ran-ok || r=ran-WRONG
	...
	out=$(tcc -Wp,-DFOO -o w1 w.c 2>&1)
	[ -n "$out" ] && echo "  -Wp,-DFOO           FAILS -- $out" || { printf '  -Wp,-DFOO           ok -- '; ./w1; }
	out=$(tcc -Wp,-MMD,dep.d,-MT,w.o -c w.c -o w.o 2>&1)
	[ -n "$out" ] && echo "  -Wp,-MMD,a,-MT,b    FAILS -- $out" || echo "  -Wp,-MMD,a,-MT,b    accepted"
	printf 'int main(void){ return 0; }\n' > t.c
	out=$(tcc -pthread t.c -o t1 2>&1)
	...
	for h in stdio.h fcntl.h unistd.h sys/stat.h dirent.h glob.h ftw.h stdlib.h; do
		printf '#include <%s>\nint main(void){return 0;}\n' "$h" > hh.c
		out=$(tcc -D_GNU_SOURCE -D_FILE_OFFSET_BITS=64 -c hh.c -o hh.o 2>&1)
