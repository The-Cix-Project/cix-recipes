...
EOF
	...
	echo
	echo "=== 6. C11 SUPPORT (blocks libuv and screen) ==="
	echo "  __STDC_VERSION__ as tcc reports it:"
	printf '#include <stdio.h>\nint main(void){\n#ifdef __STDC_VERSION__\nprintf("    %%ld\\n",(long)__STDC_VERSION__);\n#else\nprintf("    undefined\\n");\n#endif\nreturn 0;}\n' > v.c
	tcc -o vv v.c 2>/dev/null && ./vv || echo "    could not build the probe"

	for h in stdatomic.h threads.h stdalign.h stdnoreturn.h uchar.h; do
		printf '#include <%s>\nint main(void){return 0;}\n' "$h" > h11.c
		out=$(tcc -c h11.c -o h11.o 2>&1) || true
		[ -n "$out" ] && echo "  MISSING  <$h>" || echo "  ok       <$h>"
	done

	echo "  C11 _Atomic keyword:"
	printf 'int main(void){ _Atomic int x = 0; return x; }\n' > at.c
	out=$(tcc -c at.c -o at.o 2>&1) || true
	[ -n "$out" ] && echo "    MISSING -- $(printf '%s' "$out" | head -1)" || echo "    ok"

	echo "  C11 _Static_assert:"
	printf 'int main(void){ _Static_assert(1, "ok"); return 0; }\n' > sa.c
	out=$(tcc -c sa.c -o sa.o 2>&1) || true
	[ -n "$out" ] && echo "    MISSING -- $(printf '%s' "$out" | head -1)" || echo "    ok"
