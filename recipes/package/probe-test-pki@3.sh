...
pkg_changelog="3: probe 2 never ran -- its own changelog used backticks inside a double-quoted assignment, which the shell executed (line 26: off: command not found). Fourth time in this repo a probe has been stopped by its own recipe rather than by what it was measuring. 2: re-run after fixing what probe 1 found -- an uninitialised offset in pki_cert_create() that sent uninitialised stack to openssl as -addext, so cert issuance failed by stack contents. What still fails here is what decides how test_pki joins the gate (#417). 1: read test_pki own failure in a build container, rather than inferring it from #224 tally (#417)"
