...
pkg_changelog="2: ends deliberately non-zero. Revision 1 measured everything correctly and then threw the answers away: a build container stdout only reaches the log store when the build FAILS, and build-log is live-only, so a probe that succeeds is a probe whose result nobody can read. Failing on purpose is how a probe reports here."
...
pkg_build() {
	...
	#
	# Fail on purpose. A build container's stdout only reaches the log
	# store when the build fails, and pkg build-log is live-only, so a
	# probe that exits 0 is a probe whose measurements nobody can read.
	# Revision 1 measured all of this correctly and lost every answer.
	#
	echo "probe-tls: failing on purpose so the measurements above are logged"
	exit 1
}
