#
# 3.13.5-7: declare zlib as a RUNTIME dependency (#455 class). The
# binascii extension -- a core module, not one of the optional ones
# this recipe leaves unbuilt -- links libz.so.1 (crc32), so a python
# with pkg_depends="" is refused by the elfcheck gate (ADR-0251) on
# install into any image: "binascii...so links libz.so.1, which nothing
# it declares provides". zlib is packaged and tiny; declaring it is the
# correct fix, the same distinction pkg_build_depends (build sandbox)
# vs pkg_depends (installed runtime) that git 2.55.0-8 needed.
#
#
...
#
#
# 3.13.5-8: also build the _bz2 extension (bzip2 added to build+runtime
# deps). node's configure.py imports bz2, so a python without _bz2 cannot
# build node; bzip2 1.0.8-5 now ships bzlib.h to build against.
#
pkg_name="python"
pkg_version="3.13.5-8"
...
pkg_toolchain="gcc"
pkg_toolchain_reason="missing language feature: CPython's pyatomic.h finds no usable atomics: TCC implements C11 <stdatomic.h> but reports __STDC_VERSION__ as 199901, so the version gate rejects it (#235)"
pkg_build_depends="bash coreutils make gcc binutils linux-headers sed grep gawk pkgconf findutils m4 zlib bzip2"
pkg_changelog="3.13.5-6: declare pkg_toolchain=gcc and its reason (#222, ADR-0226)"
