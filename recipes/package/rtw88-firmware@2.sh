#
# 2: the header gate used `grep`, which this recipe never declared, so
# it failed on a tool that was not there rather than on the firmware:
#
#   fetched: rtw8822b_fw.bin (150984 bytes)
#   /build/recipe.sh: line 72: grep: command not found
#   rtw88-firmware: not an 8822B firmware image -- header is not 22 88
#    22 88 00 00 1b 00 02 00 ff ff ff ff 00 00 00 00
#
# The blob was correct the whole time -- the size is exact and the
# header the gate printed while rejecting it IS 22 88. Only the check
# was broken, and it failed in the worst direction: a gate that reports
# the thing it guards as bad when the gate itself is what is missing.
#
# Rewritten with `case` against the same od output, which is the
# precedent openssl.recipe already set for exactly this reason -- it
# verifies "with `case` against cat's output rather than grep so that
# verifying it costs no extra declared build tool". Adding grep to
# pkg_build_depends would also have worked and is the wrong fix: a
# gate should not enlarge the sandbox it runs in.
#
#
...
pkg_changelog="2: the header gate called grep, which this recipe does not declare, so it failed on the missing tool and reported the firmware as bad -- while printing a header that was correct. Rewritten with case against the same od output, the way openssl.recipe already verifies its own install, so the check costs no extra declared build tool. The blob itself is unchanged and was never wrong. 1: the RTL8822BU firmware blob (#30)."
...
pkg_build() {
	...
	hdr=$(head -c 2 "$src" | od -An -tx1 | tr -d ' \n')
	case "$hdr" in
	2288) ;;
	*)
		echo "rtw88-firmware: not an 8822B firmware image -- header is $hdr, not 2288" >&2
		head -c 16 "$src" | od -An -tx1 >&2
		exit 1
		;;
	esac
	echo "header ok: 8822B firmware image (first two bytes $hdr)"
