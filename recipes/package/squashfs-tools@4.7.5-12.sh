...
pkg_changelog="4.7.5-12: install to /usr/lib rather than the multiarch directory (#184 stage 2). The multiarch triplet is a Debian convention for letting several architectures share one filesystem and a Cix image has one architecture. The libgcc_s source is now searched rather than hardcoded, because that library lives in the build sandbox and gcc moves in a later stage. The artifact approval is dropped because those bytes came from the previous revision. 4.7.5-11: rebuild against tcc 0.9.28rc-10, which emits PT_GNU_STACK (#228). The artifact approval is dropped because those bytes came from rc-9."
...
pkg_install() {
	...
	# directories (#184) -- gcc moves in a later stage, and a recipe
	# naming one fixed directory would break on the day it does.
	libgcc=""
	for d in /usr/lib /lib/x86_64-linux-gnu /usr/lib/x86_64-linux-gnu; do
		if [ -e "$d/libgcc_s.so.1" ]; then libgcc="$d/libgcc_s.so.1"; break; fi
	done
	[ -n "$libgcc" ] || {
	...
	cp -a "$libgcc" "$PKG_DESTDIR/usr/lib/"
