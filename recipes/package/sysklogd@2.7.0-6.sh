...
pkg_name="sysklogd"
pkg_version="2.7.0-6"
pkg_changelog="2.7.0-6: declare diffutils and pkgconf. Its configure reaches for cmp and pkg-config, neither was declared, and both were reported by the #302 missing-command gate the first time this package was rebuilt from source."
...
#
# 2.7.0-6: diffutils and pkgconf added, both named by the #302 gate the
# first time this was actually rebuilt from source (restoring the fleet
# after the btrfs migration -- there was no artifact to install from).
# Exactly the outcome the paragraph above predicts.
#
#   ./configure: line  7022: cmp: command not found
#   ./configure: line 13959: --variable=systemdsystemunitdir: command not found
#
# The first is diffutils (its own header: "GNU diff/cmp/diff3/sdiff"),
# and configure answers a "working dd" probe from it. The second reads
# like a stray argument and is one: $PKG_CONFIG expanded to nothing, so
# the shell tried to run the flag. pkgconf ships usr/bin/pkg-config for
# exactly this. Both tools are genuinely reached for and genuinely
# absent, so both are declared rather than suppressed -- and the gate
# refuses on ANY occurrence of the phrase, not on a count, so silencing
