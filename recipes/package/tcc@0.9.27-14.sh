...
INITLIT_EOF
	...
	# correctness: this package is its own build dependency, so the
	# binary it produces was shaped by a predecessor whose bugs are
	# exactly what these gates exist to catch. #211 made that concrete
	# -- 0.9.27-12 was compiled by 0.9.27-10, which had the bug being
	# fixed.
	#
	# The answer is the one GCC uses and gcc.recipe already follows:
	# build again with the compiler just built (stage 2 -> stage 3) and
	# require the two to be byte-identical. If they are, the compiler
	# reproduces itself, and the seed cannot have influenced the result
	# -- any difference the seed introduced would have to survive being
	# recompiled by a compiler that does not contain it.
	#
	# This is strictly stronger than cross-checking against gcc, which
	# only shows two compilers agree and drags gcc's own behaviour into
	# the answer. It also stays inside the TCC-by-default policy rather
	# than widening the Tier-3 exception list.
	#
	# Byte comparison is meaningful here: tcc embeds no build
	# timestamp. Its __DATE__/__TIME__ handling implements those macros
	# for programs it compiles, it does not stamp itself, and a
	# same-compiler rebuild was confirmed byte-identical.
	#
	stage_snapshot() {
		rm -rf "$1"
		mkdir -p "$1"
	...
	# recipe does not declare, and -13 failed for exactly that reason.
	# It failed CLOSED -- unable to compare was treated as "differ" --
	# which is the right way round for a gate, but it is still a gate
	# failing on its own missing tool rather than on the thing it
	# checks. coreutils is already declared and gives a printable
	# digest for the log besides.
	...
	# Re-run both gates against stage 3, since stage 3 is what ships.
	# They are identical binaries, so this cannot fail on its own -- it
	# fails only if the comparison above was somehow wrong, which is
	# worth one second to rule out.
	./tcc -B. dowhile_check.c -o dowhile_check3
	./dowhile_check3 || {
		echo "tcc: stage 3 fails the do-while gate (issue #122)" >&2
		exit 1
	}
	./tcc -B. initlit_check.c -o initlit_check3
	./initlit_check3 || {
		echo "tcc: stage 3 fails the initializer gate (issue #211)" >&2
		exit 1
	}
}
