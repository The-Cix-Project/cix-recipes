...
ATOMIC_EOF
	...
	# Build it into libtcc1.a alongside the runtime pieces already
	# there. Purely additive: no existing object or symbol changes, so
	# this cannot alter how anything already built behaves.
	sed -i 's|^X86_64_O = libtcc1.o alloca86_64.o alloca86_64-bt.o$|X86_64_O = libtcc1.o alloca86_64.o alloca86_64-bt.o atomic.o|' \
	    lib/Makefile
	grep -q 'atomic\.o' lib/Makefile || { echo "tcc: failed to add atomic.o to libtcc1.a" >&2; exit 1; }
