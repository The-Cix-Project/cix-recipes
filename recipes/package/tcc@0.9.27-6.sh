...
#undef CONFIG_TCC_MALLOC_HOOKS\
	...
	grep -A7 'skip(TOK_WHILE);' tccgen.c | grep -q 'nocode_wanted = saved_nocode_wanted;' || {
		echo "tcc: do-while patch did not land where expected -- re-check tccgen.c" >&2
		exit 1
	}
