...
pkg_artifact_sha256="563338d42210b0a9d14eba3c5ebd7924470aa9eef6406f5b7567d6893f61528d"
pkg_depends=""
#
...
ATOMIC_EOF
	...
	#
	# __attribute__((constructor (N))) / ((destructor (N))) -- issue
	# #219, the only regression the 0.9.28rc upgrade caused in a
	# third-party package.
	#
	# The attribute parser knows `constructor` and does not read its
	# optional priority, so the '(' after it is a syntax error:
	#
	#   cap_alloc.c:23: error: ')' expected (got '(')
	#
	# Measured against the shipped compiler by
	# probe-tcc-conformance/19: bare `constructor` works, `(300)`
	# fails to compile, and so does an ordering test.
	#
	# Parsed AND HONOURED, not parsed and ignored. Accepting a
	# priority and then running constructors in some other order is
	# the silently-wrong outcome this project keeps finding; GCC's
	# rule is that a priority puts the entry in .init_array.NNNNN,
	# which the linker sorts before plain .init_array. add_array()
	# needs no change for that -- it keys init-vs-fini on sec[1],
	# which a suffixed name preserves.
	#
	# The priority lives in FuncAttr's own spare bits (`xxxx : 15`),
	# so the struct does not grow. 15 bits caps it at 32767 where GCC
	# allows 65535; out of range is an ERROR rather than a silent
	# truncation, because a truncated priority is a wrong order that
	# nothing reports.
	#
	...
	for frag in 'func_ctorprio : 15;' 'constructor priority out of range' 'destructor priority out of range' '".init_array.%05u"' '".fini_array.%05u"'; do
		grep -qF "$frag" tcc.h tccgen.c || {
...
int main(void)
{
	 ...
	 * linker places after every .init_array.NNNNN section. */
	/* Reported rather than asserted this revision -- 0 means the
	 * expected GCC order, otherwise the digits say what ran when and
	 * a zero means that constructor never ran at all. */
...
CTOR_EOF
	...
	#
	# MEASURING, not asserting, for this revision only.
	#
	# rc-11 emitted .init_array.NNNNN -- which is how GCC expresses a
	# priority and how GNU ld sorts it -- and its gate failed. tcc
	# links internally, and there is no reason to assume its linker
	# merges those suffixed sections into .init_array at all. If it
	# does not, a prioritised constructor does not merely run in the
	# wrong order, it may not run AT ALL, which is worse than the
	# compile error this set out to fix.
	#
	# So this revision reports what actually happens and does not
	# decide on the strength of a guess. The gate returns to asserting
	# in the next revision, once the answer is known.
	#
	./ctorprio_check; echo "tcc: #219 ctor order probe -> exit $?"
	./tcc -B. -c ctorprio_check.c -o ctorprio_check.o
	echo "tcc: #219 sections emitted:"
	readelf -SW ctorprio_check.o 2>/dev/null | grep -i "init_array\|fini_array" | sed 's/^/  /' || true
	echo "tcc: #219 linked binary sections:"
	readelf -SW ctorprio_check 2>/dev/null | grep -i "init_array" | sed 's/^/  /' || true
