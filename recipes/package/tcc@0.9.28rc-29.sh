...
pkg_build() {
	...
	# spaces are included into the executable.
	#
	# The MACROS are appended after the file's final #endif so they are
	# unconditional rather than scoped to non-preprocessor mode. That is
	# right for macros, which the preprocessor consumes and never emits.
	#
	# The DECLARATIONS are not appended, and that is the whole of #220
	# (rc-28). Appended there they land outside the #ifndef __TCC_PP__
	# region, and tccdefs.h is injected as a command-line prelude, so
	# every -E output carried three lines of C:
	#
	#   # 129 "<command line>"
	...
	#
	# gettext's configure captures preprocessed output into CPPFLAGS,
	# so those three lines ended up on a shell command line and
	# libtextstyle died with "syntax error near unexpected token ("
	# before the libcroco merge below could even run.
	#
	# They go beside upstream's own builtin declarations instead --
	# immediately before its #undef __BUILTIN_EXTERN -- because that
	# region does not leak. Measured on the pinned source rather than
	# reasoned about: appended at the end, -E emits 3 declarations;
	# placed here, it emits 0, exactly as upstream's own __builtin_ffs
	# family does. The declaration is still in scope when compiling,
	# which is the point of having it at all (rc-16: without one the
	# implicit int rule truncates bswap64's 64-bit result).
	#
	sed -i 's|^    #undef __BUILTIN_EXTERN$|    unsigned short __builtin_bswap16(unsigned short);\n    unsigned int __builtin_bswap32(unsigned int);\n    unsigned long long __builtin_bswap64(unsigned long long);\n    #undef __BUILTIN_EXTERN|' include/tccdefs.h
	grep -q '^    unsigned long long __builtin_bswap64(unsigned long long);$' include/tccdefs.h || {
		echo "tcc: the bswap declarations did not land beside upstream's builtins (#220)" >&2
		exit 1
	}
...
CIX_INITARRAY_EOF
	...
	#
	# 3. The priority never survived attribute merging, which is why
	#    rc-20 through rc-26 all failed the gate at 121 -- declaration
	#    order -- with the parser and the merge both already correct.
	#
	#    merge_funcattr() copies FuncAttr field by field rather than
	#    wholesale, and it has a line for func_ctor and func_dtor but
	#    none for the new priority. So the constructor ran, which made
	#    it look like the attribute had been honoured, and the number
	#    was dropped before the emitter could read it: psec was always
	#    the plain .init_array and there was nothing for the merge to
	#    sort.
	#
	#    Measured against this exact pinned source rather than
	#    inferred. Without this line the object carries only
	#    .init_array; with it, .init_array.00100 and .init_array.00900
	#    appear and the gate below exits 0.
	#
	#    Done with sed, not python: this recipe's own
	#    pkg_build_depends is tcc make linux-headers bash coreutils
	#    sed grep gawk binutils, and a python heredoc here would
	#    have failed in the build container for want of an
	#    interpreter -- one more wasted cycle on a seven-cycle bug.
	#
	sed -i 's|^      fa->func_dtor = 1;$|      fa->func_dtor = 1;\n    if (fa1->func_ctorprio)\n      fa->func_ctorprio = fa1->func_ctorprio;|' tccgen.c
	...
	for frag in 'func_ctorprio : 15;' 'constructor priority out of range' '".init_array.%05u"' 'cix_merge_prio_arrays(s1, ".init_array");' '#include "cix_initarray.inc"' 'fa->func_ctorprio = fa1->func_ctorprio;'; do
	...
	#
	# _Static_assert accepts a 64-bit constant expression (#220).
	#
	# do_Static_assert() calls expr_const(), which is the 32-bit
	# wrapper: it evaluates with expr_const64() and then raises
	# "constant exceeds 32 bit" if the result does not fit an int or
	# an unsigned. That diagnostic is right for an array size or a
	# bit-field width. It is wrong here -- C requires only an integer
	# constant expression, with no width limit at all.
	#
	# gnulib writes assertions that exceed it as a matter of course.
	# gettext's gnulib-tests/nanosleep.c line 60 is
	#
	#   static_assert (TYPE_MAXIMUM (time_t) / 24 / 24 / 60 / 60);
	#
	# which is 4447999631970 on a 64-bit time_t, and it stopped the
	# build after every one of gettext's four tools had linked.
	#
	# Two lines, and the range address confines them to this one
	# function: "int c;" appears in others.
	#
	sed -i '/^static void do_Static_assert(void)$/,/^}$/ { s/^    int c;$/    int64_t c;/; s/^    c = expr_const();$/    c = expr_const64();/ }' tccgen.c
	if ! grep -qF 'c = expr_const64();' tccgen.c; then
		echo "tcc: the _Static_assert 64-bit patch did not apply (#220)" >&2
		sed -n '/^static void do_Static_assert(void)$/,/^}$/p' tccgen.c >&2
		exit 1
	fi
	echo "tcc: _Static_assert 64-bit patch applied"
...
C11_EOF
	...
	# #220 gate: a static assertion must accept a 64-bit constant
	# expression AND must still reject a false one. Widening the
	# evaluation is only correct if it did not also stop the
	# assertion from asserting, and the second file is the half that
	# proves it -- a gate that only checks acceptance would pass a
	# compiler that accepts everything.
	cat > sa64_check.c <<'SA64_EOF'
...
int main(void) { return 0; }
SA64_EOF
	cat > sa64_false.c <<'SA64F_EOF'
_Static_assert(1LL << 32 >> 32 >> 1, "this one is false and must be rejected");
int main(void) { return 0; }
SA64F_EOF
	if ! ./tcc -B. sa64_check.c -o sa64_check; then
		echo "tcc: _Static_assert rejects a 64-bit constant expression (#220)" >&2
		exit 1
	fi
	if ./tcc -B. sa64_false.c -o sa64_false 2>/dev/null; then
		echo "tcc: _Static_assert accepted a FALSE assertion -- widening broke the assert (#220)" >&2
		exit 1
	fi
	echo "tcc: _Static_assert handles 64-bit constant expressions (#220)"
...
CTOR_EOF
	...
	#
	# Guarded. An unguarded invocation is not a gate: the recipe runs
	# under set -e, so a failing check kills the script on that line
	# and the whole diagnostic block below is unreachable exactly when
	# it is needed. That is why every build since rc-20 reported only
	# exit status 121 with a log ending several gates earlier, and why
	# rc-21, rc-23 and rc-24 each tried to fix the reporting without
	# being able to.
	#
	ctorprio_rc=0
	./ctorprio_check || ctorprio_rc=$?
	if [ "$ctorprio_rc" -ne 0 ]; then
		...
		echo "tcc: which symbol each .init_array slot resolves to:" >&2
		readelf -xinit_array ctorprio_check 2>/dev/null | sed 's/^/  /' >&2 || true
		echo "tcc: addresses of the three constructors:" >&2
		readelf -sW ctorprio_check 2>/dev/null | grep -E ' (a|b|c)$' | sed 's/^/  /' >&2 || true
		echo "tcc: dynamic tags describing the array:" >&2
		readelf -dW ctorprio_check 2>/dev/null | grep -i 'init_array' | sed 's/^/  /' >&2 || true
		exit 1
	fi
...
SLOT_EOF
	...
	if ! ./tcc -B. sa64_check.c -o sa64_check3; then
		echo "tcc: stage 3 rejects a 64-bit _Static_assert (#220)" >&2
		exit 1
	fi
	if ./tcc -B. sa64_false.c -o sa64_false3 2>/dev/null; then
		echo "tcc: stage 3 accepts a FALSE _Static_assert (#220)" >&2
		exit 1
	fi
