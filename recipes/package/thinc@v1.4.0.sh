...
pkg_name="thinc"
pkg_version="v1.4.0"
pkg_source="https://osakka:REPLACE_WITH_REAL_TOKEN@git.home.arpa/api/v1/repos/itdlabs/thinc/archive/v1.4.0.tar.gz"
...
pkg_build() {
	make build/thincd build/thincctl build/mkbootroot build/thinc-install build/mkinstalleriso
}

pkg_install() {
	cp build/thincd build/thincctl build/mkbootroot build/thinc-install build/mkinstalleriso \
	   "$PKG_DESTDIR/"
