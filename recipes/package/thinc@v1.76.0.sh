...
#
# Re-pinned again to v1.73.0: the real fix for issue #34 -- v1.72.0's
# own new mountns_pivot() per-step diagnostics pinpointed the exact
# failing syscall (mkdir(/dev/pts): Invalid cross-device link), which
# traced to a real, documented kernel overlayfs behavior: creating a
# new entry under a directory that exists only in lowerdir needs an
# internal copy-up-then-rename to bring the parent into upperdir
# first, and without redirect_dir support that rename fails outright
# with EXDEV. overlay_create() now mounts with redirect_dir=on
# explicitly. sda and dev-image staleness were both real dead ends,
# ruled out by direct testing before this was found.
#
# Re-pinned again to v1.74.0: v1.73.0's own redirect_dir=on turned out
# not to be the real fix -- confirmed live via an A/B test (identical
# diagnostic container, only disk_quota_bytes present or absent) that
# project-quota tagging was the actual trigger. overlay_create() tagged
# upperdir with the container's project id (ADR-0062) but never
# workdir -- overlayfs's own kernel copy-up mechanism creates a new
# object in workdir first, then renames it into upperdir, and an
# untagged-workdir-into-tagged-upperdir rename is exactly what ext4's
# project-quota implementation refuses with EXDEV. Now tags both. This
# is the real, complete fix for issue #34 -- it hits essentially every
# new production container, since a disk quota is required by default.
#
# Re-pinned again to v1.75.0: v1.74.0's own workdir project-quota fix
# confirmed live to close the real EXDEV -- but v1.73.0's earlier
# redirect_dir=on addition, left in place, turned out to actively
# break lowerdir content visibility on this project's own from-scratch
# kernel (never built with CONFIG_OVERLAY_FS_REDIRECT_DIR=y): execve()
# ENOENT for every binary tried, confirmed live, not just one. Reverted
# here. The workdir project-quota fix alone is the real, complete fix
# for issue #34.
#
# Re-pinned again to v1.76.0 (ADR-0175, issue #35): keep_on_failure --
# a new opt-in field on POST /pkg/install, /pkg/hostbuild, and
# /system/kmod-build. On a build failure of the chain's own final job,
# the exited build container is left registered/mounted instead of
# torn down, readable afterward via the already-existing GET
# .../files (ADR-0055) and cleaned up via an ordinary DELETE -- no new
# teardown mechanism. Built specifically to debug issue #32's own gcc
# bootstrap crash for real, deployed here to actually do that.
#
...
pkg_name="thinc"
pkg_version="v1.76.0"
pkg_source="https://osakka:REPLACE_WITH_REAL_TOKEN@git.home.arpa/api/v1/repos/itdlabs/thinc/archive/v1.76.0.tar.gz"
pkg_sha256="f4d0db4247200eb61e44e63b637ca33493d3d50b8605f16e3608645b0cd5c183"
pkg_depends=""
