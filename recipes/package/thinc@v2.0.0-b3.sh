#
# TRANSITIONAL BRIDGE RECIPE -- exists for exactly one deploy, then is
# deleted from the box catalog.
#
# The Cix rename (ADR-0200) renamed this project's own self-package from
# "thinc" to "cix". The daemon triggers its own control-plane image
# assembly from a hook that matches the hostbuild by NAME: the code now
# says "cix", but a host still running the PRE-rename daemon says
# "thinc". So on such a host a `pkg hostbuild cix` builds every artifact
# correctly and then nothing assembles them -- the hook never fires,
# because the running daemon has never heard of that name.
#
# This recipe is byte-identical to cix/v2.0.0 except for its name. It
# builds the same v2.0.0 tag and produces the same cixd/cixctl/web/
# mkbootroot artifacts; the only thing the name changes is that the
# old daemon's hook recognises it and runs the assembly.
#
# After the deploy that uses it, the host runs the new daemon, whose
# hook matches "cix" -- so this recipe has no further purpose and is
# removed from the catalog. It is kept in the repository as the record
# of how the rename was bridged, not as something to run again.
#
#
...
pkg_build() {
	# Second half of the bridge (see the header). The control-plane
	# image mkbootroot builds stages the daemon at a HARDCODED
	# /bin/cixd, but the boot entry on a pre-rename host says
	# init=/bin/thincd -- an entry only the OLD daemon writes, and it
	# writes it on every update including this one. So an image
	# containing only /bin/cixd has no init to start under that entry.
	#
	# Rather than fight that, this build makes the image carry BOTH
	# names: the same binary staged twice, so it boots under either
	# entry. Once the new daemon is running it writes its own
	# init=/bin/cixd entries and the /bin/thincd copy becomes inert --
	# which is exactly why this belongs in a transitional recipe and
	# not in mkbootroot itself.
	grep -q 'test_image_fixture_build(image_root, cixd_bin, "cixd")' image/src/mkbootroot.c || {
		echo "bridge: mkbootroot's daemon-staging call has changed -- re-check this patch" >&2
		exit 1
	}
	sed -i 's|\tif (test_image_fixture_build(image_root, cixd_bin, "cixd") != 0)|\tif (test_image_fixture_build(image_root, cixd_bin, "thincd") != 0)\n\t\treturn 1;\n\tif (test_image_fixture_build(image_root, cixd_bin, "cixd") != 0)|' \
	    image/src/mkbootroot.c
	grep -q 'cixd_bin, "thincd"' image/src/mkbootroot.c || {
		echo "bridge: failed to add the /bin/thincd staging call" >&2
		exit 1
	}
...
pkg_install() {
	...
	# The whole point of this bridge (see the header): a PRE-rename
	# daemon assembles the control-plane image from hardcoded
	# <artifact_dir>/thincd and <artifact_dir>/thincctl paths. Those
	# names no longer exist in this build, so without these two copies
	# the assembly silently stages whatever STALE thincd was left in
	# the artifact directory by the last pre-rename hostbuild -- which
	# is exactly what happened on the first attempt: the box rebooted
	# onto a freshly-assembled image still running the old daemon.
	# Installing the new binaries under BOTH names makes the old
	# daemon's own assembly stage the new ones.
	cp build/cixd "$PKG_DESTDIR/thincd"
	cp build/cixctl "$PKG_DESTDIR/thincctl"
}
