...
# ar, ld, objdump and the rest link against libz for compressed-section
# support -- confirmed the hard way, not from reading configure output:
# `ar` in a build environment holding binutils and nothing else died
# with "error while loading shared libraries: libz.so.1". -2 declared
# nothing, which was true only because the shared build sandbox happened
# to have libz lying around (#110). (-3 declared this correctly but
# could not build itself: see the zlib line below.)
pkg_depends="zlib"
#
...
#             all. The second reason is normally the platform's to
#             solve -- a declared tool arrives with its own runtime
#             dependencies -- but that closure is read from what was
#             recorded when the tool was INSTALLED, and every binutils
#             installed anywhere predates -3 declaring zlib. So this
#             entry is what breaks the bootstrap cycle exactly once;
#             the first reason is why it stays afterwards.
#
# Sufficiency is enforced by the build itself. Minimality is review,
# not enforcement (ADR-0199).
pkg_build_depends="tcc make libc-dev bash coreutils sed grep gawk binutils zlib"
