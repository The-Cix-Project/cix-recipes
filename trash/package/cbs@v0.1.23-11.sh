...
#
pkg_name="cbs"
pkg_version="v0.1.23-11"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix-build-system/archive/0fcbd35b.tar.gz"
pkg_sha256="7f7e9f35363efbbabf7815eb9cc34e220881071ec43d0228d7d2c9289a70ef1b"
pkg_artifact_sha256="cc2f512b9af982fcb2c4ab6e3d4027380ab4bd6a6913115bbb6ba532b9081ff3"
...
pkg_build() {
	...
	echo "==================== make test"
	# TEMPORARY, and it must be removed the moment upstream fixes it.
	# cix-build-system#148 replaced the hardcoded /bin/ls in
	# tests/observe-test.c with ./cbs, and tests/seams-test.c carries
	# the identical assertion on line 52 and was missed. On a Cix build
	# image /bin holds exactly one file, sh, so seams-test returns 1
	# with no output and stops the suite one program from the end.
	#
	# Patched here rather than reported around, for one reason: with
	# it, the suite either passes completely or it does not, and that
	# is the only way to learn whether seams-test is the last blocker
	# or merely the next one. Anchored exactly, so it fails loudly if
	# upstream changes the line.
	sed -i 's|cbs_observe_dependencies(observe, "/bin/ls", NULL)|cbs_observe_dependencies(observe, "./cbs", NULL)|' tests/seams-test.c
	grep -n 'cbs_observe_dependencies(observe' tests/seams-test.c
