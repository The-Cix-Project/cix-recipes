...
#
pkg_name="cbs"
pkg_version="v0.1.24-4"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix-build-system/archive/d3f84ba5.tar.gz https://codeload.github.com/TinyCC/tinycc/tar.gz/2ba12e83b3599ca8f5d50c179fe5138fe956f0c9 https://github.com/facebook/zstd/archive/refs/tags/v1.5.4.tar.gz"
pkg_sha256="27769080299fa598d8631157eacdbdf46bfeaf5f98a6558fbb401788bb2c83b6 4eb5f0266d4d9deabe9650abedc3f0261dc06295f89ad798bb495abf680dc074 35ad983197f8f8eb0c963877bf8be50490a0b3df54b4edeb8399ba8a8b2f60a4"
...
pkg_changelog="v0.1.24-4: build commit d3f84ba5, which moves the tcc bootstrap out of the default test path so upstream-test builds only zstd. The upstream cache is still seeded here, which is what tests whether the caller-cache hook actually reaches the build. v0.1.24-3: seed CBS upstream cache from extra sources, so make test can run with no network. The -2 build failed at upstream-test with Could not resolve host -- test now depends on upstream-test, which fetches tcc and zstd unless a caller cache is supplied. cixd fetches and verifies both host-side into /build/extra and this recipe renames them to their sha256, which is the cache key CBS looks up. v0.1.24-2: build commit 8a984514, six commits past the v0.1.24 tag, which land bounded stdout assertions and bindings, a VERSION file, the public header split, and make test depending on upstream-test. That last one is the thing to measure: upstream-test builds tcc and zstd, and unseeded it fetches over the network, which a build container does not have. v0.1.24-1: build the v0.1.24 TAG rather than a bare commit. Every revision from -3 to -12 built an untagged commit and therefore carried a version string it did not contain; this is the first one whose name is true. Reports what a tarball build of the tag actually derives for CBS_VERSION, since the git-describe path is unavailable in a source tarball and the fallback reads cbs.cbs. v0.1.23-12: build commit 3e59cb97 with NO recipe-side workarounds at all. It fixes the last hardcoded /bin/ls, in tests/seams-test.c, so the sed the -11 revision carried is removed. This is the first revision whose only edit to CBS is the zstd pragma define, which is a TCC gap rather than a CBS one. v0.1.23-11: build commit 0fcbd35b, which replaces the python3 http.server in cli-build-test.sh with a 98-line C helper built by tcc. python is dropped from pkg_build_depends here so that the suite passing is itself the proof the dependency is gone. Temporarily patches the /bin/ls in tests/seams-test.c that the cix-build-system#148 fix missed, to establish whether it is the last blocker. v0.1.23-10: build commit 7aa33eb1, which fixes both remaining reasons the suite could not run on a Cix host -- the #!/bin/bash shebang in recipe-metadata-test.sh (cix-build-system#145) and observe-test.c hardcoding /bin/ls and /etc/hosts (cix-build-system#148, now using ./cbs and an mkstemp junk file). Both workarounds are removed and make test becomes a real gate rather than a report. v0.1.23-9: report what tests/observe-test.c assumes about the filesystem. The -8 build passed 20 of 21 test programs and observe-test failed with no output; it hardcodes /bin/ls and /etc/hosts. v0.1.23-8: append the zstd define to the Makefile rather than overriding CPPFLAGS on the make command line. Revisions -3..-7 discarded the Makefile own -DCBS_VERSION that way, so cbs --version reported unknown and 09fc6e9 version fix looked broken when it was not -- the recipe was at fault, not CBS. v0.1.23-7: build commit 09fc6e9, which adds a source-tarball version fallback for #146 -- CBS_VERSION now falls back to reading cbs.cbs when git describe finds nothing, and cli-build-test.sh pattern-matches instead of asserting a literal. The #145 shebang is still unfixed upstream, so the sed edit below is still required for the suite to run at all. v0.1.23-6: disable errexit around the diagnostics. The -5 build printed nothing from the traced run because pkg_build inherits set -e from cixd, so the failing script aborted the recipe before its own rc could be reported. v0.1.23-5: trace cli-build-test.sh before running the suite. The -4 build got past the shebang and then failed at tests/cli-build-test.sh with a bare Error 1 -- the script runs under set -eu and so exits silently at its first failing command. v0.1.23-4: point tests/recipe-metadata-test.sh at /usr/bin/bash. It is the only one of the four test scripts with a #!/bin/bash shebang, and this platform has no /bin/bash, so make test died at Error 127 naming a file that is present -- the kernel ENOENT is for the interpreter. The suite has therefore never run on a Cix host. v0.1.23-3: build commit 0a5313f6, which is ahead of the v0.1.23 tag and has no tag of its own -- the version string is therefore inaccurate and this should be replaced by a tagged revision. Adds make test as a reported gate; the suite grew by repro, fuzz, seams, typed-package, policy and observe tests that had never run under this platform tcc. v0.1.23-2: define ZSTD_DISABLE_DEPRECATE_WARNINGS. The -1 build failed at zstd.h:72 -- TCC defines neither __GNUC__ nor _MSC_VER, takes the header fallback arm, and that arm emits a #pragma message TCC does not implement, which -Werror makes fatal. CBS keeps -Werror. v0.1.23-1: first build of cix-build-system on a Cix host, closing cix-build-system#124. TCC, against the Cix-built libarchive 3.8.1-4 and zstd 1.5.7-3. Builds and validates a real recipe; not integrated with cixd, and cix-build-system#132 remains open."
...
pkg_build() {
	...
	# Seed CBS's upstream cache, so `make test` -- which now depends on
	# upstream-test -- can run with no network.
	#
	# The -2 build failed here, and it is the one finding this revision
	# exists to answer:
	#
	#   ./tests/upstream-smoke-test.sh ./cbs
	#   error[CPDL-E5001]: source `tcc` could not be fetched from
	#   https://codeload.github.com/TinyCC/tinycc/...; cause: Could not
	#   resolve host: codeload.github.com
	#   make: *** [Makefile:164: upstream-test] Error 3
	#
	# upstream-test builds recipes/tcc.cbs and recipes/zstd.cbs, and
	# with an empty cache it fetches them. A Cix build container has no
	# network by policy, so making it a hard prerequisite of `test`
	# turned "make test cannot run here" into "make test cannot pass
	# here" -- the same wall, one step further along.
	#
	# The hook to fix it already exists: upstream-smoke-test.sh takes a
	# caller cache as $2 or CBS_UPSTREAM_CACHE and copies its contents
	# in. What it needs is someone to supply one, which is exactly what
	# an embedder is for. Those two tarballs are declared as extra
	# sources above, so cixd fetches and checksum-verifies them
	# host-side and drops them in /build/extra (ADR-0036); CBS looks
	# each up as <cache>/<sha256>, so the copy is a rename.
	mkdir -p /run/cbs-upstream
	cp /build/extra/2ba12e83b3599ca8f5d50c179fe5138fe956f0c9 \
	   /run/cbs-upstream/4eb5f0266d4d9deabe9650abedc3f0261dc06295f89ad798bb495abf680dc074
	cp /build/extra/v1.5.4.tar.gz \
	   /run/cbs-upstream/35ad983197f8f8eb0c963877bf8be50490a0b3df54b4edeb8399ba8a8b2f60a4
	ls -l /run/cbs-upstream
	export CBS_UPSTREAM_CACHE=/run/cbs-upstream
