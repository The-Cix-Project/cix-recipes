#
# v2.55.11: test_direct_rootfs stages a device node and reports the code.
#
# v2.55.10's dt4 assertion failed for a reason unrelated to nodev, while
# its message named nodev. The shared fixture builder stages no device
# nodes -- the /dev set lives in stage_toolchain(), which this test does
# not use -- so the open failed ENOENT. It stages its own node now and
# prints the child's exit code and output before asserting.
#
#
#
# v2.55.10: a direct-rootfs container can open a device node again.
#
# ntp-1/ntp-2 crash-looped on "Could not open /dev/urandom : Permission
# denied" while six containers beside them ran -- they are the only
# userns:false workloads, so the only ones on this path. Not the device
# policy (a container declaring no devices gets no BPF program at all):
# boot_init() mounts the containers partition MS_NODEV, and ADR-0207
# phase 2 replaced this path's overlay with a self-bind of it, which
# inherits the flag. #173 on the path that fix said could never need it.
#
# stat() succeeds on a nodev mount and only open() fails, which is why
# every node looked fine. daemon_child gains an optional device to open;
# test_direct_rootfs asserts it.
#
#
#
# v2.55.9: test_pkg_finalize provides wc, and asserts the refusal.
#
# v2.55.8's gate refused with "policy exited 1 on a well-formed tree":
# the test sets PATH to a stub directory alone, so every tool the policy
# declares must be provided there, and wc was not. A fourth case now
# asserts the policy REFUSES without wc and names coreutils -- wc is
# what measures an archive, so a skip rather than a refusal would drop
# libpthread.a again, which is the whole of #324.
#
#
#
# v2.55.8: measure an archive with wc, not with a read builtin.
#
# v2.55.7's gate refused it: "libc.a survived and must be removed". The
# shell cannot measure binary at all -- a NUL byte cannot be held in a
# variable and read stops at one regardless of -N or -d '', so both
# spellings reported eight bytes for a 64-byte NUL-padded archive and
# every archive was kept. test_pkg_finalize's own helper pads with NUL,
# which is why it caught this at once.
#
# wc -c, named in the tool check beside rm; both are coreutils, which
# the prune already required.
#
#
#
# v2.55.7: pkg-finalize's clause 2 does not reach an empty archive.
#
# btop failed with "ld: cannot find -lpthread". glibc ships
# libpthread.so.0 and neither libpthread.so nor libpthread.a, the only
# two names ld -lpthread looks for -- clause 2 dropped the archive on a
# stem match with the runtime stub. But an archive with NO MEMBERS is
# eight bytes and duplicates nothing, so the reason that clause exists
# does not apply to it; for glibc's folded-in stubs it IS the link-time
# contract. Member-less archives are kept now; libc.a still goes.
#
# Every gcc package passing -pthread was affected. Not fixed with a
# libpthread.so symlink, which would make every such binary record a
# DT_NEEDED upstream deliberately avoids.
#
#
#
...
#
pkg_name="cix"
pkg_version="v2.55.11"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.55.11.tar.gz"
pkg_sha256="0099343e8ec989e4ca94a3d765d19b33d58a2ca2db34ef14caa3133eb0990f7b"
pkg_artifact_sha256="8047d78afc6b7dacfcf51911d7852be46241fd31fbe0a711e6a81713f7081338"
