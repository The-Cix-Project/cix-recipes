#
# v2.55.21: make a container's stop and exit visible.
#
# Two selftests fail on a network delete refused 409 because the
# container that held it is still tearing down, and both already wait
# for it to disappear -- so the teardown is taking longer than the
# SIGTERM path should need, and nothing anywhere says why. cix-init's
# own SIGTERM handling is correct in isolation (test_cix_init case 12,
# measured), so the question is what the daemon and the container are
# actually doing, and until now neither said.
#
# So: the daemon prints the stop (signal, pid, grace), the exit
# (status, signal, lifetime) and the grace expiry. This revision exists
# to be read.
#
pkg_name="cix"
pkg_version="v2.55.21"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.55.21.tar.gz"
