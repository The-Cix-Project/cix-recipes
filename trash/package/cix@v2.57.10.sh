#
# v2.57.10: the assembly trigger (#308) with its CLI caller, and #347's
# module staging reaching its second assembly.
#
# v2.57.9 shipped POST /v1/system/assembly with no CLI caller and
# failed its own build on two gates: test_apigen (291 operations,
# expected 290 -- a number that has to change deliberately) and
# test_api_surfaces ("postSystemAssembly is declared x-cix-expose [cli]
# but the CLI never calls it"). An x-cix-expose list is a claim about
# what a surface offers, and it is checked rather than trusted.
#
# cixctl assembly status|start is that caller. And the completion of
# this hostbuild is itself the second assembly #347 needs, since the
# module staging runs in the daemon that PERFORMS the assembly, and
# that daemon is now v2.57.8 or later.
#
pkg_name="cix"
pkg_version="v2.57.10"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.10.tar.gz"
pkg_sha256="852b3a75c55d3ce32768eb2a317cf34ddedd3071ad8be636a97ea2427e3ac0fe"
pkg_artifact_sha256="6e972c06c7f4fcddcf762b5d7f6c81028ad956d759d1741451259f96c8cc8a57"
