#
# v2.57.124: test_pki waits for a delete to settle, not for an error
# message (#413, #417, #421)
#
# v2.57.123's gate failed on test_pki, in the code #417 added a cycle
# earlier: post_container_retrying() retried a create past a 409 whose
# MESSAGE said "shutting down", and the one time it mattered the message
# was the other branch, so the retry never fired. It now polls the
# container's own GET to 404 before recreating -- the protocol
# docs/api/README.md already documents. A GET is an interface; an error
# body is not.
#
# Why that 409 reported no teardown in progress for a name that did not
# free up is not established, and is #421 rather than a guess.
#
#
pkg_name="cix"
pkg_version="v2.57.124"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.124.tar.gz"
pkg_sha256="e6309e9ee69b826a8cc6ed4f4ed32e597037ea8c66a21e5f661d7a94ef379b82"
pkg_artifact_sha256="df42f9efb9c0e7637e7cde681b6cffca251703529f630f372e039d78b6a5410e"
