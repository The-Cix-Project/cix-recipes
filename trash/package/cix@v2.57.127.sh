#
# v2.57.127: which device backs each platform partition is an API fact
# (#305)
#
# #305's resolver and root=PARTUUID= loader entries shipped earlier;
# what was missing was any way to see the result. It went to stderr,
# which on an installed box is the serial console -- and a disk coming
# back under a different name is exactly when nobody has one. The log
# store cannot carry it either: these resolve before anything is
# mounted, and the store lives on the partition being resolved.
#
# GET /v1/system/boot gains platform_devices; cixctl boot prints one
# line per partition; test_daemon asserts the shape and not the values,
# since a test daemon is not an installed system.
#
# #305 stays open -- done is a real boot on renamed hardware, which
# needs one hypervisor change, and this is what makes it readable.
#
#
pkg_name="cix"
pkg_version="v2.57.127"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.127.tar.gz"
pkg_sha256="f46caccf1a1140fcf6ec1e3667905f90835e7230463b3d9c8f44a69011b91b90"
pkg_artifact_sha256="2cf7c10d7a82e35bed4e955b2ef9bd0ce304cb02d731b81df2ebad2b643cfd00"
