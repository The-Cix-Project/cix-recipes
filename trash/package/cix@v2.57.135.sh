#
# v2.57.135: the installer can see a real machine's built-in Ethernet
# (#429 follow-on)
#
# It could not, and the reason was structural: the five NIC chipsets
# this platform supports are kernel MODULES, and the installer ISO
# carried no module tree and no module tools. So the only interfaces
# cix-install could list were those whose driver is built in -- in
# practice virtio_net alone -- and a real machine with a built-in
# Ethernet port showed "(none found)". Reported from a bare-metal
# attempt, 2026-09-12.
#
# mkinstalleriso stages the drivers and modprobe onto the media, and
# cix-install loads them before listing, through kmod_load() -- the
# daemon's own runner, not a second way to run modprobe. The five
# names live in include/bootmodules.h and are shared with cixd's
# load_boot_modules(): a drifted copy would offer an interface the
# installed system cannot bring up, or vice versa.
#
# Staged by dependency closure read out of the kernel's own
# modules.dep. Measured: ixgbe needs mdio, mdio_devres, libphy and
# mdio_bus; r8169 needs mdio_devres, libphy, mdio_bus; tg3 needs libphy
# and mdio_bus. A hand-written list of five .ko files would have loaded
# e1000e and igb and failed the other three -- which presents exactly
# like staging nothing. 9 files, 2.06 MiB, plus 422 KiB of modules.*
# metadata. The rest of the tree, which now carries amdgpu and rtw88,
# is left out: media for choosing a NIC has no use for a GPU driver.
#
# Absence of either input is not fatal -- such media still installs on
# virtio, and cix-install says which of the two it lacks, since "no
# tree" and "no tools" are different diagnoses.
#
# Two things found while wiring the call site: char *argv[15] had no
# room for the two new arguments (#415 was a real overflow on this same
# pattern), and kmod.c needs json.c for the endpoint-rendering half
# cix-install never calls. resolve_kernel_modules_dir() and
# resolve_kmod_bin_dir() are extracted rather than copied, so one place
# knows where a kernel artifact keeps its modules.
#
#
pkg_name="cix"
pkg_version="v2.57.135"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.135.tar.gz"
pkg_sha256="49611b2e0ca6bcd1968aadc0b0406d12cbaee38572dbef43ff5f0d65fb049cee"
pkg_artifact_sha256="64a5a0980a8d0b2beac140f93f4296242cf3f831ee5321820b54af57f9c541b6"
