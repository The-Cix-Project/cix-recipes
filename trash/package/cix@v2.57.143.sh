#
# v2.57.143: the dashboard survives blocked storage, and a listener
# knows where it is bound (#453, #454).
#
# #453 is why this build exists rather than waiting: an unguarded
# localStorage read at module scope threw NS_ERROR_FAILURE in a real
# browser and stopped app.js before anything initialised, so the
# dashboard rendered its static HTML and loaded no data at all. It
# looks like a dead daemon and is a storage permission. web/ lives
# inside the control-plane squashfs, so there is no live fix.
#
# #454 was found by finally running the reboot-survival test ADR-0283
# and ADR-0284 both asserted and neither had run. A live management
# address move never rebound the HTTPS listener: the guard compared
# strcmp(new_bind_addr, g_bind_addr) while the caller passed
# g_bind_addr itself, so it was strcmp(x, x) -- always "nothing to do".
# http answered on the new address, https answered nowhere, and only a
# reboot repaired it.
#
# Also carried: jump's deployment recipe gains dns_servers (it could
# not resolve anything at all -- ADR-0143 stages no resolv.conf when
# the field is empty), and the create form's Sysctls field moves from
# Lifecycle to Networking.
#
pkg_name="cix"
pkg_version="v2.57.143"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.143.tar.gz"
pkg_sha256="8d8eb4840e80e7b0276d664183e25d3177c601611706d8a7c93ed9bcf9fa550f"
