#
# v2.57.19: the virtualised /proc reaches containers (#336, ADR-0262
# part 2).
#
# Part 1's server compiled clean and is running; this binds its files
# over each container's own /proc, in the one window where that is
# possible -- after the fresh /proc is mounted inside the container's
# mount namespace, and before the old root is detached, since the
# host-side source is only reachable through put_old until then.
#
# Non-fatal per file: a container that does not get them reads the
# host's real /proc, which is what every container did before this
# existed. procfuse:false on the create declines it; absent means on.
#
pkg_name="cix"
pkg_version="v2.57.19"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.19.tar.gz"
pkg_sha256="e71ef1cf761d993f3f983986c5a5115f54a3df22836279ba5dced4ec596d7368"
pkg_artifact_sha256="b41cc6a1da9347e421e7d672f73c5df85f7fb75855d3b6ecb49aa5a52bf979da"
