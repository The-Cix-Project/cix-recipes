#
# v2.57.20: find out why the procfuse bind bound nothing (#336).
#
# v2.57.19 shipped the bind and jump's /proc/self/mountinfo shows no
# per-file mounts at all -- with no diagnostic anywhere, because the
# bind runs in the container child whose perror() goes to stderr, and
# stderr is never mirrored into the log store. A non-fatal failure
# there is invisible.
#
# The parent now logs the path it resolved, and the child names source,
# target and errno. Also allow_other on the mount: containers are
# idmapped by default, so their processes run as mapped uids and a FUSE
# mount is otherwise reachable only by the uid that mounted it.
#
pkg_name="cix"
pkg_version="v2.57.20"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.20.tar.gz"
pkg_sha256="87a3ad92f476e8edf5e06298ed78b8ab6861958bc11866988f0ec882695f596c"
pkg_artifact_sha256="b76c66087e33534953a47975275d292b45acf6ce27bd74fb8464919ec6c256df"
