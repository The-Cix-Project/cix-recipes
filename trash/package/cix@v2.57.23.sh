#
# v2.57.23: a container's /proc numbers are believable, not just
# present (#359).
#
# /proc/stat's idle column was zero, and every CPU tool computes
# 100 x (total - idle) / total -- so the denominator was the busy time
# and the answer was 100% at any load. A container using half a CPU
# read 100%. stat and meminfo now pass the host's file through and
# override what the cgroup knows, which also restores btime (ps derives
# wall-clock start from it), the per-CPU lines htop needs, and 42
# meminfo fields that were simply absent.
#
pkg_name="cix"
pkg_version="v2.57.23"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.23.tar.gz"
pkg_sha256="12efcb4048672cf178014265c7c8e3800de83e15a87d76e711dc01e478dd3355"
pkg_artifact_sha256="7033d208d928d69ef5d1a77981b6c5740a27081ad253b94b7d778d80e0491311"
