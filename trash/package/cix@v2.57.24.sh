#
# v2.57.24: the CPU cap belongs in the idle denominator (#359).
#
# Measured on v2.57.23: a container with cpuset_cpus "0" and cpu.max
# "50000 100000", running one busy loop for 20 s, used 1003 ticks --
# exactly its half-CPU cap -- and a CPU tool read 50.1%. Half idle, for
# a workload throttled flat out and unable to consume another cycle.
# That is #278/#279's own shape: a number claiming headroom that does
# not exist.
#
# The denominator is now min(cpuset count, quota/period). cpuinfo still
# follows the cpuset alone -- parallelism and throughput are different
# limits, and a quota-derived processor count would make every
# make -j$(nproc) into -j1.
#
pkg_name="cix"
pkg_version="v2.57.24"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.24.tar.gz"
pkg_sha256="f6794f4dbd8a76477e893481534e7e62a0ad807c60d1035defebe2eac087e348"
