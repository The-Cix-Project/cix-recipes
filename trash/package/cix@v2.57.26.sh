#
# v2.57.26: a synthesised /proc/stat counter must still be monotonic
# (#359).
#
# v2.57.25 confirmed the cpu-cap fix -- a container pegged at a
# half-CPU cap read 100.1% where it had read 50.1% -- and the same
# output showed the idle column FALLING by a tick between samples:
# delta [1001, 0, 1, -1]. CFS bandwidth lets a container overrun its
# nominal quota within a period, so measured busy can exceed capacity
# computed from that quota. Consumers compute deltas from these
# counters, several in unsigned arithmetic where -1 is a number near
# 2^64.
#
pkg_name="cix"
pkg_version="v2.57.26"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.26.tar.gz"
pkg_sha256="1030cf253f9e1331c0ae3a5f95e1daa03436eb3c433e739b78f756a0770a4770"
