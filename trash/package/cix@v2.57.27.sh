#
# v2.57.27: v2.57.26's monotonic idle, plus one definition of how long
# a container takes to stop (#309).
#
# v2.57.26 failed on test_cli and test_container_lifecycle, both
# bounding async teardown at five seconds while the platform promises a
# 15-second SIGKILL grace -- a signal to a pid-namespace init with no
# handler yet is discarded rather than queued (ADR-0260), so a stop's
# duration is a property of the container's age. Eight such waits
# across seven files each guessed their own budget; they now share
# TEST_SETTLE_ATTEMPTS.
#
pkg_name="cix"
pkg_version="v2.57.27"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.27.tar.gz"
pkg_sha256="6e8e3b55dba1da3d399ee9811c9bfdb6169ea8d82cf8d0ab54ebf39ec9c6f899"
pkg_artifact_sha256="f0681c9fa7b423ea1680949f6a10d401022d60ff65ddea9368a8b58b5fa0bf7b"
