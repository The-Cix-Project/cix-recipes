#
# v2.57.30: a container's virtualised /proc is reported, not assumed.
#
# jump reported 7.71 GiB and two CPUs to htop against real limits of
# 1 GiB and one CPU. The daemon had offered it a valid procfuse path --
# the binds failed inside the child, where mountns_pivot() reports them
# with perror() to the container's own stderr, which nothing collects.
# The parent now counts the landed binds in the child's own
# /proc/<pid>/mountinfo and records procfuse_expected/procfuse_bound on
# GET /containers/{name}. Non-fatal by choice: this host is shell-less.
#
# GET /containers/{name}/files sized every read from st_size, so a
# procfs file (st_size == 0) returned 200 with an empty body -- which
# is why the fault above could not be measured from outside a container
# at all. Zero-size files now read to EOF.
#
# cix-init moves to /sbin/cix-init behind one definition, and
# mountns_pivot() rmdir()s the .old_root husk it used to leave in every
# container's overlay.
#
pkg_name="cix"
pkg_version="v2.57.30"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.30.tar.gz"
pkg_sha256="5c2a9c4f9679510dedc3692b9f782792d133f9b3de755733482b9d42e73ada24"
pkg_artifact_sha256="cc5a6a858f41f14c67e459dee5b845ddd65f3617421d79ffa3585a1b89e2cdf9"
