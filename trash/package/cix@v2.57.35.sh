#
# v2.57.35: how full is this volume? (#365)
#
# A volume's record described its configuration and nothing about its
# contents. df inside the container cannot answer it and never will --
# a volume is a bind of a directory, statfs(2) reports the superblock,
# so df shows the whole backing filesystem. True of a container rootfs
# too, even one with a real qgroup limit; btrfs does not put qgroups in
# statfs. Measured.
#
# GET /v1/volumes/{name}/usage -> {name, bytes, quota_bytes,
# measured_at}, measured with overlay_upperdir_size() -- the same
# walker disk.upper_bytes uses, so both numbers mean the same thing.
# A separate endpoint because the walk is O(files) and GET /volumes is
# what the dashboard renders.
#
# cixctl volume usage NAME, and a Size column filled lazily per row.
#
pkg_name="cix"
pkg_version="v2.57.35"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.35.tar.gz"
pkg_sha256="09a3cd5ed007b241e55a2a465bf9d82ecef1ebd336cd5c8071ee72bca77377f5"
pkg_artifact_sha256="7acc19654bc6ccf38a2f6d8137112b59d7f18250e4dc4739a07268714ea93f11"
