#
# v2.57.38: an open control plane is no longer silent (#370)
#
# v2.57.37 was published with a checksum, then its git tag was moved --
# recipe versions are immutable, so the box kept the first checksum and
# refused the retagged tarball. The fix for a published version is a new
# version, never a moved tag. This is that version.
#
# 192.168.15.95 accepted unauthenticated writes for four days and a
# full day of deploys went through it. Found by accident, when a deploy
# script failed on a missing token file nothing had needed.
#
# The cause was the 2026-09-06 reinstall wiping hostauth_config.json
# and the cix-admins group. That part is a reinstall doing its job.
# Two things were real defects.
#
# Gating needs an admin group configured AND an enabled user in it, and
# five ordinary operations broke the second half while the first still
# read correctly: deleting the admin group, renaming it or changing its
# gidnumber (config names it by NAME, members join by GID), deleting
# the last admin, disabling or de-admining them, and pointing
# admin_groups at an empty group. Each is now 409. Only the LAST admin
# is protected, and only turning ACTIVE gating off is refused -- setting
# admin_groups before the admins exist is how gating gets enabled.
#
# And it was invisible: /health said {"status":"ok"}, nothing logged it
# at boot, and the dashboard drew an open box like a secured one. Now
# gating_active, auth_gating_active, a boot warning and a banner.
#
# NOT done: failing closed, which turns an orphaned config into a
# lockout recoverable only via cix-recover on a shell-less host.
#
# ADR-0268.
#
pkg_name="cix"
pkg_version="v2.57.38"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.38.tar.gz"
pkg_sha256="2e3a6e4fc0bbae69e3f0865d7ff5bca06057fb299be10b239b1d4d8c494757fd"
pkg_artifact_sha256="7c574e0c30969f951a9f256ab662e6aea4bc57d11af54de68539a8fb1ada88a2"
