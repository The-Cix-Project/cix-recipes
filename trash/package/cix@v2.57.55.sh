#
# v2.57.55: the platform says what it is (#387, ADR-0274)
#
# Cix shipped no /etc/os-release, on the host or in any container image
# -- confirmed against the running jump container, which answered
# {"error":"no such file"}. The freedesktop spec's documented fallback
# when ID is absent is "linux", so every tool that asked what this
# platform is got the one answer a system built from source
# specifically so that it is not somebody else's distribution should
# never give. Silently.
#
# osrelease_render() is ONE definition, called from both places a Cix
# filesystem is assembled: mkbootroot for the control-plane root, and
# pkg_seed_image_baseline() for every container image. Two literals
# would drift, and that drift is a host and its own containers
# disagreeing about what they are.
#
# ID=cix is now a compatibility surface. No VERSION_ID: Cix is
# rolling-release, and setting it to the daemon build -- the plausible
# mistake -- would make every host-to-host release comparison answer
# confidently and wrongly. BUILD_ID carries the build, the same string
# GET /v1/system/boot reports. No URL fields; everything points at
# git.home.arpa.
#
# test_osrelease is in SELFTESTS and gates the deliberate omissions as
# well as the fields, because a test that only checks what exists
# cannot protect a decision not to write something.
#
# Also in this build: cmake and fastfetch recipes (the first CMake-only
# package), and test_toolchain_policy corrected to ADR-0226's wording.
#
pkg_name="cix"
pkg_version="v2.57.55"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.55.tar.gz"
pkg_sha256="56ff5fe2c6acfdbba5f53ddb3acadc8f10a6c4d09facb5728f6190bb770e0c85"
