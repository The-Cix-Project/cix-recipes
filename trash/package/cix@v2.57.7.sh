#
# v2.55.25: no package could be built under v2.55.24.
#
# A build container's capture pipe and the output fd named on
# cix-init's argv are the same descriptor by design -- one service, one
# stream. container_create()'s capture_output block dup2s it onto 1 and
# 2 and then closed the original, which predates anything needing that
# original to live; ADR-0260's fcntl(F_SETFD) loop then cleared
# close-on-exec on a closed fd and got EBADF. Every build failed before
# writing a byte.
#
# The daemon reported it as "build killed by signal 10", because pkg.c
# carried its own copy of container.c's exit-code tables, 138 was added
# to the real one and not the copy, and 138 is also 128+10. The copy is
# gone -- pkg.c calls container_decode_exit_status() like registry.c
# does.
#
# This shipped because the release that introduced it was built by the
# daemon it replaced.
#
pkg_name="cix"
pkg_version="v2.57.7"
pkg_source="https://osakka:{{REPO_TOKEN}}@git.home.arpa/api/v1/repos/itdlabs/cix/archive/v2.57.7.tar.gz"
pkg_sha256="f325a0c4fa836c28021e3af3df1dc77f3eff7ad8dc7241d8fdfb602ff7abd010"
pkg_artifact_sha256="42202b3766ba5ab83b8328d76549901033c140f0e766179cfcd95bbe638aff1d"
