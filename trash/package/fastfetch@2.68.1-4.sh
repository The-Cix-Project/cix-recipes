...
#     platform shipped none. THAT IS NOW FIXED (ADR-0274): a real
#     /etc/os-release is staged into every container, and fastfetch in
#     `jump` correctly reports `OS: Cix x86_64`. What it still cannot do
#     is find the Cix LOGO from it -- see 2 -- so it drew the generic
#     Tux penguin beside a correct "Cix". Identification and logo
#     selection are separate lookups and only the first one was fixed.
#
#  3. Revision 3 pointed fastfetch at that logo through
#     /etc/fastfetch/config.jsonc and, in doing so, emptied the
#     module list -- see the comment on that file in pkg_install().
#     Revision 4 declares the modules and gates the config it ships.
#
...
pkg_changelog="2.68.1-4: the config revision 3 installed selected the logo and SILENCED EVERY INFORMATION MODULE. Measured in jump: with that config fastfetch printed the Cix mark and not one line beside it; the same binary with --config none printed the full default set. A loaded config IS the module list, and one with no \"modules\" key is a list of none. This revision declares the set explicitly. No change to the logo or the binary."
...
pkg_install() {
	...
	#
	#
	# And point fastfetch at it by default.
	#
	# Shipping the logo file was not enough: `ID=cix` in /etc/os-release
	# makes fastfetch look for a BUILT-IN logo named "cix", there is no
	# such built-in (see 2 above), and it falls back to the generic Tux.
	# So a correct "OS: Cix" appeared beside a penguin, and the only way
	# to get the real mark was to pass --logo by hand every time.
	#
	# /etc/fastfetch/config.jsonc is fastfetch's own documented
	# system-wide config path, so this is the mechanism rather than a
	# wrapper around one. type "file" (not "file-raw") is required: the
	# logo uses $1/$2 colour placeholders, and file-raw would print them
	# literally -- the exact failure pkg_build()'s logo gate below
	# already tests for.
	#
	# A user's own ~/.config/fastfetch/config.jsonc still wins, which is
	# the right precedence: this is a default, not a policy.
	#
	# "modules" is not optional here. fastfetch's default module list
	# is compiled into the binary and is used only when NO config is
	# found -- /usr/share/fastfetch/presets/ holds alternative presets
	# (all, archey, neofetch, ...) and no default.jsonc to inherit
	# from. So the moment this file exists it IS the module list, and
	# revision 3's config, which set only the logo, was a list of
	# none: fastfetch drew the Cix mark with nothing beside it.
	#
	# The set below is what this platform should say about itself, not
	# a copy of fastfetch's default. Desktop modules (DE, WM, theme,
	# icons, font, cursor, player, media) are left out deliberately --
	# Cix has no desktop and each would print nothing or an error. The
	# rest is the default order, verified rendering in `jump` before
	# this revision was written.
	#
	mkdir -p "$PKG_DESTDIR/etc/fastfetch"
	cat > "$PKG_DESTDIR/etc/fastfetch/config.jsonc" <<'CFG'
{
    "logo": {
        "type": "file",
...
CFG
	...
	#
	# And a gate on the config as installed, which is the one thing
	# revision 3 had no test for. Its gates all passed `--structure
	# Title`, and --structure OVERRIDES the config's module list -- so
	# every gate ran against a module selection the shipped file would
	# never produce, and the empty output shipped.
	#
	# `Kernel:` is the assertion because uname needs nothing from the
	# build container: no /etc/os-release, no DMI, no hardware. --logo
	# none because the config points at an installed path that does
	# not exist yet at build time; the flag overrides the config's
	# logo rather than failing on it (confirmed in a real container
	# before this was written).
	#
	cfg_out=$(./build/fastfetch --pipe --logo none \
		--config "$PKG_DESTDIR/etc/fastfetch/config.jsonc" 2>&1) || {
		echo "fastfetch could not run with the config it installs: $cfg_out" >&2
		exit 1
	}
	case "$cfg_out" in
	*"Kernel:"*) ;;
	*)
		echo "the installed config produced no information modules:" >&2
		echo "$cfg_out" >&2
		exit 1
		;;
	esac
	echo "=== installed-config gate: modules render ==="
	echo "$cfg_out" | sed 's/^/  /'
