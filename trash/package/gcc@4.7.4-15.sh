	...
	*)  seed_fixed="/usr/lib/gcc/$(/usr/bin/gcc -dumpmachine)/$(/usr/bin/gcc -dumpversion)/include-fixed" ;;
	...
	fixup_stage_headers() {
		for d in gcc prev-gcc stage1-gcc stage2-gcc stage3-gcc; do
			...
			mkdir -p "$fixed"
			if [ ! -f "$fixed/limits.h" ] || ! grep -q include_next "$fixed/limits.h"; then
				compose_limits_h "$fixed/limits.h"
			fi
	...
	#
	make bootstrap LIMITS_H_TEST=true -j"$(nproc)"
	make_rc=$?
	#
