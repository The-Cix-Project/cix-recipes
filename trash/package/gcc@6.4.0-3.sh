...
MINIEXTRACT
	...
	# v11, not guessed), and genuinely different from the already-fixed
	# kernel/elfutils syslimits.h bug this signature initially looked
	# identical to: gcc-4.7.4-6's own
	# .../4.7.4/include-fixed/ directory exists but is completely EMPTY
	# (confirmed via a direct `ls -la` in the same build-image context)
	# -- not a broken syslimits.h, no compiler-private limits.h at all.
	# Same underlying mechanism as the kernel's own fix, just triggered
	# by absence instead of a broken chain: gcc-4.7.4-6's own private
	# search path (checked first) finds nothing, falls straight through
	# to glibc's real /usr/include/limits.h, which -- unconditionally,
	# whenever __GNUC__ is defined -- demands a SECOND, compiler-
	# provided limits.h reachable further down the search path via its
	# own #include_next; with nothing there at all, that #include_next
	# has nowhere left to search. Real, minimal, correct fix (same
	# passthrough gcc's own gsyslimits.h template is designed to be,
	# already proven for the kernel build): supply one, defining
	# _GCC_LIMITS_H_ and immediately chaining onward -- this time
	# creating it from nothing, not replacing a broken one.
