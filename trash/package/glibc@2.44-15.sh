...
pkg_changelog="2.44-15: no source or recipe change. The revision exists so this package is actually rebuilt, because an image version is a hash of the package manifest (ADR-0155) and a same-version reinstall is deduped and thrown away. What changed is outside this recipe: the finalize policy no longer drops an archive with no members, so the eight-byte libpthread.a/librt.a/libdl.a this build has always installed into /usr/lib now survive into the artifact -- and `ld -lpthread` has something to resolve against again (#324). They stay in /usr/lib only, where glibc puts them and where a compiler here searches; copying them into the multiarch directories would duplicate for no reason. 2.44-13: declare pkg_toolchain=gcc and its reason (#222, ADR-0226)"
