#
# go -- stage 2 of the bootstrap: the first Go built BY a Go (#261).
#
...
#
pkg_name="go"
pkg_version="1.19.13-1"
pkg_source="https://dl.google.com/go/go1.19.13.src.tar.gz"
pkg_sha256="ccf36b53fb0024a017353c3ddb22c1f00bc7a8073c6aac79042da24ee34434d3"
pkg_depends=""
#
...
#
pkg_build_depends="bash coreutils gcc binutils sed grep gawk go-bootstrap"
pkg_toolchain="gcc"
...
pkg_build() {
	...
	#
	if [ ! -x /usr/lib/go-bootstrap/bin/go ]; then
		echo "go: /usr/lib/go-bootstrap/bin/go is missing -- stage 1 (go-bootstrap) must be" >&2
		echo "    installed in this build environment before this stage can run" >&2
		exit 1
	fi
	...
	cd src
	TMPDIR=/run/gotmp \
	GOROOT_BOOTSTRAP=/usr/lib/go-bootstrap \
...
GO
	...
	echo "  compiled and ran a real program -- stage 2 works"
}
