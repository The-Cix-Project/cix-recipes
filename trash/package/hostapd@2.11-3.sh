...
pkg_version="2.11-3"
#
# Source moved back to hostapd's own upstream. It had been pinned to
# an ad-hoc LAN re-serve (http://192.168.15.31:8920/...), which is the
# documented unblock for a fetch that fails against a real upstream --
# but as a PERMANENT pkg_source it is worse than the dead Debian pool
# URL bird had: that at least was a real archive once. This one was a
# one-off convenience that stopped existing, and the recipe went with
# it:
#
#   curl: (7) Failed to connect to 192.168.15.31 port 8920
#
# w1.fi is the project's own release host and answers fine. The
# checksum is recomputed from the bytes actually downloaded, because
# the LAN copy was a renamed/repacked ".tarball" and hashes
# differently from upstream's .tar.gz for the same release.
#
...
pkg_changelog="2.11-3: sources from w1.fi, hostapd's own release host, instead of an ad-hoc LAN re-serve that no longer runs. 2.11-2: declares its build tools so it can be rebuilt through the ordinary install path (#206)"
#
# Build tools derived rather than guessed: the baseline the declaring
# recipes converge on, plus what this recipe's own pkg_build() invokes
# and the libraries it already declares. See
