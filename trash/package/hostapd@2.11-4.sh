...
WRAP
	...
	# Index 1 arrives as a plain file in /build/extra (multi-source entries
	# are fetched and checksum-verified but never extracted). Unpack it
	# into the tree under the name the build below expects.
	tar xzf /build/extra/40493a655d8caa2ccf5206dde1e733abe2920432 \
		-C . 2>/dev/null || tar xzf /build/extra/*.tar.gz -C . 2>/dev/null
	rm -rf libnl-tiny
	mv libnl-tiny-40493a655d8caa2ccf5206dde1e733abe2920432 libnl-tiny
	if [ ! -f libnl-tiny/genl.c ]; then
		echo "libnl-tiny did not unpack -- /build/extra held: $(ls /build/extra)" >&2
		exit 1
	fi
