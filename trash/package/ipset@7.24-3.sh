...
pkg_name="ipset"
pkg_version="7.24-3"
pkg_changelog="7.24-3: add pkgconf and diffutils, which the first build named as missing (#206)"
...
# Iterated against a real build, not guessed. The first attempt named
# both gaps itself:
#   configure: error: The pkg-config script could not be found  -> pkgconf
#   line 7431: cmp: command not found                           -> diffutils
# `cmp` is a diffutils binary, which is why 18 other recipes already
# declare diffutils -- it is a common configure-time need rather than
# anything specific to ipset.
