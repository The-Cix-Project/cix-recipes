...
pkg_changelog="1.8.13-2: enables the nftables backend, now that libnftnl>=1.2.6 exists as a Cix package (1.2.9); ships libxtables/libip4tc/libip6tc headers and pkg-config files so other packages can build against them; declares libmnl/libnftnl (#206, #207)"
...
pkg_install() {
	...
	# The three shared libraries, and the dev files that make them usable
	# by another package's build.
	for lib in libxtables libip4tc libip6tc; do
		ls "$PKG_DESTDIR"/usr/lib/$lib.so.* >/dev/null 2>&1 || {
			echo "$lib shared library missing" >&2
			exit 1
		}
		test -e "$PKG_DESTDIR/usr/lib/pkgconfig/$lib.pc"
	done
	test -e "$PKG_DESTDIR/usr/include/xtables.h"
	test -e "$PKG_DESTDIR/usr/include/libiptc/libiptc.h"
