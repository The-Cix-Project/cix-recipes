...
#
#   gcc         NOT the C compiler here -- tcc still compiles every
#               object, and this package is not a Tier-3 exception. It is
#               for one C++ header syntax check: keyutils' `all` target
...
pkg_build() {
	make CC=tcc CXX=/usr/bin/g++ LIBDIR=/lib/x86_64-linux-gnu USRLIBDIR=/usr/lib \
