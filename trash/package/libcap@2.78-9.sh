...
pkg_changelog="2.78-9: the -8 guard used `grep -r --include=`, which this build image's grep does not accept and reads as a filename -- so the guard errored, produced output, and reported a surviving shebang that did not exist. 2.78-8: declares findutils -- the -7 shebang fix used find, which this recipe had never needed before and so had not declared, and the build died at line 153 with 'find: command not found'. 2.78-7: rewrites #!/bin/bash shebangs to /usr/bin/bash -- the bash package ships bin/sh and usr/bin/bash but no /bin/bash, so progs/mkcapshdoc.sh died with exit 127 and blocked every dependent build. 2.78-6: libc-dev retired; linux-headers declared for the kernel uapi headers glibc's own limits.h needs (#187)"
...
pkg_build() {
	...
	# find+grep, NOT `grep -r --include=`: this build image's grep does
	# not accept --include and reads it as a filename, so that spelling
	# makes the guard fail on its own error output and report a
	# surviving shebang that is not there. That is exactly what -8 did,
	# and it is the third time this same grep difference has cost a
	# revision in this recipe set (libnl -1, libnftnl -1, here).
	if find . -name '*.sh' -exec grep -l '^#!/bin/bash' {} + | grep -q .; then
