...
#
# Deliberately NOT a kernel build. `make headers_install` preprocesses
# and copies the exported UAPI headers, and is independent of the kernel
# CONFIG used to build a bootable image -- which is why this is a
# separate, cheap package rather than an output of kernel.recipe: an
# image needs these headers without wanting a 50-minute kernel build.
#
# It is NOT compiler-free, which -1 of this recipe claimed and was wrong
# about. Before doing anything else the kernel build system compiles
# scripts/basic/fixdep with HOSTCC, and HOSTCC defaults to gcc:
#   HOSTCC  scripts/basic/fixdep
#   /bin/sh: line 1: gcc: command not found
# fixdep is a small, ordinary C program with no GCC-specific
# requirements, so HOSTCC is pinned to tcc here rather than reaching for
# the Tier-3 gcc exception. Kernel-proper would be a different question;
# this is one host tool.
#
...
pkg_depends=""
# ADR-0199/0209: composed from exactly these. tcc is here for HOSTCC
# (see above), libc-dev for the headers and CRT it needs to link
# fixdep.
pkg_build_depends="bash coreutils make tcc libc-dev sed grep gawk findutils"

pkg_build() {
	# headers_install writes into usr/include/ under the build tree; it
	# does not compile, so ARCH is the only thing it needs told.
	make ARCH=x86 HOSTCC=tcc headers_install INSTALL_HDR_PATH=/build/hdr
}
