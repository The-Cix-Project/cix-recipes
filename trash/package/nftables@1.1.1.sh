...
pkg_changelog="1.1.1: first packaging -- the nft CLI, which no amount of libnftnl provides on its own. Built --without-cli: the interactive shell needs libreadline/libedit/linenoise and none is packaged; every non-interactive use is unaffected (#207, ADR-0222)"
...
pkg_install() {
	...
	# It must really link the netlink stack and gmp this recipe declared.
	for lib in libnftnl libmnl libgmp; do
