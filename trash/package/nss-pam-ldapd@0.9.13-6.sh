...
pkg_version="0.9.13-6"
pkg_changelog="0.9.13-6: rebuilt so libnss_ldap.so.2 and pam_ldap.so finally carry PT_GNU_STACK (#228). Without that header the loader reads a shared object as requiring an executable stack, and glibc 2.44 refuses to grant it at dlopen time with cannot enable executable stack as shared object requires. The effect was an identity chain that half worked: getent passwd returned nothing for an LDAP account while id returned the full record, because glibc takes its stack bookkeeping from the main executable and skips the check entirely when that executable is itself unmarked, so a marked getent was refused and an unmarked id and sshd were not. tcc has never emitted the header and the fix for that landed in tcc 0.9.28rc-10 on 2026-09-01, but the build image was never upgraded past rc-9, so every package built in the four days since still lacked it, including 0.9.13-5. cix-builder now has rc-19 and this revision exists because an image version is a hash of the package manifest (ADR-0155), so a same-version rebuild is deduped and thrown away. No source change. 0.9.13-5: install to /usr/lib rather than the multiarch directory (#184 stage 2), together with linux-pam 1.6.1-6. The pam-seclib-dir must name the same directory libpam has compiled into its own DEFAULT_MODULE_PATH, so these two move in one change or PAM looks where this module is not. The artifact approval is dropped because those bytes came from the previous revision. 0.9.13-4: rebuilt against tcc 0.9.28rc (ADR-0223). The 2017 0.9.27 release could give two simultaneously-live locals the same stack slot (#216), a fault that corrupts values silently wherever the aliased pair is only read and written, so every binary it produced is suspect rather than merely the ones that failed. No source change: the revision exists to make the rebuild real, because an image version is a hash of the package manifest (ADR-0155) and a same-version reinstall is deduped and discarded. 0.9.13-3: declare build tools so this can be rebuilt (#206); rebuilt against the TCC-clean linux-pam 1.6.1-4 and openldap-client 2.6.14-3 (#113)"
...
pkg_artifact_sha256="fe3c7af92cdd7023a3c6b0fa38b7ac5894a94a98f28624c12e13a66d0a6e4a88"
