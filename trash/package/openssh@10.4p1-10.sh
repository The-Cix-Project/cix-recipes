...
pkg_changelog="10.4p1-10: declares libxcrypt instead of copying the build host's libcrypt.so.1 into the package -- a foreign binary, and the wrong soname besides, since Cix's libxcrypt provides libcrypt.so.2. 10.4p1-9: declares its build tools so it can be rebuilt through the ordinary install path (#206)"
#
# Build tools derived rather than guessed: the baseline the declaring
# recipes converge on, plus what this recipe's own pkg_build() invokes
# and the libraries it already declares. See
