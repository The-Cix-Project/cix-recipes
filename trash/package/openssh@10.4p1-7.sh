...
# retired (pkg recipe rm) once root-caused; -7 is the real fix.
...
#
# --with-privsep-user=sshd / --with-pid-dir=/run / --with-privsep-
# path=/var/empty: unchanged from 10.4p1, see that recipe's own
# comment for why each is needed. CC=tcc: see this file's own header
# comment -- this build sandbox's own default `cc` can no longer be
# trusted to actually mean tcc.
pkg_build() {
