...
# `--libdir=/usr/lib` gives `LIBDIR=` plus
# `libdir=/usr/lib`. The libraries are therefore INSTALLED
...
pkg_install() {
	...
	rm -rf "$PKG_DESTDIR/usr/lib/engines-3" \
	       "$PKG_DESTDIR/usr/lib/ossl-modules"

	# No relocation: with libdir at /usr/lib the install writes the .pc
	# files straight into /usr/lib/pkgconfig, where pkgconf looks
	# (#184). This recipe is where that whole class of fix started
	# (#174) -- the move existed only because libdir was somewhere
	# pkgconf does not search.
	test -f "$PKG_DESTDIR/usr/lib/pkgconfig/libcrypto.pc"
	...
	*"libdir=/usr/lib"*) ;;
	*)
		echo "libcrypto.pc does not name /usr/lib" >&2
		exit 1
		;;
	esac
}
