...
#
pkg_build_depends="gcc binutils make linux-headers bash coreutils sed grep gawk findutils diffutils"
pkg_changelog="5.40.1-4: built with gcc instead of tcc. TCC-built miniperl segfaulted deterministically partway through the build (#216), which no amount of recipe-level work fixes -- it is a codegen fault, not a flag problem. Switching compilers also removes the two TCC workarounds this recipe carried: the sed against perl's own perl.h, and the cc wrapper that translated -Wl,-E (and, until #214, mistakenly a bare -E too). A gate now runs the perl it just built and loads a real XS module through it, so a binary that builds but does not work fails here instead of on an installed host"
