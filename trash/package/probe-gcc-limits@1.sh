...
#
# EXPECTED RESULT: the install SUCCEEDS either way. Read the build
# log; the answer is the four lines under "=== verdict ===".
#
# Source is the wireless-regdb tarball, which fetched successfully
# from mirrors.kernel.org on 2026-09-22. A probe needs a real,
...
pkg_artifact_sha256="bd92ec8c592d6c1ee0e51d682e8f545cf23a239f39df7511101432fed7fb8cb2"
pkg_depends=""
pkg_build_depends="bash coreutils gcc binutils linux-headers diffutils"
pkg_changelog="1: re-measures whether gcc's include-fixed/syslimits.h is still a copy of limits.h, which elfutils@0.192-12 works around by rewriting that file -- a write CPDL cannot make, since it is outside the confined build roots. Not re-measured since gcc moved to 16.2.0."
...
EOF
	...
	echo "=== verdict ==="
	./limits_probe
	echo "  expected with a correct chain: PATH_MAX=4096 MB_LEN_MAX=16"
	echo "  broken chain looks like:       PATH_MAX=UNDEFINED MB_LEN_MAX=1"
}
