...
pkg_changelog="1: measures whether a non-blocking TCP connect to a listening local peer returns 0 or EINPROGRESS, which decides whether cix-init's tcp_port readiness probe can ever report ready (#413), and whether an inline poll(0) or a carried fd is the fix."
