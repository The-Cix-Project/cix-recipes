...
pkg_changelog="3: exercises ADR-0273's roll gate end to end against a real daemon -- hold, report as blocked_on approval, approve one of two queued images, and confirm the other stays held rather than the queue stalling behind it."

pkg_build() {
	echo "probe: probe-run-trigger-dep revision 3, nothing to build"
	true
}

pkg_install() {
	mkdir -p "$PKG_DESTDIR/usr/share/probe-run-trigger-dep"
	echo "probe-run-trigger-dep revision 3" > "$PKG_DESTDIR/usr/share/probe-run-trigger-dep/REVISION"
}
