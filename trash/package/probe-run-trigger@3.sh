...
pkg_changelog="3: source moved to git.home.arpa (ftp.gnu.org does not resolve from a build container on this host, and revision 1's apparent success was a cache hit); records that resolve_chain() skips an already-installed dependency regardless of version, so the probe uninstalls it first."

pkg_build() {
	echo "probe: probe-run-trigger revision 3, nothing to build"
	true
}

pkg_install() {
	mkdir -p "$PKG_DESTDIR/usr/share/probe-run-trigger"
	echo "probe-run-trigger revision 3" > "$PKG_DESTDIR/usr/share/probe-run-trigger/REVISION"
}
