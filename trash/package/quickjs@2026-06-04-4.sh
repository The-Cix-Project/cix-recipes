#
# quickjs 2026-06-04-4 -- a JavaScript parser the release build can run
# (#340).
#
...
# member references any. tcc links libtcc1.a, not libgcc, and nm over
# every archive in the tcc-0.9.28rc-29 artifact (the build cix-builder
# uses) finds no TImode symbol at all -- so nothing tcc links could
# supply them.
#
# NOT OBSERVED, and the -3 header said otherwise: the link was never
# seen to fail on one of these. The build that would have hit it died
# earlier, at -ldl, and never reached symbol resolution. -3 asserted
# the failure and quoted its error message, and also said tcc has no
# __int128 at all -- which is not established either; that token
# appears in the tcc binary three times. What is established: tcc
# ships no TImode runtime, and does not predefine __SIZEOF_INT128__
# (that string appears zero times in the binary, against __x86_64__,
# __linux__, __SIZEOF_POINTER__ and __SIZEOF_LONG__ at exactly one
# each -- the control that makes the zero mean something).
#
...
# __SIZEOF_INT128__ (measured above) and so chose 32 -- library and
# consumer disagreeing
...
pkg_artifact_sha256="47b9a005b65e950a8d553bd371673f97a62c9ce41e25221e43a3bee06f767042"
...
pkg_build() {
	...
	# This gates the LIBRARY's arm only. test_web_syntax.c carries the
	# consumer's -- _Static_assert(JS_LIMB_BITS == 32) -- because this
	# check would stay green if tcc ever grew __SIZEOF_INT128__ while
	# the two sides silently diverged.
