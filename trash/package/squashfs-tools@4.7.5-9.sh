...
#
# glibc implements pthread_exit's stack unwinding by loading
# libgcc_s.so.1 at the moment of the call, and aborts if it cannot.
# libgcc_s.so.1 ships only in the gcc package, which is not installed
# in cix-hosttools -- the image mkbootroot stages its tools from
# (ADR-0078) -- nor in base.
#
# Three of the four pthread_exit() calls are the last statement of a
# thread START ROUTINE (block_reader, fragment_reader, initial_reader),
# where returning is EXACTLY equivalent by definition: returning from a
# start routine terminates the thread with that value, which is what
# pthread_exit(NULL) does. Rewriting those is not a workaround, it is
# the same thing written without reaching for the unwinder.
#
# The fourth, in prep_exit(), is NOT a start routine -- it deliberately
# terminates whichever thread hit a catastrophic failure -- and is left
# alone. So this removes the dependency from every normal path and not
# from the failure path. That is a real, stated limit: the general
# problem (any threaded package needing libgcc_s.so.1) is not solved
# here and is tracked separately.
#
# An earlier note in this trail blamed -pthread. That was wrong and is
# corrected here: this Makefile links -lpthread, and the source calls
# pthread_exit() directly. The compiler upgrade did not add the call.
#
pkg_build() {
	cd squashfs-tools
	...
	#
	# Plain sed, line-based. 4.7.5-8 used `perl -0pi` for a multi-line
	# match and died with exit 127 -- perl is not in this recipe's
	# declared tools and should not need to be for a one-line
	# substitution. All three of reader.c's pthread_exit calls are a
	# whole line of their own, and all three are the last statement of
	# a thread start routine, so matching the line is exact: verified
	# against the real 4.7.5 source before this was written, not
	# assumed.
	sed -i 's/^\tpthread_exit(NULL);$/\treturn NULL;/' reader.c
	if grep -n 'pthread_exit' reader.c; then
		echo "squashfs-tools: a pthread_exit survived in reader.c" >&2
		exit 1
	fi
	grep -c 'return NULL;' reader.c | grep -qE '^[3-9]|^[0-9]{2}' || {
		echo "squashfs-tools: the rewrite did not produce the expected returns in reader.c" >&2
		exit 1
	}
	# prep_exit()'s call is deliberately untouched -- assert it is
	# still there, so a future broader rewrite has to be a decision
	# rather than an accident.
	grep -q 'pthread_exit(NULL);' mksquashfs.c || {
		echo "squashfs-tools: prep_exit's pthread_exit disappeared -- that call is deliberate" >&2
		exit 1
	}
