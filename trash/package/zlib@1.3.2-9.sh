...
# limit.
pkg_build_depends="tcc make bash coreutils sed binutils linux-headers"
# 1.3.2-8 (#187): libc-dev dropped, linux-headers declared in its place.
#
# 1.3.2-7 dropped libc-dev alone and failed to build, which is the whole
# result of that probe:
#
#   /usr/include/bits/local_lim.h:38: include file 'linux/limits.h' not found
#
# glibc's own <limits.h> chain includes a KERNEL uapi header. Our glibc
# package ships glibc's headers and not the kernel's -- those arrived
# only via libc-dev's wholesale copy of the build host's /usr/include.
# So the replacement for libc-dev is two things, not one: glibc (implicit
# in every composed environment, ADR-0216) plus linux-headers, declared
# by any recipe that actually compiles against libc.
#
# Original note from -7: libc-dev dropped. It never staged libc's headers -- it
